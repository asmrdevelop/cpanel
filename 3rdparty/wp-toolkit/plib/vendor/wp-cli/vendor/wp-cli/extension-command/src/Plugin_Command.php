...
class Plugin_Command extends \WP_CLI\CommandWithUpgrade {
	...
	private function delete_plugin( $plugin ) {
		...
		if ( Utils\check_proc_available( 'launch', true ) && !Utils\is_windows() ) {
	...
	/**
	 * Pure-PHP implementation of "rm -rf" Linux command.
	 *
	 * We need pure-PHP implementation as we could be unable to launch "rm -rf" due to disabled proc_open function.
	 *
	 * @param $path
	 */
	private static function delete_file_or_dir( $path ) {
		if ( is_dir($path) ) {
			$items = @scandir($path);
			if ( $items ) {
				foreach ( scandir($path) as $item ) {
					...
					self::delete_file_or_dir( $path . DIRECTORY_SEPARATOR . $item );
				}
			}
			@rmdir($path);
