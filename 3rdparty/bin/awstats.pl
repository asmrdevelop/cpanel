...
sub Read_Config {
	...
	if ($configdir) {
        ...
        ## cpanel patch
            ...
            if ( $ENV{'AWSTATS_SSL_DIR'} ) { 
                $awdir .= 'ssl/'; 
            }
            ## put $awdir after $DIR
            splice(@PossibleConfigDir, 1, 0, $awdir);
        }
        ## /cpanel
...
sub Read_Plugins {
    ...
    ## cpanel patch
    # Limit Plugins to approved directories
    my @PossiblePluginsDir = (
                              '/usr/local/cpanel/3rdparty/bin/plugins',
	...
	foreach my $plugininfo (@PluginsToLoad) {
		...
		$pluginparam ||=
		  "";    # If split has only on part, pluginparam is not initialized
                ## cpanel: (the spacing changed in the next line, mix of tabs and spaces)
		$pluginfile =~ s/\.pm$//i;
		$pluginfile =~ s/\.\.\///g; # Prevent directory traversal
                ## /cpanel
		$pluginfile =~ /([^\/\\]+)$/;
...
"<input type=\"hidden\" name=\"configdir\" value=\"$DirConfig\" />\n";
		...
		if ( $QueryString =~ /(^|&|&amp;)lang=(\w+)/i ) {
			print "<input type=\"hidden\" name=\"lang\" value=\"$2\" />\n";
                ...
                ## cpanel patch
                if ( $QueryString =~ /(^|&|&amp;)ssl=(\w*)/i ) {
                    print "<input type=\"hidden\" name=\"ssl\" value=\"$2\" />\n";
                }
                ## /cpanel
...
sub HTMLShowDomains{
	...
	print
"<tr bgcolor=\"#$color_TableBGRowTitle\"><th colspan=\"2\">$Message[17]</th>";
...
"<th class=\"datasize\" bgcolor=\"#$color_k\" width=\"80\">$Message[75]</th>";
	...
	foreach my $key (@keylist) {
		...
		if ( $newkey eq 'ip' || !$DomainsHashIDLib{$newkey} ) {
			print
"<tr><td class=\"aws aws_locale\">$Message[0]</td><td>$newkey</td>";
		}
		else {
			print
"<tr><td class=\"aws aws_locale\">$DomainsHashIDLib{$newkey}</td><td>$newkey</td>";
		}
...
"<img src=\"$DirIcons\/other\/$BarPng{'hk'}\" width=\"$bredde_k\" height=\"5\""
	...
	{    # All other domains (known or not)
		print
"<tr><td colspan=\"2\" class=\"aws\"><span style=\"color: #$color_other\">$Message[2]</span></td>";
...
"Warning: HostAliases parameter is not defined, $PROG choose \"$SiteDomain localhost 127.0.0.1\"."
	...
	if ($Debug) {
        ...
        ## cpanel patch
        # Test for Overlap with cpanellogd
        if ( -e "$LogFile.bkup" ) {
            # At this point we know that a backup file has been created, so
            # cpanellogd should be along shortly to process it.
            error(
                  "Log is currently being processed in the background. Cannot update.",
                  undef, undef, 1
                 );
        }
        ## /cpanel
