...
sub main_suicide_kids {
  my ($self, $now) = @_;

  # do not display any warnings during suicide
  local *STDERR;
  open STDERR, '>', '/dev/null' or warn("Could not redirect STDERR to /dev/null");
  ...
  foreach my $kid ( @kids ) {
     alarm 2; # give it a chance to exit properly
     eval { waitpid( $kid, 0 ); 1 } or do {
