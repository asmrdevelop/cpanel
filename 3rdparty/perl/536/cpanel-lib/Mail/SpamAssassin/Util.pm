...
#
sub untaint_var {
  return $_[0] if !${^TAINT} && !wantarray; # If we're not taint and the caller hasn't done a wantarray stunt, then just give back what was passed.
  ...
  if (!ref $_[0]) { # optimized by-far-the-most-common case
    ...
    # Bug 7120 does not apply to newer perls so this is safe to use
    return undef unless defined $_[0]; ## no critic (ProhibitExplicitReturnUndef)
    my $copy = $_[0];
    Taint::Util::untaint($copy);
    return $copy;
  } else {
    my $r = ref $_[0];
    if ($r eq 'ARRAY') {
