...
in version . It should be noted that many of these functions accept
...
an optional one, I<flag>, whose default value is 0. (The C<{}> should not be
...
C<{>I<x>C<}*>, means that any number of such arguments (possibly none) can
...
B<*> generic: all valid values for the flag are individually
described (``If I<flag> is equal to C<1>, then...'').

B<*> binary:X<binary flag> use customary binary notation as a
...
are set to C<0>). This is announced as ``The binary digits of I<flag> mean 1:
...
from description of C<X<ploth>ploth(X = a,b,>I<expr>C<,{>I<flag>C< = 0},{n = 0})>)
...
C<Recursive> is unset by default (default value of I<flag> is C<0>,
...
sign has to be typed in explicitly. As of version , this X<pointer>I<pointer>
...
X<gadd>The library syntax is B<gadd>C<(x,y)> C<x> C<+> C<y>, \funs{gsub}{x,y} for C<x> C<-> C<y>.
...
\funs{gsqr}{x} for C<x> C<*> C<x> (faster of course!).
...
\subsecidx{divrem}C<(x,y,{v})>: creates a column vector with two components,
...
\funs{gdiventres}{x,y} when C<v> is not needed.
...
PARI is able to rewrite the multiplication C<x * x> of two I<identical>
...
as here for B<Z>C</4>B<Z>, a result will be correct but chances are an error
...
\subsecidx{bittest}C<(x,n)>: outputs the C<n-th> bit of C<x> starting
...
\subsecidx{shift}C<(x,n)> or C<x> C<E<lt>E<lt> > C<n> ( = C<x> C<E<gt>E<gt> > C<(-n)>): shifts
...
\subsecidx{shiftmul}C<(x,n)>: multiplies C<x> by C<2^n>. The difference with
...
The library syntax is I<co>C<(x,y)>, where I<co> is the comparison
...
C<x-y>, where C<x> and C<y> must be in B<R>, and \funs{gequal}{x,y} which can be
...
C<gequal>: \funs{gcmp0}{x} (C<x == 0> ?), \funs{gcmp1}{x} (C<x == 1> ?), and
\funs{gcmp_1}{x} (C<x == -1> ?).
...
\subsecidx{lex}C<(x,y)>: gives the result of a lexicographic comparison
...
\subsecidx{sign}C<(x)>: X<sign>sign (C<0>, C<1> or C<-1>) of C<x>, which must be of
type integer, real or fraction.

X<gsigne>The library syntax is B<gsigne>C<(x)>. The result is a C<long>.

\subsecidx{max}C<(x,y)> and \funs{min}{x,y}: creates the
maximum and minimum of C<x> and C<y> when they can be compared.

X<gmax>The library syntax is B<gmax>C<(x,y)> and \funs{gmin}{x,y}.

\subsecidx{vecmax}C<(x)>: if C<x> is a vector or a matrix, returns the maximum
of the elements of C<x>, otherwise returns a copy of C<x>. Error if C<x> is
empty.

X<vecmax>The library syntax is B<vecmax>C<(x)>.

\subsecidx{vecmin}C<(x)>: if C<x> is a vector or a matrix, returns the minimum
of the elements of C<x>, otherwise returns a copy of C<x>. Error if C<x> is
empty.
...
\subsecidx{Col}C<({x = []})>: transforms the object C<x> into a column vector.
...
1-column) matrix, I<unless> all elements are column (resp.S< >row) vectors
...
\subsecidx{Mod}C<(x,y,{>I<flag>C< = 0})>:X<Label se:Mod> creates the PARI object
...
I<flag> is obsolete and should not be used.

X<gmodulo>The library syntax is B<gmodulo>C<(x,y)>.

\subsecidx{Pol}C<(x,{v = x})>: transforms the object C<x> into a polynomial with
...
B<Warning:.> this is I<not> a substitution function. It will not
...
\subsecidx{Polrev}C<(x,{v = x})>: transform the object C<x> into a polynomial
...
\subsecidx{Qfb}C<(a,b,c,{D = 0.})>: creates the binary quadratic form
...
\funs{qfi}{a,b,c} (when C<b^2-4ac E<lt> 0>), and
\funs{qfr}{a,b,c,d} (when C<b^2-4ac E<gt> 0>).X<binary quadratic form>

\subsecidx{Ser}C<(x,{v = x})>: transforms the object C<x> into a power series
...
\subsecidx{Set}C<({x = []})>: converts C<x> into a set, i.e.S< >into a row
...
\subsecidx{Str}C<({x}*)>: converts its argument list into a
...
\subsecidx{Strchr}C<(x)>: converts C<x> to a string, translating each integer
into a character.
...
\subsecidx{Strexpand}C<({x}*)>: converts its argument list into a
single character string (type C<t_STR>, the empty string if C<x> is omitted).
...
\subsecidx{Strtex}C<({x}*)>: translates its arguments to TeX
format, and concatenates the results into a single character string (type
C<t_STR>, the empty string if C<x> is omitted).

The individual arguments are read in string context, see L<Label se:strings>.

\subsecidx{Vec}C<({x = []})>: transforms the object C<x> into a row vector.
The vector will be with one component only, except when C<x> is a
...
\subsecidx{Vecsmall}C<({x = []})>: transforms the object C<x> into a row
...
\subsecidx{binary}C<(x)>: outputs the vector of the binary digits of C<|x|>.
...
\subsecidx{bitand}C<(x,y)>:X<Label se:bitand> bitwise X<and>C<and>
X<bitwise and>of two integers C<x> and C<y>, that is the integer

S<  >F<sum>C<_i (x_iS< >andS< >y_i) 2^i>
...
\subsecidx{bitneg}C<(x,{n = -1})>: X<bitwise negation>bitwise negation of an integer C<x>,
truncated to C<n> bits, that is the integer 

S<  >F<sum>C<_{i = 0}^{n-1} not(x_i)
2^i>
...
\subsecidx{bitnegimply}C<(x,y)>: bitwise negated imply of two integers C<x> and
...
\subsecidx{bitor}C<(x,y)>: X<bitwise inclusive or>bitwise (inclusive)
...
\subsecidx{bitxor}C<(x,y)>: bitwise (exclusive) X<or>C<or>
...
\subsecidx{ceil}C<(x)>: ceiling of C<x>. When C<x> is in B<R>, the result is the
...
\subsecidx{centerlift}C<(x,{v})>: lifts an element C<x = a mod n> of B<Z>C</n>B<Z>
...
as C<-1>. Also available is \funs{centerlift}{x} = C<centerlift0(x,-1)>.

\subsecidx{changevar}C<(x,y)>: creates a copy of the object C<x> where its
...
\funs{polcoeff}{x,n} gives the coefficient of degree C<n> of the polynomial
...
\subsecidx{conj}C<(x)>: conjugate of C<x>. The meaning of this
...
\subsecidx{conjvec}C<(x)>: conjugate vector representation of C<x>. If C<x> is a
...
\subsecidx{denominator}C<(x)>: denominator of C<x>. The meaning of this
...
\subsecidx{floor}C<(x)>: floor of C<x>. When C<x> is in B<R>, the result is the
...
\subsecidx{frac}C<(x)>: fractional part of C<x>. Identical to
C<x-floor(x)>. If C<x> is real, the result is in C<[0,1[>.

X<gfrac>The library syntax is B<gfrac>C<(x)>.

\subsecidx{imag}C<(x)>: imaginary part of C<x>. When
...
\subsecidx{length}C<(x)>: number of non-code words in C<x> really used
...
\subsecidx{lift}C<(x,{v})>: lifts an element C<x = a mod n> of B<Z>C</n>B<Z> to
...
C<-1>. Also available is \funs{lift}{x} = C<lift0(x,-1)>.

\subsecidx{norm}C<(x)>: algebraic norm of C<x>, i.e.S< >the product of C<x> with
...
\subsecidx{norml2}C<(x)>: square of the C<L^2>-norm of C<x>. More precisely,
if C<x> is a scalar, C<norml2(x)> is defined to be C<x * conj(x)>.
If C<x> is a (row or column) vector or a matrix, C<norml2(x)> is
defined recursively as F<sum>C<_i norml2(x_i)>, where C<(x_i)> run through
the components of C<x>. In particular, this yields the usual F<sum>C< |x_i|^2>
(resp.S< >F<sum>C< |x_{i,j}|^2>) if C<x> is a vector (resp.S< >matrix) with complex
components.
...
\subsecidx{numerator}C<(x)>: numerator of C<x>. The meaning of this
is clear when C<x> is a rational number or function. If C<x> is an integer
or a polynomial, it is treated as a rational number of function,
...
\subsecidx{numtoperm}C<(n,k)>: generates the C<k>-th permutation (as a
...
\subsecidx{padicprec}C<(x,p)>: absolute C<p>-adic precision of the object C<x>.
...
\subsecidx{permtonum}C<(x)>: given a permutation C<x> on C<n> elements,
...
\subsecidx{precision}C<(x,{n})>: gives the precision in decimal digits of the
...
\funs{ggprecision}{x} (result is a C<GEN>) and \funs{gprec}{x,n}, where
C<n> is a C<long>.

\subsecidx{random}C<({N = 2^{31}})>: returns a random integer between C<0> and
...
Note that C<random(2^31)> is I<not> equivalent to C<random()>,
...
\subsecidx{real}C<(x)>: real part of C<x>. In the case where C<x> is a quadratic
...
\subsecidx{round}C<(x,{&e})>: If C<x> is in B<R>, rounds C<x> to the nearest
...
\subsecidx{simplify}C<(x)>: this function simplifies C<x> as much as it can.
...
\subsecidx{sizebyte}C<(x)>: outputs the total number of bytes occupied by the
tree representing the PARI object C<x>.

X<taille2>The library syntax is B<taille2>C<(x)> which returns a C<long>; \funs{taille}{x} returns the
number of I<words> instead.

\subsecidx{sizedigit}C<(x)>: outputs a quick bound for the number of decimal
...
\subsecidx{truncate}C<(x,{&e})>: truncates C<x> and sets C<e> to the number of
...
\subsecidx{valuation}C<(x,p)>:X<Label se:valuation> computes the highest
...
\subsecidx{variable}C<(x)>: gives the main variable of the object C<x>, and
...
is in C<x[2]>, or call the function \funs{gvar}{x} which returns the variable
...
B<*> If the argument is an intmod or a C<p>-adic, at present only a
...
I<identical>. The same statement holds true for C<p>-adics raised to the
...
version , will have a different name if they are implemented some day.
...
\subsecidx{Euler}: Euler's constant F<gamma>C< = 0.57721...>. Note that
...
\subsecidx{I}: the complex number  F<sqrt> C<{-1}>.

The library syntax is the global variable C<gi> (of type C<GEN>).

\subsecidx{Pi}: the constant F<pi> (C<3.14159...>).X<Label se:pi>

X<mppi>The library syntax is B<mppi>C<(>I<prec>C<)> where I<prec> I<must> be given. Note that
this creates F<pi> on the PARI stack, but a copy is also created on the heap
for quicker computations next time the function is called.

\subsecidx{abs}C<(x)>: absolute value of C<x> (modulus if C<x> is complex).
Rational functions are not allowed. Contrary to most transcendental
functions, an exact argument is I<not> converted to a real number before
...
\subsecidx{acos}C<(x)>: principal branch of C<cos^{-1}(x)>,
i.e.S< >such that C<{Re(acos}(x))\in [0,>F<pi>C<]>. If
C<x\in >B<R> and C<|x| E<gt> 1>, then C<acos(x)> is complex.

X<gacos>The library syntax is B<gacos>C<(x,>I<prec>C<)>.

\subsecidx{acosh}C<(x)>: principal branch of C<cosh^{-1}(x)>,
i.e.S< >such that C<{Im(acosh}(x))\in [0,>F<pi>C<]>. If
C<x\in >B<R> and C<x E<lt> 1>, then C<acosh(x)> is complex.

X<gach>The library syntax is B<gach>C<(x,>I<prec>C<)>.

\subsecidx{agm}C<(x,y)>: arithmetic-geometric mean of C<x> and C<y>. In the
...
\subsecidx{arg}C<(x)>: argument of the complex number C<x>, such that
C<->F<pi>C< E<lt> arg(x) E<lt>= >F<pi>.

X<garg>The library syntax is B<garg>C<(x,>I<prec>C<)>.

\subsecidx{asin}C<(x)>: principal branch of C<sin^{-1}(x)>, i.e.S< >such
that C<{Re(asin}(x))\in [->F<pi>C</2,>F<pi>C</2]>. If C<x\in >B<R> and C<|x| E<gt> 1> then
C<asin(x)> is complex.

X<gasin>The library syntax is B<gasin>C<(x,>I<prec>C<)>.

\subsecidx{asinh}C<(x)>: principal branch of C<sinh^{-1}(x)>, i.e.S< >such
that C<{Im(asinh}(x))\in [->F<pi>C</2,>F<pi>C</2]>.

X<gash>The library syntax is B<gash>C<(x,>I<prec>C<)>.

\subsecidx{atan}C<(x)>: principal branch of C<tan^{-1}(x)>, i.e.S< >such
that C<{Re(atan}(x))\in ]->F<pi>C</2,>F<pi>C</2[>.

X<gatan>The library syntax is B<gatan>C<(x,>I<prec>C<)>.

\subsecidx{atanh}C<(x)>: principal branch of C<tanh^{-1}(x)>, i.e.S< >such
that C<{Im(atanh}(x))\in ]->F<pi>C</2,>F<pi>C</2]>. If C<x\in >B<R> and C<|x| E<gt> 1> then
C<atanh(x)> is complex.

X<gath>The library syntax is B<gath>C<(x,>I<prec>C<)>.

\subsecidx{bernfrac}C<(x)>: Bernoulli numberX<Bernoulli numbers> C<B_x>,
...
\subsecidx{bernreal}C<(x)>: Bernoulli numberX<Bernoulli numbers>
...
\subsecidx{bernvec}C<(x)>: creates a vector containing, as rational numbers,
...
\subsecidx{besselh1}C<(>I<nu>C<,x)>: C<H^1>-Bessel function of index I<nu>
and argument C<x>.

X<hbessel1>The library syntax is B<hbessel1>C<(>I<nu>C<,x,>I<prec>C<)>.

\subsecidx{besselh2}C<(>I<nu>C<,x)>: C<H^2>-Bessel function of index I<nu>
and argument C<x>.

X<hbessel2>The library syntax is B<hbessel2>C<(>I<nu>C<,x,>I<prec>C<)>.

\subsecidx{besseli}C<(>I<nu>C<,x)>: C<I>-Bessel function of index I<nu> and
argument C<x>. If C<x> converts to a power series, the initial factor
C<(x/2)^>F<nu>C</>F<Gamma>C<(>F<nu>C<+1)> is omitted (since it cannot be represented in PARI
when F<nu> is not integral).

X<ibessel>The library syntax is B<ibessel>C<(>I<nu>C<,x,>I<prec>C<)>.

\subsecidx{besselj}C<(>I<nu>C<,x)>: C<J>-Bessel function of index I<nu> and
argument C<x>. If C<x> converts to a power series, the initial factor
C<(x/2)^>F<nu>C</>F<Gamma>C<(>F<nu>C<+1)> is omitted (since it cannot be represented in PARI
when F<nu> is not integral).

X<jbessel>The library syntax is B<jbessel>C<(>I<nu>C<,x,>I<prec>C<)>.

\subsecidx{besseljh}C<(n,x)>: C<J>-Bessel function of half integral index.
...
present version , this function is not very accurate when C<x> is
small.

X<jbesselh>The library syntax is B<jbesselh>C<(n,x,>I<prec>C<)>.

\subsecidx{besselk}C<(>I<nu>C<,x,{>I<flag>C< = 0})>: C<K>-Bessel function of index
I<nu> (which can be complex) and argument C<x>. Only real and positive
arguments C<x> are allowed in the present version . If I<flag> is equal to
1, uses another implementation of this function which is faster when C<x E<gt>E<gt> 1>.

X<kbessel>The library syntax is B<kbessel>C<(>I<nu>C<,x,>I<prec>C<)> and
\funs{kbessel2}{I<nu>,x,I<prec>} respectively.

\subsecidx{besseln}C<(>I<nu>C<,x)>: C<N>-Bessel function of index I<nu>
and argument C<x>.

X<nbessel>The library syntax is B<nbessel>C<(>I<nu>C<,x,>I<prec>C<)>.

\subsecidx{cos}C<(x)>: cosine of C<x>.

X<gcos>The library syntax is B<gcos>C<(x,>I<prec>C<)>.

\subsecidx{cosh}C<(x)>: hyperbolic cosine of C<x>.

X<gch>The library syntax is B<gch>C<(x,>I<prec>C<)>.

\subsecidx{cotan}C<(x)>: cotangent of C<x>.

X<gcotan>The library syntax is B<gcotan>C<(x,>I<prec>C<)>.

\subsecidx{dilog}C<(x)>: principal branch of the dilogarithm of C<x>,
...
\subsecidx{eint1}C<(x,{n})>: exponential integral
F<int>C<_x^ oo (e^{-t})/(t)dt> (C<x\in>B<R>)
...
X<veceint1>The library syntax is B<veceint1>C<(x,n,prec)>. Also available is \funs{eint1}{x,prec}.

\subsecidx{erfc}C<(x)>: complementary error function
C<(2/> F<sqrt> F<pi>C<)>F<int>C<_x^ oo e^{-t^2}dt> (C<x\in>B<R>).

X<erfc>The library syntax is B<erfc>C<(x,>I<prec>C<)>.

\subsecidx{eta}C<(x,{>I<flag>C< = 0})>: X<Dedekind>Dedekind's F<eta> function, without the
...
\subsecidx{exp}C<(x)>: exponential of C<x>.
C<p>-adic arguments with positive valuation are accepted.

X<gexp>The library syntax is B<gexp>C<(x,>I<prec>C<)>.

\subsecidx{gammah}C<(x)>: gamma function evaluated at the argument C<x+1/2>.

X<ggamd>The library syntax is B<ggamd>C<(x,>I<prec>C<)>.

\subsecidx{gamma}C<(x)>: gamma function of C<x>.

X<ggamma>The library syntax is B<ggamma>C<(x,>I<prec>C<)>.

\subsecidx{hyperu}C<(a,b,x)>: C<U>-confluent hypergeometric function with
...
\subsecidx{incgam}C<(s,x,{y})>: incomplete gamma function. The argument C<x>
and C<s> are complex numbers (C<x> must be a positive real number if C<s = 0>).
The result returned is F<int>C<_x^ oo e^{-t}t^{s-1}dt>. When C<y> is given,
assume (of course without checking!) that C<y = >F<Gamma>C<(s)>. For small C<x>, this
will speed up the computation.

X<incgam>The library syntax is B<incgam>C<(s,x,>I<prec>C<)> and \funs{incgam0}{s,x,y,prec},
respectively (an omitted C<y> is coded as C<NULL>).

\subsecidx{incgamc}C<(s,x)>: complementary incomplete gamma function.
The arguments C<x> and C<s> are complex numbers such that C<s> is not a pole of
F<Gamma> and C<|x|/(|s|+1)> is not much larger than 1 (otherwise the
convergence is very slow). The result returned is F<int>C<_0^x
e^{-t}t^{s-1}dt>.

X<incgamc>The library syntax is B<incgamc>C<(s,x,>I<prec>C<)>.

\subsecidx{log}C<(x)>: principal branch of the natural logarithm of
C<x>, i.e.S< >such that C<{Im(log}(x))\in ]->F<pi>C<,>F<pi>C<]>. The result is complex
(with imaginary part equal to F<pi>) if C<x\in >B<R> and C<x E<lt> 0>. In general,
the algorithm uses the formula

S<  > F<log> C<(x)  ~  (>F<pi>C<)/(2agm(1, 4/s)) - m > F<log> C< 2, >
...
C<p>-adic arguments are also accepted for C<x>, with the convention that
 F<log> C<(p) = 0>. Hence in particular  F<exp> C<(> F<log> C<(x))/x> is not in general equal to
1 but to a C<(p-1)>-th root of unity (or C<F<+->1> if C<p = 2>) times a power of
C<p>.

X<glog>The library syntax is B<glog>C<(x,>I<prec>C<)>.

\subsecidx{lngamma}C<(x)>: principal branch of the logarithm of the gamma
...
\subsecidx{polylog}C<(m,x,{>I<flag>C< = 0})>: one of the different polylogarithms,
depending on I<flag>:

If I<flag>C< = 0> or is omitted: C<m-th> polylogarithm of C<x>, i.e.S< >analytic
continuation of the power series C<Li_m(x) = >F<sum>C<_{n E<gt>= 1}x^n/n^m>
...
C<|x|^2 E<lt>= 1/2>, and the power series expansion in  F<log> C<(x)> otherwise.

Using I<flag>, computes a modified C<m-th> polylogarithm of C<x>.
We use Zagier's notations; let  F<Re> C<_m> denotes  F<Re>  or  F<Im>  depending
whether C<m> is odd or even:

If I<flag>C< = 1>: compute C<~ D_m(x)>, defined for C<|x| E<lt>= 1> by

S<  > F<Re> C<_m(>F<sum>C<_{k = 0}^{m-1} ((-> F<log> C<|x|)^k)/(k!)Li_{m-k}(x)
+((-> F<log> C<|x|)^{m-1})/(m!)> F<log> C<|1-x|).>

If I<flag>C< = 2>: compute C<D_m(x)>, defined for C<|x| E<lt>= 1> by

S<  > F<Re> C<_m(>F<sum>C<_{k = 0}^{m-1}((-> F<log> C<|x|)^k)/(k!)Li_{m-k}(x)
-(1)/(2)((-> F<log> C<|x|)^m)/(m!)).>

If I<flag>C< = 3>: compute C<P_m(x)>, defined for C<|x| E<lt>= 1> by
...
\subsecidx{sqrt}C<(x)>: principal branch of the square root of C<x>,
i.e.S< >such that C<Arg(sqrt(x))\in ]->F<pi>C</2, >F<pi>C</2]>, or in other
words such that  F<Re> C<(sqrt(x)) E<gt> 0> or  F<Re> C<(sqrt(x)) = 0> and
 F<Im> C<(sqrt(x)) E<gt>= 0>. If C<x\in >B<R> and C<x E<lt> 0>, then the result is
...
\subsecidx{sqrtn}C<(x,n,{&z})>: principal branch of the C<n>th root of C<x>,
i.e.S< >such that C<Arg(sqrt(x))\in ]->F<pi>C</n, >F<pi>C</n]>. Intmod
...
X<weber0>The library syntax is B<weber0>C<(x,>I<flag>C<,prec)>. Associated to the various values of I<flag>, the
following functions are also available: \funs{werberf}{x,prec},
\funs{werberf1}{x,prec} or \funs{werberf2}{x,prec}.

\subsecidx{zeta}C<(s)>: For C<s> a complex number, Riemann's zeta
function X<Riemann zeta-function> F<zeta>C<(s) = >F<sum>C<_{n E<gt>= 1}n^{-s}>,
...
that interpolates the values of C<(1 - p^{-k}) >F<zeta>C<(k)> at negative
...
In the present version , all arithmetic functions in the narrow sense
...
\subsecidx{addprimes}C<({x = []})>: adds the integers contained in the
...
do I<not> use this extra table.

To remove primes from the list use C<removeprimes>.

X<addprimes>The library syntax is B<addprimes>C<(x)>.

\subsecidx{bestappr}C<(x,A,{B})>: if C<B> is omitted, finds the best rational
approximation to C<x\in>B<R> (or B<R>C<[X]>, or B<R>C<^n>,...) with denominator at
...
X<bestappr0>The library syntax is B<bestappr0>C<(x,A,B)>. Also available is \funs{bestappr}{x,A} corresponding
to an omitted C<B>.

\subsecidx{bezout}C<(x,y)>: finds C<u> and C<v> minimal in a
natural sense such that C<x*u+y*v = > F<gcd> C<(x,y)>. The arguments
must be both integers or both polynomials, and the result is a
row vector with three components C<u>, C<v>, and  F<gcd> C<(x,y)>.
X<extended gcd>

X<vecbezout>The library syntax is B<vecbezout>C<(x,y)> to get the vector, or \funs{gbezout}{x,y, &u, &v}
...
\subsecidx{bezoutres}C<(x,y)>: as C<bezout>, with the resultant of C<x> and
C<y> replacing the gcd. X<extended gcd> The algorithm uses
(subresultant) assumes the base ring is a domain.

X<vecbezoutres>The library syntax is B<vecbezoutres>C<(x,y)> to get the vector, or \funs{subresext}{x,y, &u, &v}
...
\subsecidx{bigomega}C<(x)>: number of prime divisors of C<|x|> counted with
multiplicity. C<x> must be an integer.

X<bigomega>The library syntax is B<bigomega>C<(x)>, the result is a C<long>.

\subsecidx{binomial}C<(x,y)>: X<binomial coefficient>binomial coefficient C<binom{x}{y}>.
...
\subsecidx{chinese}C<(x,{y})>: if C<x> and C<y> are both intmods or both
...
\subsecidx{content}C<(x)>: computes the gcd of all the coefficients of C<x>,
when this gcd makes sense. This is the natural definition
if C<x> is a polynomial (and by extension a power series) or a
vector/matrix. This is in general a weaker notion than the I<ideal>
...
matrix or vector I<coefficient> C<x> appears, the gcd is taken 
...
\subsecidx{contfrac}C<(x,{b},{nmax})>: creates the row vector whose
...
\funs{gboundcf}{x,nmax}, \funs{gcf}{x}, or \funs{gcf2}{b,x}, where C<nmax>
is a C integer.

\subsecidx{contfracpnqn}C<(x)>: when C<x> is a vector or a one-row matrix, C<x>
...
\subsecidx{core}C<(n,{>I<flag>C< = 0})>: if C<n> is a non-zero integer written as
...
\subsecidx{coredisc}C<(n,{>I<flag>C<})>: if C<n> is a non-zero integer written as
...
\subsecidx{dirdiv}C<(x,y)>: C<x> and C<y> being vectors of perhaps different
...
\subsecidx{direuler}C<(p = a,b,>I<expr>C<,{c})>: computes the
...
than C<p> (say C<X>) and I<expr>C<(X)> is understood as
the local factor I<expr>C<(p^{-s})>.
...
the X<sigma>B<sigma> function, associated to F<zeta>C<(s)>F<zeta>C<(s-1)>:
...
\subsecidx{dirmul}C<(x,y)>: C<x> and C<y> being vectors of perhaps different
...
\subsecidx{divisors}C<(x)>: creates a row vector whose components are the
...
\subsecidx{eulerphi}C<(x)>: Euler's F<phi>
...
\subsecidx{factor}C<(x,{>I<lim>C< = -1})>: general factorization function.
...
(except when I<lim>C< = 0> where the effect is identical to setting
I<lim>C< = primelimit>). In this case, the remaining part may actually
...
number fields. Over B<Q>, X<van Hoeij>van Hoeij's method is used, which is able to
...
\subsecidx{factorback}C<(f,{e},{nf})>: gives back the factored object
...
X<factorback0>The library syntax is B<factorback0>C<(f,e,>I<nf>C<)>, where an omitted
I<nf> or C<e> is entered as C<NULL>. Also available is
X<factorback>C<factorback>C<(f,>I<nf>C<)> (case C<e = NULL>) where an omitted
I<nf> is entered as C<NULL>.

\subsecidx{factorcantor}C<(x,p)>: factors the polynomial C<x> modulo the
...
\subsecidx{factorff}C<(x,p,a)>: factors the polynomial C<x> in the field
...
B<F>C<_p>, a much faster algorithm is applied, using the computation of
isomorphisms between finite fields.

X<factorff>The library syntax is B<factorff>C<(x,p,a)>.

\subsecidx{factorial}C<(x)> or C<x!>: factorial of C<x>. The expression C<x!>
...
\funs{mpfactr}{x,prec} for C<factorial(x)>. C<x> must be a C<long>
integer and not a PARI integer.

\subsecidx{factorint}C<(n,{>I<flag>C< = 0})>: factors the integer C<n> into a product of
...
\subsecidx{factormod}C<(x,p,{>I<flag>C< = 0})>: factors the polynomial C<x> modulo
...
\subsecidx{fibonacci}C<(x)>: C<x-th> Fibonacci number.

X<fibo>The library syntax is B<fibo>C<(x)>. C<x> must be a C<long>.

\subsecidx{ffinit}C<(p,n,{v = x})>: computes a monic polynomial of degree
...
\subsecidx{gcd}C<(x,{y})>: creates the greatest common divisor of C<x>
...
B<*> integers: use modified right-shift binary (``plus-minus''
variant).

B<*> univariate polynomials with coeffients in the same number
field (in particular rational): use modular gcd algorithm.

B<*> general polynomials: use the X<subresultant algorithm>subresultant algorithm if
coefficient explosion is likely (exact, non modular, coefficients).

X<ggcd>The library syntax is B<ggcd>C<(x,y)>. For general polynomial inputs, \funs{srgcd}{x,y} is also
available. For univariate I<rational> polynomials, one also has
\funs{modulargcd}{x,y}.

\subsecidx{hilbert}C<(x,y,{p})>: X<Hilbert symbol>Hilbert symbol of C<x> and C<y> modulo
...
\subsecidx{isfundamental}C<(x)>: true (1) if C<x> is equal to 1 or to the
discriminant of a quadratic field, false (0) otherwise.

X<gisfundamental>The library syntax is B<gisfundamental>C<(x)>, but the simpler function \funs{isfundamental}{x}
which returns a C<long> should be used if C<x> is known to be of type
integer.

\subsecidx{ispower}C<(x,{k}, {&n})>:
...
\subsecidx{isprime}C<(x,{>I<flag>C< = 0})>: true (1) if C<x> is a (proven) prime
...
If I<flag>C< = 0>, use a combination of Baillie-PSW pseudo primality test (see
...
If I<flag>C< = 1>, use Selfridge-Pocklington-Lehmer ``C<p-1>'' test and output a
...
If I<flag>C< = 2>, use APRCL.

X<gisprime>The library syntax is B<gisprime>C<(x,>I<flag>C<)>, but the simpler function \funs{isprime}{x}
which returns a C<long> should be used if C<x> is known to be of
type integer.

\subsecidx{ispseudoprime}C<(x,{>I<flag>C<})>: true (1) if C<x> is a strong pseudo
...
If I<flag>C< = 0>, checks whether C<x> is a Baillie-Pomerance-Selfridge-Wagstaff
...
If I<flag>C< E<gt> 0>, checks whether C<x> is a strong Miller-Rabin pseudo prime  for
I<flag> randomly chosen bases (with end-matching to catch square roots of
C<-1>).

X<gispseudoprime>The library syntax is B<gispseudoprime>C<(x,>I<flag>C<)>, but the simpler function \funs{ispseudoprime}{x}
which returns a C<long> should be used if C<x> is known to be of type
integer.

\subsecidx{issquare}C<(x,{&n})>: true (1) if C<x> is a square, false (0)
...
numbers of the form C<s^2> with C<s> in B<Z>, B<Q> and B<Z>C</N>B<Z> respectively.
...
This will I<not> work for C<t_INTMOD> (use quadratic reciprocity) or
C<t_SER> (only check the leading coefficient).

X<gissquarerem>The library syntax is B<gissquarerem>C<(x,&n)>. Also available is \funs{gissquare}{x}.

\subsecidx{issquarefree}C<(x)>: true (1) if C<x> is squarefree, false (0) if not.
Here C<x> can be an integer or a polynomial.

X<gissquarefree>The library syntax is B<gissquarefree>C<(x)>, but the simpler function \funs{issquarefree}{x}
which returns a C<long> should be used if C<x> is known to be of type
...
\subsecidx{kronecker}C<(x,y)>:
X<Kronecker symbol>Kronecker symbol C<(x|y)>, where C<x> and C<y> must be of type integer. By
definition, this is the extension of X<Legendre symbol>Legendre symbol to B<Z>C<  x >B<Z>
by total multiplicativity in both arguments with the following special rules
for C<y = 0, -1> or C<2>:

B<*> C<(x|0) = 1> if C<|x |= 1> and C<0> otherwise.

B<*> C<(x|-1) = 1> if C<x E<gt>= 0> and C<-1> otherwise.

B<*> C<(x|2) = 0> if C<x> is even and C<1> if C<x = 1,-1 mod 8> and C<-1>
if C<x = 3,-3 mod 8>.

X<kronecker>The library syntax is B<kronecker>C<(x,y)>, the result (C<0> or C<F<+-> 1>) is a C<long>.

\subsecidx{lcm}C<(x,{y})>: least common multiple of C<x> and C<y>, i.e.S< >such
that C<lcm(x,y)*> F<gcd> C<(x,y) = abs(x*y)>. If C<y> is omitted and C<x>
...
\subsecidx{moebius}C<(x)>: X<Moebius>Moebius F<mu>-function of C<|x|>. C<x> must
be of type integer.

X<mu>The library syntax is B<mu>C<(x)>, the result (C<0> or C<F<+-> 1>) is a C<long>.

\subsecidx{nextprime}C<(x)>: finds the smallest pseudoprime (see
...
\subsecidx{numdiv}C<(x)>: number of divisors of C<|x|>. C<x> must be of type
integer.

X<numbdiv>The library syntax is B<numbdiv>C<(x)>.

\subsecidx{numbpart}C<(n)>: gives the number of unrestricted partitions of
...
\subsecidx{omega}C<(x)>: number of distinct prime divisors of C<|x|>. C<x>
must be of type integer.

X<omega>The library syntax is B<omega>C<(x)>, the result is a C<long>.

\subsecidx{precprime}C<(x)>: finds the largest pseudoprime (see
...
\subsecidx{qfbclassno}C<(D,{>I<flag>C< = 0})>: ordinary class number of the quadratic
order of discriminant C<D>. In the present version , a C<O(D^{1/2})>
...
either the version with I<flag>C< = 1> or the function C<quadclassunit>.
...
which is equal to the I<narrow> class number. The two notions are the same
when C<D E<lt> 0> or the fundamental unit F<varepsilon> has negative norm; when C<D
E<gt> 0> and C<N>F<varepsilon>C< E<gt> 0>, the number of classes of forms is twice the
...
X<qfbclassno0>The library syntax is B<qfbclassno0>C<(D,>I<flag>C<)>. Also available:
...
\subsecidx{qfbcompraw}C<(x,y)> X<composition>composition of the binary quadratic forms
...
\subsecidx{qfbhclassno}C<(x)>: X<Hurwitz class number>Hurwitz class number of C<x>, where
...
\subsecidx{qfbnucomp}C<(x,y,l)>: X<composition>composition of the primitive positive
...
The current implementation is straightforward and in general I<slower>
than the generic routine (since the latter take advantadge of asymptotically
fast operations and careful optimizations).

X<nucomp>The library syntax is B<nucomp>C<(x,y,l)>. The auxiliary function \funs{nudupl}{x,l} can be
used when C<x = y>.

\subsecidx{qfbnupow}C<(x,n)>: C<n>-th power of the primitive positive definite
...
\subsecidx{qfbpowraw}C<(x,n)>: C<n>-th power of the binary quadratic form
...
\subsecidx{qfbprimeform}C<(x,p)>: prime binary quadratic form of discriminant
...
\subsecidx{qfbred}C<(x,{>I<flag>C< = 0},{D},{>I<isqrtD>C<},{>I<sqrtD>C<})>:
...
discriminant, C<floor{> F<sqrt> C<{D}}>, and  F<sqrt> C<{D}> respectively
...
\funs{redimag}{x} ( = \funs{qfbred}{x} where C<x> is definite),
...
\subsecidx{qfbsolve}C<(Q,p)>: Solve the equation C<Q(x,y) = p> over the integers,
...
Let C<D = > F<disc> C< Q>. The algorithm used runs in probabilistic polynomial time
...
\subsecidx{quadclassunit}C<(D,{>I<flag>C< = 0},{>I<tech>C< = []})>:
...
when C<D E<lt> -10^{25}>, C<D E<gt> 10^{10}>, or when the I<structure> is wanted. It
...
, this does not work at all: use the general function X<bnfnarrow>C<bnfnarrow>.
...
I<assume> that prime ideals of norm less than C<c_2 (> F<log> C< |D|)^2> generate
the class group, but the bulk of the work is done with prime ideals of norm
less than C<c_1 (> F<log> C< |D|)^2>. A larger C<c_1> means that relations are easier
to find, but more relations are needed and the linear algebra will be harder.
The default is C<c_1 = c_2 = 0.2>, so the result is I<not> rigorously
proven.

The result is a vector C<v> with 3 components if C<D E<lt> 0>, and
C<4> otherwise. The correspond respectively to

B<*> C<v[1]>: the class number

B<*> C<v[2]>: a vector giving the structure of the class group as a
product of cyclic groups;

B<*> C<v[3]>: a vector giving generators of those cyclic groups (as
binary quadratic forms).

B<*> C<v[4]>: (omitted if C<D E<lt> 0>) the regulator, computed to an
...
\funs{buchimag}{D,c_1,c_2} and \funs{buchreal}{D,I<flag>,c_1,c_2}.

\subsecidx{quaddisc}C<(x)>: discriminant of the quadratic field
B<Q>C<(> F<sqrt> C<{x})>, where C<x\in>B<Q>.

X<quaddisc>The library syntax is B<quaddisc>C<(x)>.

\subsecidx{quadhilbert}C<(D,{pq})>: relative equation defining the
...
efficiency,  F<gcd> C<(24,(p-1)(q-1))> should be as large as possible.
...
X<quadhilbert>The library syntax is B<quadhilbert>C<(D,pq,>I<prec>C<)>.

\subsecidx{quadgen}C<(D)>: X<Label se:quadgen>creates the quadratic
numberX<omega> F<omega>C< = (a+> F<sqrt> C<{D})/2> where C<a = 0> if C<x = 0 mod 4>,
C<a = 1> if C<D = 1 mod 4>, so that C<(1,>F<omega>C<)> is an integral basis for the
...
\subsecidx{quadpoly}C<(D,{v = x})>: creates the ``canonical'' quadratic
...
\subsecidx{quadray}C<(D,f,{>I<lambda>C<})>: relative equation for the ray
...
For C<D E<lt> 0>, uses the F<sigma> function. If supplied, I<lambda> is is the
technical element F<lambda> of C<bnf> necessary for Schertz's method. In
that case, returns 0 if F<lambda> is not suitable.
...
\subsecidx{quadregulator}C<(x)>: regulator of the quadratic field of
...
\subsecidx{quadunit}C<(D)>: fundamental unitX<fundamental units> of the
real quadratic field B<Q>C<(> F<sqrt> C< D)> where  C<D> is the positive discriminant
...
\subsecidx{removeprimes}C<({x = []})>: removes the primes listed in C<x> from
...
\subsecidx{sigma}C<(x,{k = 1})>: sum of the C<k-th> powers of the
positive divisors of C<|x|>. C<x> and C<k> must be of type integer.

X<sumdiv>The library syntax is B<sumdiv>C<(x)> ( = \funs{sigma}{x}) or \funs{gsumdivk}{x,k} ( = 
\funs{sigma}{x,k}), where C<k> is a C long integer.

\subsecidx{sqrtint}C<(x)>: integer square root of C<x>, which must be a
...
\subsecidx{zncoppersmith}C<(P, N, X, {B = N})>: finds all integers C<x_0> with
C<|x_0| E<lt>= X> such that

S<  > F<gcd> C<(N, P(x_0)) E<gt>= B.>

If C<N> is prime or a prime power, X<polrootsmod>C<polrootsmod> or X<polrootspadic>C<polrootspadic>
will be much faster. C<X> must be smaller than  F<exp> C<(> F<log> C<^2 B / (> F<deg> C<(P) > F<log> C<
N))>.

X<zncoppersmith>The library syntax is B<zncoppersmith>C<(P, N, X, B)>, where an omitted C<B> is coded as C<NULL>.

\subsecidx{znlog}C<(x,g)>: C<g> must be a primitive root mod a prime C<p>, and
the result is the discrete log of C<x> in the multiplicative group
C<(>B<Z>C</p>B<Z>C<)^*>. This function uses a simple-minded combination of
Pohlig-Hellman algorithm and Shanks baby-step/giant-step which requires
C<O(> F<sqrt> C<{q})> storage, where C<q> is the largest prime factor of C<p-1>. Hence
it cannot be used when the largest prime divisor of C<p-1> is greater than
about C<10^{13}>.

X<znlog>The library syntax is B<znlog>C<(x,g)>.

\subsecidx{znorder}C<(x,{>I<o>C<})>: C<x> must be an integer mod C<n>, and the
result is the order of C<x> in the multiplicative group C<(>B<Z>C</n>B<Z>C<)^*>. Returns
...
\subsecidx{znprimroot}C<(n)>: returns a primitive root (generator) of
C<(>B<Z>C</n>B<Z>C<)^*>, whenever this latter group is cyclic (C<n = 4> or C<n = 2p^k> or
C<n = p^k>, where C<p> is an odd prime and C<k E<gt>= 0>).

X<gener>The library syntax is B<gener>C<(x)>.

\subsecidx{znstar}C<(n)>: gives the structure of the multiplicative group
...
for curves over B<Q>. Conversely, C<tate> and C<w> are only available
for curves defined over B<Q>C<_p>. The use of member functions is best described
by an example:
...
the function C<ellminimalmodel>. I<Using a non-minimal model in such a
...
requirements are given as the I<minimal> ones: any richer structure may
...
\subsecidx{elladd}C<(E,z1,z2)>: sum of the points C<z1> and C<z2> on the
elliptic curve corresponding to C<E>.

X<addell>The library syntax is B<addell>C<(E,z1,z2)>.

\subsecidx{ellak}C<(E,n)>: computes the coefficient C<a_n> of the
...
\subsecidx{ellan}C<(E,n)>: computes the vector of the first C<n> C<a_k>
...
\subsecidx{ellap}C<(E,p,{>I<flag>C< = 0})>: computes the C<a_p> corresponding to the
...
output by C<ellinit>, defined over B<Q>, B<F>C<_p> or B<Q>C<_p>. C<E> must be
given by a Weierstrass equation minimal at C<p>.

X<ellap0>The library syntax is B<ellap0>C<(E,p,>I<flag>C<)>. Also available are \funs{apell}{E,p}, corresponding
to I<flag>C< = 0>, and \funs{apell2}{E,p} (I<flag>C< = 1>).

\subsecidx{ellbil}C<(E,z1,z2)>: if C<z1> and C<z2> are points on the elliptic
...
\subsecidx{ellchangecurve}C<(E,v)>: changes the data for the elliptic curve C<E>
...
\subsecidx{ellchangepoint}C<(x,v)>: changes the coordinates of the point or
...
\subsecidx{ellconvertname}C<(>I<name>C<)>: 
converts an elliptic curve name, as found in the X<elldata>C<elldata> database,
from a string to a triplet C<[>I<conductor>C<, >I<isogeny class>C<,
>I<index>C<]>. It will also convert a triplet back to a curve name.
Examples:
...
X<ellconvertname>The library syntax is B<ellconvertname>C<(>I<name>C<)>.

\subsecidx{elleisnum}C<(E,k,{>I<flag>C< = 0})>: C<E> being an elliptic curve as
output by C<ellinit> (or, alternatively, given by a 2-component vector
C<[>F<omega>C<_1,>F<omega>C<_2]> representing its periods), and C<k> being an even
positive integer, computes the numerical value of the Eisenstein series of
weight C<k> at C<E>, namely

S<  >C<
(2i >F<pi>C</>F<omega>C<_2)^k
(1 + 2/>F<zeta>C<(1-k) >F<sum>C<_{n E<gt>= 0} n^{k-1}q^n / (1-q^n)),
>

where C<q = e(>F<omega>C<_1/>F<omega>C<_2)>.
...
\subsecidx{elleta}C<(om)>: returns the two-component row vector
...
\subsecidx{ellgenerators}C<(E)>: returns a B<Z>-basis of the free part of the
...
is only available for curves over B<Z> of small conductors.

X<ellgenerators>The library syntax is B<ellgenerators>C<(E)>.

\subsecidx{ellglobalred}C<(E)>: calculates the arithmetic conductor, the global
minimal model of C<E> and the global X<Tamagawa number>Tamagawa number C<c>. 
C<E> must be an I<sell> as output by C<ellinit>, I<and is supposed
to have all its coefficients >C<a_i>I< in> B<Q>. The result is a 3 component
vector C<[N,v,c]>. C<N> is the arithmetic conductor of the curve. C<v> gives the
coordinate change for C<E> over B<Q> to the minimal integral model (see
...
\subsecidx{ellheight}C<(E,z,{>I<flag>C< = 2})>: global X<NE<eacute>ron-Tate height>NE<eacute>ron-Tate height of
the point C<z> on the elliptic curve C<E> (defined over B<Q>), given by a
...
local height. If I<flag>C< = 0>, this computation is done using sigma and
theta-functions and a trick due to J. Silverman. If I<flag>C< = 1>, use Tate's C<4^n>
algorithm. If I<flag>C< = 2>, use Mestre's AGM algorithm. The latter is much faster
than the other two, both in theory (converges quadratically) and in practice.

X<ellheight0>The library syntax is B<ellheight0>C<(E,z,>I<flag>C<,>I<prec>C<)>. Also available are
\funs{ghell}{E,z,I<prec>} (I<flag>C< = 0>) and \funs{ghell2}{E,z,I<prec>}
(I<flag>C< = 1>).

\subsecidx{ellheightmatrix}C<(E,x)>: C<x> being a vector of points, this
...
\subsecidx{ellidentify}C<(E)>: look up the elliptic curve C<E> (over B<Z>)
...
model, C<G> a B<Z>-basis of the free part of the X<Mordell-Weil group>Mordell-Weil group
of C<E> and C<C> the coordinates change (see C<ellchangecurve>).

X<ellidentify>The library syntax is B<ellidentify>C<(E)>.

\subsecidx{ellinit}C<(E,{>I<flag>C< = 0})>: initialize an X<ell>C<ell> structure,
...
being, only curves over a prime field B<F>C<_p> and over the C<p>-adic or
...
a shorted X<sell>I<sell> if I<flag>C< = 1>. Both contain the following information in
their components:
...
B<*> When C<E> is defined over B<R>, C<E.roots> is a vector whose
...
Then F<omega>C<_1 = >C<E.omega[1]> is the real period of C<E> (integral of
C<dx/(2y+a_1x+a_3)> over the connected component of the identity element of
the real points of the curve), and F<omega>C<_2 = >C<E.omega[2]> is a
...
C<E.eta> is a row vector containing the corresponding values F<eta>C<_1>
and F<eta>C<_2> such that F<eta>C<_1>F<omega>C<_2->F<eta>C<_2>F<omega>C<_1 = i>F<pi>.
...
notations of Tate. If the C<u>-component does not belong to B<Q>C<_p>, it is set
to zero.
...
\funs{initell}{E,I<prec>} (I<flag>C< = 0>) and
\funs{smallinitell}{E,I<prec>} (I<flag>C< = 1>).

\subsecidx{ellisoncurve}C<(E,z)>: gives 1 (i.e.S< >true) if the point C<z> is on
...
X<ellisoncurve>The library syntax is B<ellisoncurve>C<(E,z)>. Also available is \funs{oncurve}{E,z}
which returns a C<long> but does not accept vector of points.

\subsecidx{ellj}C<(x)>: elliptic C<j>-invariant. C<x> must be a complex number
...
\subsecidx{elllocalred}C<(E,p)>: calculates the X<Kodaira>Kodaira type of the
...
all its coefficients C<a_i> in B<Z>. The result is a 4-component vector
...
\subsecidx{elllseries}C<(E,s,{A = 1})>: C<E> being an I<sell> as output by
C<ellinit>, this computes the value of the L-series of C<E> at C<s>. It is
assumed that C<E> is defined over B<Q>, not necessarily minimal. The optional
...
X<elllseries>The library syntax is B<elllseries>C<(E,s,A,>I<prec>C<)> where I<prec> is a C<long> and an
omitted C<A> is coded as C<NULL>.

\subsecidx{ellminimalmodel}C<(E,{&v})>:  return the standard minimal
...
\subsecidx{ellorder}C<(E,z)>: gives the order of the point C<z> on the elliptic
curve C<E> if it is a torsion point, zero otherwise. In the present version
, this is implemented only for elliptic curves defined over B<Q>.

X<orderell>The library syntax is B<orderell>C<(E,z)>.

\subsecidx{ellordinate}C<(E,x)>: gives a 0, 1 or 2-component vector containing
the C<y>-coordinates of the points of the curve C<E> having C<x> as
C<x>-coordinate.

X<ordell>The library syntax is B<ordell>C<(E,x)>.

\subsecidx{ellpointtoz}C<(E,z)>: if C<E> is an elliptic curve with coefficients
...
real and complex periods of C<E>, C<t> is such that C<0 E<lt>= > F<Re> C<(t) E<lt> w1>
and C<0 E<lt>= > F<Im> C<(t) E<lt> > F<Im> C<(w2)>.
...
\subsecidx{ellpow}C<(E,z,n)>: computes C<n> times the point C<z> for the
...
\subsecidx{ellrootno}C<(E,{p = 1})>: C<E> being an I<sell> as output by
...
have coefficients in B<Q> but need I<not> be minimal.

X<ellrootno>The library syntax is B<ellrootno>C<(E,p)> and the result (equal to C<F<+->1>) is a C<long>.

\subsecidx{ellsigma}C<(E,z,{>I<flag>C< = 0})>: value of the Weierstrass F<sigma>
...
\subsecidx{ellsearch}C<(N)>: if C<N> is an integer, it is taken as a conductor
...
equation of the curve and C<G> is a B<Z>-basis of the free part of the
...
X<ellsearch>The library syntax is B<ellsearch>C<(N)>. Also available is \funs{ellsearchcurve}{N} that only
accept complete curve names.

\subsecidx{ellsub}C<(E,z1,z2)>: difference of the points C<z1> and C<z2> on the
elliptic curve corresponding to C<E>.

X<subell>The library syntax is B<subell>C<(E,z1,z2)>.

\subsecidx{elltaniyama}C<(E)>: computes the modular parametrization of the
...
is equal to C<f(z)dz>, a differential form on C<H/>F<Gamma>C<_0(N)> where C<N> is the
conductor of the curve. The variable used in the power series for C<u> and C<v>
is C<x>, which is implicitly understood to be equal to  F<exp> C<(2i>F<pi>C< z)>. It is
assumed that the curve is a I<strong> X<Weil curve>Weil curve, and that the
...
\subsecidx{elltors}C<(E,{>I<flag>C< = 0})>: if C<E> is an elliptic curve I<defined
...
If I<flag>C< = 0>, use Doud's algorithm: bound torsion by computing C<#E(>B<F>C<_p)>
...
If I<flag>C< = 1>, use Lutz-Nagell (I<much> slower), C<E> is allowed to be an
I<sell>.

X<elltors0>The library syntax is B<elltors0>C<(E,flag)>.

\subsecidx{ellwp}C<(E,{z = x},{>I<flag>C< = 0})>:
...
I<even> power in the expansion is the default serieslength in C<gp>, and the
...
\funs{weipell}{E,I<precdl>} for the power series.

\subsecidx{ellzeta}C<(E,z)>: value of the Weierstrass F<zeta> function of the
...
\subsecidx{ellztopoint}C<(E,z)>: C<E> being an I<ell> as output by
...
form, C<[x,y]> represents the X<Weierstrass F<wp>-function>Weierstrass F<wp>-function and its
derivative. If C<z> is in the lattice defining C<E> over B<C>, the result is
...
Let C<K = >B<Q>C<[X] / (T)> a number field, B<Z>C<_K> its ring of integers, C<T\in>B<Z>C<[X]>
is monic. Three basic number field structures can be associated to C<K> in
GP:

B<*> C<X<nf>>I<nf> denotes a number field, i.e.S< >a data structure output by
...
B<*> C<X<bnf>>I<bnf> denotes a ``Buchmann's number field'', i.e.S< >a
data structure output by X<bnfinit>C<bnfinit>. This contains
I<nf> and the deeper invariants of the field: units C<U(K)>, class group
 F<Cl> C<(K)>, as well as technical data required to solve the two associated
discrete logarithm problems.

B<*> C<X<bnr>>I<bnr> denotes a ``ray number field'', i.e.S< >a data structure
...
C<f>, the ray class group  F<Cl> C<_f(K)> and data associated to
the discrete logarithm problem therein.

=head2 Algebraic numbers and ideals

An X<algebraic number>I<algebraic number> belonging to C<K = >B<Q>C<[X]/(T)> is given as

B<*> a C<t_INT>, C<t_FRAC> or C<t_POL> (implicitly modulo C<T>), or

B<*> a C<t_POLMOD> (modulo C<T>), or

B<*> a C<t_COL>S< >C<v> of dimension C<N = [K:>B<Q>C<]>, representing
...
B<*> an algebraic number in one of the above forms, defining a principal ideal.

B<*> a prime ideal, i.e.S< >a 5-component vector in the format output by
C<idealprimedec>.

B<*> a C<t_MAT>, square and in Hermite Normal Form (or at least
...
If strictly less than C<N = [K:>B<Q>C<]> generators are given, it is assumed they
form a B<Z>C<_K>-basis. If C<N> or more are given, a B<Z>-basis is assumed. If
...
B<*> an X<idele>I<idele> is a 2-component vector, the first being an ideal as
...
C<G = \oplus_{i E<lt>= n} (>B<Z>C</d_i>B<Z>C<) g_i>, with C<d_n | ... | d_2 | d_1>
and F<prod>C< d_i = h>. This information can also be retrieved as
C<G.no>, C<G.cyc> and C<G.gen>.

B<*> a X<character>I<character> on the abelian group
C<\oplus (>B<Z>C</d_i>B<Z>C<) g_i>
is given by a row vector F<chi>C< = [a_1,...,a_n]> such that
F<chi>C<(>F<prod>C< g_i^{n_i}) = > F<exp> C<(2i>F<pi>F<sum>C< a_i n_i / d_i)>.

B<*> given such a structure, a X<subgroup>I<subgroup> C<H> is input as a square
...
When defining a relative extension, the base field I<nf> must be defined
...
B<*> C<X<rnf>>I<rnf> denotes a relative number field, i.e.S< >a data structure
output by C<rnfinit>.

B<*> A I<relative matrix> is a matrix whose entries are
elements of a (fixed) number field I<nf>, always expressed as column
vectors on the integral basis I<nf>C<.zk>. Hence it is a matrix of
vectors.

B<*> An X<ideal list>I<ideal list> is a row vector of (fractional)
ideals of the number field I<nf>.

B<*> A X<pseudo-matrix>I<pseudo-matrix> is a pair C<(A,I)> where C<A> is a
...
B<*> The X<projective module>I<projective module> generated by a pseudo-matrix C<(A,I)> is
the sum F<sum>C<_i {>B<a>C<}_j A_j> where the C<{>B<a>C<}_j> are the ideals of C<I>
...
needed to compute in C<(>B<Z>C<_K/I)^*>, where C<I> is a modulus in the above sense.
...
defining set C<[a1,a2,a3]> can have any of the following forms: C<[>I<bnr>C<]>,
C<[>I<bnr>C<,>I<subgroup>C<]>, C<[>I<bnf>C<,>I<module>C<]>,
C<[>I<bnf>C<,>I<module>C<,>I<subgroup>C<]>.

B<*> I<bnf> is as output by C<bnfinit>, where units are mandatory
unless the modulus is trivial; I<bnr> is as output by C<bnrinit>. This
is the ground field C<K>.

B<*> I<module> is a modulus B<f>, as described above.

B<*> I<subgroup> a subgroup of the ray class group modulo B<f> of
C<K>. As described above, this is input as a square matrix expressing
generators of a subgroup of the ray class group I<bnr>C<.clgp> on the
given generators.

The corresponding I<bnr> is the subfield of the ray class field of C<K>
modulo B<f>, fixed by the given subgroup.
...
use a I<bnf> instead, which contains much more information. On the other
hand, if the function requires a C<bnf>, it will I<not> launch
...
you could get directly with a simple I<bnr>C<.zk>.
...
S<  >I<tech>C< = [c, c2, >I<nrpid>C< ],>

where the parameters are to be understood as follows:

C<P> is the defining polynomial for the number field, which must be in
B<Z>C<[X]>, irreducible and monic. In fact, if you supply a non-monic polynomial
at this point, C<gp> issues a warning, then I<transforms your
...
\subsecidx{bnfclassunit}C<(P,{>I<flag>C< = 0},{>I<tech>C< = []})>: I<this function
...
I<not> a I<bnf>, you need C<bnfinit> for that. First we describe
the default behaviour (I<flag>C< = 0>):
...
\subsecidx{bnfclgp}C<(P,{>I<tech>C< = []})>: as C<bnfinit>, but only
outputs C<bnf.clgp>, i.e.S< >the class group.

X<classgrouponly>The library syntax is B<classgrouponly>C<(P,>I<tech>C<,>I<prec>C<)>, where I<tech>
is as described under C<bnfinit>.

\subsecidx{bnfdecodemodule}C<(>I<nf>C<,m)>: if C<m> is a module as output in the
...
\subsecidx{bnfinit}C<(P,{>I<flag>C< = 0},{>I<tech>C< = []})>: initializes a
...
the units by default (I<flag>C< = 0>).

When I<flag>C< = 1>, we insist on finding the fundamental units exactly. Be
...
When I<flag>C< = 2>, on the contrary, it is initially agreed that units are not
...
When I<flag>C< = 3>, computes a very small version of C<bnfinit>, a ``small
Buchmann's number field'' (or I<sbnf> for short) which contains enough
information to recover the full I<bnf> vector very rapidly, but which is
much smaller and hence easy to store and print. It is supposed to be used in
conjunction with C<bnfmake>.

I<tech> is a technical vector (empty by default, see X<se:GRHbnf>se:GRHbnf).
...
the casual user. In fact: I<never access a component directly, always use
...
explained in the book by H. Cohen, I<A Course in Computational Algebraic
...
C<(>F<wp>C<_i)_{1 E<lt>= i E<lt>= r}>.

I<bnf>C<[2]> contains the matrix C<B>, i.e.S< >the matrix containing the
expressions of the prime ideal factorbase in terms of the F<wp>C<_i>. It is an
C<r x c> matrix.
...
I<bnf>C<[6]> used to contain a permutation of the prime factor base, but
...
I<bnf>C<[8]> is a vector containing the classgroup I<bnf>C<.clgp>
as a finite abelian group, the regulator I<bnf>C<.reg>, a C<1> (used to
contain an obsolete ``check number''), the number of roots of unity and a
generator I<bnf>C<.tu>, the fundamental units I<bnf>C<.fu>.
...
I<bnf>C<[10]> is by default unused and set equal to 0. This field is used
...
An I<sbnf> is a 12 component vector C<v>, as follows. Let I<bnf> be
...
C<nfinit> giving a B<Z>-basis of the different, C<v[7]> is the matrix
C<W = >I<bnf>C<[1]>, C<v[8]> is the matrix C<matalpha = >I<bnf>C<[2]>,
C<v[9]> is the prime ideal factor base I<bnf>C<[5]> coded in a compact way,
and ordered according to the permutation I<bnf>C<[6]>, C<v[10]> is the
...
the roots in C<v[5]>. Note also that member functions will I<not> work on
...
\subsecidx{bnfisintnorm}C<(>I<bnf>C<,x)>: computes a complete system of
...
\subsecidx{bnfisnorm}C<(>I<bnf>C<,x,{>I<flag>C< = 1})>: tries to tell whether the
...
this case, C<x> is a norm iff C<b = 1>). If I<flag> is non zero the program adds to
C<S> the following prime ideals, depending on the sign of I<flag>. If I<flag>C< E<gt> 0>,
the ideals of norm less than I<flag>. And if I<flag>C< E<lt> 0> the ideals dividing I<flag>.

Assuming X<GRH>GRH, the answer is guaranteed (i.e.S< >C<x> is a norm iff C<b = 1>),
if C<S> contains all primes less than C<12> F<log> C<(> F<disc> C<(>I<Bnf>C<))^2>, where
I<Bnf> is the Galois closure of I<bnf>.
...
\subsecidx{bnfissunit}C<(>I<bnf>C<,>I<sfu>C<,x)>: I<bnf> being output by
...
\subsecidx{bnfisprincipal}C<(>I<bnf>C<,x,{>I<flag>C< = 1})>: I<bnf> being the
...
(specifically I<bnf>C<.gen>). The C<c_i> are chosen so that C<0 E<lt>= c_i E<lt> n_i>
where C<n_i> is the order of C<g_i> (the vector of C<n_i> being I<bnf>C<.cyc>).
...
C<v_1> is equal to the zero vector. In the latter case, C<x = >F<alpha>B<Z>C<_K> where
...
\subsecidx{bnfisunit}C<(>I<bnf>C<,x)>: I<bnf> being the number field data
...
More precisely, if C<u_1>,...,C<u_r> are the fundamental units, and F<zeta>
is the generator of the group of roots of unity (C<bnf.tu>), the output is
a vector C<[x_1,...,x_r,x_{r+1}]> such that C<x = u_1^{x_1}...
u_r^{x_r}.>F<zeta>C<^{x_{r+1}}>. The C<x_i> are integers for C<i E<lt>= r> and is an
integer modulo the order of F<zeta> for C<i = r+1>.

X<isunit>The library syntax is B<isunit>C<(>I<bnf>C<,x)>.

\subsecidx{bnfmake}C<(>I<sbnf>C<)>: I<sbnf> being a ``small I<bnf>''
...
C<gp> (or I<prec> in library mode) is greater than the precision of the
...
\subsecidx{bnfnarrow}C<(>I<bnf>C<)>: I<bnf> being as output by
C<bnfinit>, computes the narrow class group of I<bnf>. The output is
a 3-component row vector C<v> analogous to the corresponding class group
component I<bnf>C<.clgp> (I<bnf>C<[8][1]>): the first component
...
\subsecidx{bnfsignunit}C<(>I<bnf>C<)>: I<bnf> being as output by
...
\subsecidx{bnfreg}C<(>I<bnf>C<)>: I<bnf> being as output by
...
\subsecidx{bnfsunit}C<(>I<bnf>C<,S)>: computes the fundamental C<S>-units of the
...
\subsecidx{bnfunit}C<(>I<bnf>C<)>: I<bnf> being as output by
...
\subsecidx{bnrL1}C<(>I<bnr>C<,{>I<subgroup>C<},{>I<flag>C< = 0})>: I<bnr> being
...
if omitted), returns for each X<character>character F<chi> of the ray class group
which is trivial on this subgroup, the value at C<s = 1> (or C<s = 0>) of the
abelian C<L>-function associated to F<chi>. For the value at C<s = 0>, the
function returns in fact for each character F<chi> a vector C<[r_>F<chi>C< ,
c_>F<chi>C<]> where C<r_>F<chi> is the order of C<L(s, >F<chi>C<)> at C<s = 0> and C<c_>F<chi>
the first non-zero term in the expansion of C<L(s, >F<chi>C<)> at C<s = 0>; in other
words
...
primitive C<L>-functions associated to F<chi> if set to 0 or the C<L>-function
with Euler factors at prime ideals dividing the modulus of I<bnr> removed
if set to 1 (this is the so-called C<L_S(s, >F<chi>C<)> function where C<S> is the
...
X<bnrL1>The library syntax is B<bnrL1>C<(>I<bnr>C<,>I<subgroup>C<,>I<flag>C<,>I<prec>C<)>, where an omitted
I<subgroup> is coded as C<NULL>.

\subsecidx{bnrclass}C<(>I<bnf>C<,>I<ideal>C<,{>I<flag>C< = 0})>: I<this function
is DEPRECATED, use >C<bnrinit>.

I<bnf> being as output by C<bnfinit> (the units are mandatory unless
...
X<bnrclass0>The library syntax is B<bnrclass0>C<(>I<bnf>C<,>I<ideal>C<,>I<flag>C<)>.

\subsecidx{bnrclassno}C<(>I<bnf>C<,I)>: I<bnf> being as output by
...
X<bnrclassno>The library syntax is B<bnrclassno>C<(>I<bnf>C<,I)>.

\subsecidx{bnrclassnolist}C<(>I<bnf>C<,>I<list>C<)>: I<bnf> being as
...
X<bnrclassnolist>The library syntax is B<bnrclassnolist>C<(>I<bnf>C<,>I<list>C<)>.

\subsecidx{bnrconductor}C<(a_1,{a_2},{a_3}, {>I<flag>C< = 0})>: conductor C<f> of
the subfield of a ray class field as defined by C<[a_1,a_2,a_3]> (see
C<bnr> at the beginning of this section).

If I<flag>C< = 0>, returns C<f>.

If I<flag>C< = 1>, returns C<[f, Cl_f, H]>, where C<Cl_f> is the ray class group
modulo C<f>, as a finite abelian group; finally C<H> is the subgroup of C<Cl_f>
defining the extension.

If I<flag>C< = 2>, returns C<[f, >I<bnr>C<(f), H]>, as above except C<Cl_f> is
replaced by a C<bnr> structure, as output by C<X<bnrinit>bnrinit(,f,1)>.

X<conductor>The library syntax is B<conductor>C<(>I<bnr>C<, >I<subgroup>C<, >I<flag>C<)>, where an omitted subgroup
(trivial subgroup, i.e.S< >ray class field) is input as C<NULL>, and I<flag> is
a C long.

\subsecidx{bnrconductorofchar}C<(>I<bnr>C<,>I<chi>C<)>: I<bnr> being a big
...
X<bnrconductorofchar>The library syntax is B<bnrconductorofchar>C<(>I<bnr>C<,>I<chi>C<)>.

\subsecidx{bnrdisc}C<(a1,{a2},{a3},{>I<flag>C< = 0})>: C<a1>, C<a2>, C<a3>
...
If I<flag>C< = 2>, as the default case, except that if the modulus is not the
exact conductor corresponding to the C<L>, no data is computed and the result
is C<0>.

If I<flag>C< = 3>, as case 2, but output relative data.

X<bnrdisc0>The library syntax is B<bnrdisc0>C<(a1,a2,a3,>I<flag>C<)>.

\subsecidx{bnrdisclist}C<(>I<bnf>C<,>I<bound>C<,{>I<arch>C<})>:
I<bnf> being as output by C<bnfinit> (with units), computes a list of
...
The alternative syntax C<bnrdisclist(>I<bnf>C<,>I<list>C<)> is
...
gives invariants associated to the ray class field C<L> of I<bnf> of
conductor C<[m, >I<arch>C<]>. Namely, each contains a vector C<[m,d,r,D]> with
the following meaning: C<m> is the prime ideal factorization of the modulus,
C<d = [L:>B<Q>C<]> is the absolute degree of C<L>, C<r> is the number of real places
...
Finally, each prime ideal C<pr = [p,>F<alpha>C<,e,f,>F<beta>C<]> in the prime
...
X<bnrdisclist0>The library syntax is B<bnrdisclist0>C<(bnf,>I<bound>C<,>I<arch>C<)>.

\subsecidx{bnrinit}C<(>I<bnf>C<,f,{>I<flag>C< = 0})>: I<bnf> is as
...
I<bnf>, which refer to the class group; use I<bnr>C<.bnf.>I<xxx>
to access these, e.g.S< >I<bnr>C<.bnf.cyc> to get the cyclic decomposition
of the class group.

They are also different from the members of the underlying I<bid>, which
refer to C<(\O_K/f)^*>; use I<bnr>C<.bid.>I<xxx> to access these,
e.g.S< >I<bnr>C<.bid.no> to get F<phi>C<(f)>.

If I<flag>C< = 0> (default), the generators of the ray class group are not computed,
which saves time. Hence I<bnr>C<.gen> would produce an error.

If I<flag>C< = 1>, as the default, except that generators are computed.

X<bnrinit0>The library syntax is B<bnrinit0>C<(>I<bnf>C<,f,>I<flag>C<)>.

\subsecidx{bnrisconductor}C<(a1,{a2},{a3})>: C<a1>, C<a2>, C<a3> represent
...
\subsecidx{bnrisprincipal}C<(>I<bnr>C<,x,{>I<flag>C< = 1})>: I<bnr> being the
...
that C<x = >F<alpha>F<prod>C<_ig_i^{x_i}>.
...
X<bnrisprincipal>The library syntax is B<bnrisprincipal>C<(>I<bnr>C<,x,>I<flag>C<)>.

\subsecidx{bnrrootnumber}C<(>I<bnr>C<,>I<chi>C<,{>I<flag>C< = 0})>:
...
returns the root number of the character F<chi> of  F<Cl> C<_7(>B<Q>C<(> F<sqrt> C<{145}))>
...
\subsecidx{bnrstark}C<{(>I<bnr>C<,{>I<subgroup>C<})}>: I<bnr>
...
congruence subgroup (as usual, omit I<subgroup> if you want the whole
ray class group).
...
polynomials defining I<independent> relative extensions, whose compositum
...
class field must be totally real. When the base field is B<Q>, the vastly
...
returns the ray class field of B<Q>C<(> F<sqrt> C<{3})> modulo C<5>. Usually, one wants
to apply to the result one of

  rnfpolredabs(bnf, pol, 16 + 2) \\ compute a reduced relative polynomial
  rnfpolredabs(bnf, pol, 16 + 2) \\ compute a reduced absolute polynomial

X<bnrstark>The library syntax is B<bnrstark>C<(>I<bnr>C<,>I<subgroup>C<)>, where an omitted I<subgroup>
is coded by C<NULL>.

\subsecidx{dirzetak}C<(>I<nf>C<,b)>: gives as a vector the first C<b>
...
\subsecidx{factornf}C<(x,t)>: factorization of the univariate polynomial C<x>
over the number field defined by the (univariate) polynomial C<t>. C<x> may
have coefficients in B<Q> or in the number field. The algorithm reduces to
factorization over B<Q> (X<Trager>Trager's trick). The direct approach of
X<nffactor>C<nffactor>, which uses X<van Hoeij>van Hoeij's method in a relative setting, is
in general faster.

The main variable of C<t> must be of I<lower> priority than that of C<x>
...
these polmods I<must> be the same as the main variable of C<t>. For
example
...
\subsecidx{galoisexport}C<(>I<gal>C<,{>I<flag>C< = 0})>:
...
for (no flags or I<flag>C< = 0>) GAP or (I<flag>C< = 1>) Magma. The following example
...
X<galoisexport>The library syntax is B<galoisexport>C<(>I<gal>C<,>I<flag>C<)>.

\subsecidx{galoisfixedfield}C<(>I<gal>C<,>I<perm>C<,{>I<flag>C< = 0},{v = y}))>:
...
must be less than the priority of the variable of I<gal>C<.pol> (see
...
X<galoisfixedfield>The library syntax is B<galoisfixedfield>C<(>I<gal>C<,>I<perm>C<,>I<flag>C<,v)>, where C<v> is a variable number, an omitted C<v> being coded by C<-1>.

\subsecidx{galoisidentify}C<(>I<gal>C<)>:
I<gal> being be a Galois field as output by X<galoisinit>C<galoisinit>,
...
X<galoisidentify>The library syntax is B<galoisidentify>C<(>I<gal>C<)>.

\subsecidx{galoisinit}C<(>I<pol>C<,{den})>: computes the Galois group
...
S< >S< >B<*> for any C<x\in G> there exists a unique family
...
\subsecidx{galoisisabelian}C<(>I<gal>C<,{fl = 0})>: I<gal> being as output by C<galoisinit>, return C<0> if
I<gal> is not an abelian group, and the HNF matrix of I<gal> over C<gal.gen> if C<fl = 0>, C<1> if
C<fl = 1>.

This command also accepts subgroups returned by C<galoissubgroups>.

X<galoisisabelian>The library syntax is B<galoisisabelian>C<(>I<gal>C<,>I<fl>C<)> where I<fl> is a C long integer.

\subsecidx{galoispermtopol}C<(>I<gal>C<,>I<perm>C<)>: I<gal> being a
...
\subsecidx{galoissubcyclo}C<(N,H,{fl = 0},{v})>: computes the subextension
of B<Q>C<(>F<zeta>C<_n)> fixed by the subgroup C<H \subset (>B<Z>C</n>B<Z>C<)^*>. By the
...
B<*> C<N> an integer: then C<n = N>; C<H> is a generator, i.e. an
integer or an integer modulo C<n>; or a vector of generators.

B<*> C<N> the output of C<znstar(n)>. C<H> as in the first case
above, or a matrix, taken to be a HNF left divisor of the SNF for C<(>B<Z>C</n>B<Z>C<)^*>
(of type C<N.cyc>), giving the generators of C<H> in terms of C<N.gen>.

B<*> C<N> the output of C<bnrinit(bnfinit(y), m, 1)> where C<m> is a
...
an integer, then it is not a subgroup of C<(>B<Z>C</n>B<Z>C<)^*>, but of its quotient by
C<{F<+-> 1}>.

If C<fl = 0>, compute a polynomial (in the variable I<v>) defining the
the subfield of B<Q>C<(>F<zeta>C<_n)> fixed by the subgroup I<H> of C<(>B<Z>C</n>B<Z>C<)^*>.
...
B<Q>C<(>F<zeta>C<_n)> (of exact degree C<d>, if C<d> is set):
...
\subsecidx{galoissubfields}C<(G,{fl = 0},{v})>: Output all the subfields of
...
X<galoissubfields>The library syntax is B<galoissubfields>C<(>I<G>C<,fl,v)>, where I<fl> is a long and I<v> a
variable number.

\subsecidx{galoissubgroups}C<(gal)>: Output all the subgroups of the Galois
group C<gal>. A subgroup is a vector [I<gen>, I<orders>], with the same meaning
as for I<gal>C<.gen> and I<gal>C<.orders>. Hence I<gen> is a vector of
...
X<galoissubgroups>The library syntax is B<galoissubgroups>C<(>I<gal>C<)>.

\subsecidx{idealadd}C<(>I<nf>C<,x,y)>: sum of the two ideals C<x> and C<y> in the
...
\subsecidx{idealaddtoone}C<(>I<nf>C<,x,{y})>: C<x> and C<y> being two co-prime
integral ideals (given in any form), this gives a two-component row vector
C<[a,b]> such that C<a\in x>, C<b\in y> and C<a+b = 1>.
...
B<Z>C<_K>. This outputs a C<k>-component vector C<e> such that C<e[i]\in x[i]> for
...
\subsecidx{idealappr}C<(>I<nf>C<,x,{>I<flag>C< = 0})>: if C<x> is a fractional ideal
(given in any form), gives an element F<alpha> in I<nf> such that for
all prime ideals F<wp> such that the valuation of C<x> at F<wp> is non-zero, we
have C<v_{>F<wp>C<}(>F<alpha>C<) = v_{>F<wp>C<}(x)>, and. C<v_{>F<wp>C<}(>F<alpha>C<) E<gt>= 0> for all other
C<{>F<wp>C<}>.
...
This yields an element F<alpha> such that for all prime ideals F<wp> occurring
in C<x>, C<v_{>F<wp>C<}(>F<alpha>C<)> is equal to the exponent of F<wp> in C<x>, and for all
other prime ideals, C<v_{>F<wp>C<}(>F<alpha>C<) E<gt>= 0>. This generalizes
C<idealappr(>I<nf>C<,x,0)> since zero exponents are allowed. Note that
the algorithm used is slightly different, so that
C<idealappr(>I<nf>C<,idealfactor(>I<nf>C<,x))> may not be the same as
C<idealappr(>I<nf>C<,x,1)>.

X<idealappr0>The library syntax is B<idealappr0>C<(>I<nf>C<,x,>I<flag>C<)>.

\subsecidx{idealchinese}C<(>I<nf>C<,x,y)>: C<x> being a prime ideal factorization
...
C<v_>F<wp>C<(b - y_>F<wp>C<) E<gt>= v_>F<wp>C<(x)> for all prime ideals in C<x> and C<v_>F<wp>C<(b) E<gt>= 0>
for all other F<wp>.

X<idealchinese>The library syntax is B<idealchinese>C<(>I<nf>C<,x,y)>.

\subsecidx{idealcoprime}C<(>I<nf>C<,x,y)>: given two integral ideals C<x> and C<y>
in the number field I<nf>, finds a F<beta> in the field, expressed on the
integral basis I<nf>C<[7]>, such that F<beta>C<.x> is an integral ideal
coprime to C<y>.

X<idealcoprime>The library syntax is B<idealcoprime>C<(>I<nf>C<,x,y)>.

\subsecidx{idealdiv}C<(>I<nf>C<,x,y,{>I<flag>C< = 0})>: quotient C<x.y^{-1}> of the
...
are \funs{idealdiv}{I<nf>,x,y} (I<flag>C< = 0>) and
\funs{idealdivexact}{I<nf>,x,y} (I<flag>C< = 1>).

\subsecidx{idealfactor}C<(>I<nf>C<,x)>: factors into prime ideal powers the
...
\subsecidx{idealhnf}C<(>I<nf>C<,a,{b})>: gives the X<Hermite normal form>Hermite normal form
...
Also available is \funs{idealhermite}{I<nf>,a} (C<b> omitted).

\subsecidx{idealintersect}C<(>I<nf>C<,A,B)>: intersection of the two ideals
C<A> and C<B> in the number field I<nf>. The result is given in HNF.
...
This function does not apply to general B<Z>-modules, e.g.S< >orders, since its
arguments are replaced by the ideals they generate. The following script
intersects B<Z>-modules C<A> and C<B> given by matrices of compatible
...
X<idealintersect>The library syntax is B<idealintersect>C<(>I<nf>C<,A,B)>.

\subsecidx{idealinv}C<(>I<nf>C<,x)>: inverse of the ideal C<x> in the
number field I<nf>. The result is the Hermite normal form of the
...
\subsecidx{ideallist}C<(>I<nf>C<,>I<bound>C<,{>I<flag>C< = 4})>: computes the list
...
The possible values of I<flag> are:
...
component is of the form C<[>I<bid>C<,U]>, where I<bid> is as case 0
...
where we ask for the structures of the C<(>B<Z>C<[i]/I)^*> for all
...
be a C long integer. Also available is \funs{ideallist}{I<nf>,I<bound>},
corresponding to the case I<flag>C< = 4>.

\subsecidx{ideallistarch}C<(>I<nf>C<,>I<list>C<,>I<arch>C<)>:
...
X<ideallist>C<ideallist>, in particular the meaning of I<flag>.
  ...
  ? La = ideallistarch(bnf, L, [1,1]); \\ two places at infinity
  ? L = ideallist(bnf, 100, 0);
  ? l = L[98]; vector(#l, i, l[i].clgp)
  %4 = [[42, [42]], [36, [6, 6]], [42, [42]]]
...
add C<t> copies of B<Z>C</2>B<Z> to the ray class group. The following application
is more typical:
...
X<ideallistarch>The library syntax is B<ideallistarch>C<(>I<nf>C<,>I<list>C<,>I<arch>C<)>.

\subsecidx{ideallog}C<(>I<nf>C<,x,>I<bid>C<)>: I<nf> is a number field,
I<bid> a ``big ideal'' as output by C<idealstar> and C<x> a
non-necessarily integral element of I<nf> which must have valuation
equal to 0 at all prime ideals dividing C<I = >I<bid>C<[1]>. This function
computes the ``discrete logarithm'' of C<x> on the generators given in
I<bid>C<[2]>. In other words, if C<g_i> are these generators, of orders
...
\subsecidx{idealmin}C<(>I<nf>C<,x,{>I<vdir>C<})>: computes a minimum of
...
\subsecidx{idealmul}C<(>I<nf>C<,x,y,{>I<flag>C< = 0})>: ideal multiplication of the
...
\funs{idealmulred}{I<nf>,x,y,I<prec>} (I<flag>C< != 0>), where as usual,
I<prec> is a C long integer representing the precision.

\subsecidx{idealnorm}C<(>I<nf>C<,x)>: computes the norm of the idealS< >C<x>
in the number fieldS< >I<nf>.

X<idealnorm>The library syntax is B<idealnorm>C<(>I<nf>C<, x)>.

\subsecidx{idealpow}C<(>I<nf>C<,x,k,{>I<flag>C< = 0})>: computes the C<k>-th power of
...
\funs{idealpow}{I<nf>,x,k}. If C<k> is a C<long>, you can use
\funs{idealpows}{I<nf>,x,k}. Corresponding to I<flag>C< = 1> is
\funs{idealpowred}{I<nf>,vp,k,I<prec>}, where I<prec> is a
C<long>.

\subsecidx{idealprimedec}C<(>I<nf>C<,p)>: computes the prime ideal
...
and C<b> represents a F<beta>C<\in>B<Z>C<_K> such that C<P^{-1} = >B<Z>C<_K+>F<beta>C</p>B<Z>C<_K> which
will be useful for computing valuations, but which the user can ignore. The
number F<alpha> is guaranteed to have a valuation equal to 1 at the prime
...
X<primedec>The library syntax is B<primedec>C<(>I<nf>C<,p)>.

\subsecidx{idealprincipal}C<(>I<nf>C<,x)>: creates the principal ideal
...
\subsecidx{idealred}C<(>I<nf>C<,I,{>I<vdir>C< = 0})>: X<LLL>LLL reduction of
...
\subsecidx{idealstar}C<(>I<nf>C<,I,{>I<flag>C< = 1})>: outputs a I<bid> structure,
necessary for computing in the finite abelian group C<G = (>B<Z>C<_K/I)^*>. Here,
...
I<bid>C<.mod> (the modulus),
I<bid>C<.clgp> (C<G> as a finite abelian group),
I<bid>C<.no> (the cardinality of C<G>),
I<bid>C<.cyc> (elementary divisors) and
I<bid>C<.gen> (generators).

If I<flag>C< = 1> (default), the result is a I<bid> structure without
generators.

If I<flag>C< = 2>, as I<flag>C< = 1>, but including generators, which wastes some time.

If I<flag>C< = 0>, I<deprecated>. Only outputs C<(>B<Z>C<_K/I)^*> as an abelian group,
...
\subsecidx{idealtwoelt}C<(>I<nf>C<,x,{a})>: computes a two-element
...
and C<a\in>B<Z>, where C<a> is the one passed as argument if any. If C<x> is given
...
\subsecidx{idealval}C<(>I<nf>C<,x,>I<vp>C<)>: gives the valuation of the
...
\subsecidx{ideleprincipal}C<(>I<nf>C<,x)>: creates the principal idele
generated by the algebraic number C<x> (which must be of type integer,
rational or polmod) in the number field I<nf>. The result is a
...
\subsecidx{matalgtobasis}C<(>I<nf>C<,x)>: I<nf> being a number field in
...
\subsecidx{matbasistoalg}C<(>I<nf>C<,x)>: I<nf> being a number field in
C<nfinit> format, and C<x> a matrix whose coefficients are expressed as
...
\subsecidx{modreverse}C<(a)>: C<a> being a polmod C<A(X)> modulo C<T(X)>, finds
...
\subsecidx{newtonpoly}C<(x,p)>: gives the vector of the slopes of the Newton
...
\subsecidx{nfalgtobasis}C<(>I<nf>C<,x)>: this is the inverse function of
...
\subsecidx{nfbasis}C<(x,{>I<flag>C< = 0},{>I<fa>C<})>: X<integral basis>integral basis of the number
...
of I<flag> have the following meaning:
...
X<nfbasis0>The library syntax is B<nfbasis0>C<(x,>I<flag>C<,>I<fa>C<)>. An extended version is
\funs{nfbasis}{x,&d,I<flag>,I<fa>}, where C<d> receives the discriminant of the
number field (I<not> of the polynomial C<x>), and an omitted I<fa> is input
as C<NULL>. Also available are \funs{base}{x,&d} (I<flag>C< = 0>),
\funs{base2}{x,&d} (I<flag>C< = 2>) and \funs{factoredbase}{x,I<fa>,&d}.

\subsecidx{nfbasistoalg}C<(>I<nf>C<,x)>: this is the inverse function of
...
\subsecidx{nfdetint}C<(>I<nf>C<,x)>: given a pseudo-matrix C<x>, computes a
...
\subsecidx{nfdisc}C<(x,{>I<flag>C< = 0},{fa})>: X<field discriminant>field discriminant of the
number field defined by the integral, preferably monic, irreducible
polynomial C<x>. I<flag> and C<fa> are exactly as in C<nfbasis>. That is, C<fa>
...
X<nfdiscf0>The library syntax is B<nfdiscf0>C<(x,>I<flag>C<,fa)> where an omitted C<fa> is input as C<NULL>. You
can also use \funs{discf}{x} (I<flag>C< = 0>).

\subsecidx{nfeltdiv}C<(>I<nf>C<,x,y)>: given two elements C<x> and C<y> in
...
\subsecidx{nfeltdiveuc}C<(>I<nf>C<,x,y)>: given two elements C<x> and C<y> in
...
\subsecidx{nfeltdivmodpr}C<(>I<nf>C<,x,y,>I<pr>C<)>: given two elements C<x>
...
\subsecidx{nfeltdivrem}C<(>I<nf>C<,x,y)>: given two elements C<x> and C<y> in
...
X<nfdivrem>The library syntax is B<nfdivrem>C<(>I<nf>C<,x,y)>.

\subsecidx{nfeltmod}C<(>I<nf>C<,x,y)>: given two elements C<x> and C<y> in
...
\subsecidx{nfeltmul}C<(>I<nf>C<,x,y)>: given two elements C<x> and C<y> in
I<nf>, computes their product C<x*y> in the number field I<nf>.

X<element_mul>The library syntax is B<element_mul>C<(>I<nf>C<,x,y)>.

\subsecidx{nfeltmulmodpr}C<(>I<nf>C<,x,y,>I<pr>C<)>: given two elements C<x> and
...
\subsecidx{nfeltpow}C<(>I<nf>C<,x,k)>: given an element C<x> in I<nf>,
...
\subsecidx{nfeltpowmodpr}C<(>I<nf>C<,x,k,>I<pr>C<)>: given an element C<x> in
...
\subsecidx{nfeltreduce}C<(>I<nf>C<,x,>I<ideal>C<)>: given an ideal in
...
\subsecidx{nfeltreducemodpr}C<(>I<nf>C<,x,>I<pr>C<)>: given
...
X<nfreducemodpr>The library syntax is B<nfreducemodpr>C<(>I<nf>C<,x,>I<pr>C<)>.

\subsecidx{nfeltval}C<(>I<nf>C<,x,>I<pr>C<)>: given an element C<x> in
...
\subsecidx{nffactor}C<(>I<nf>C<,x)>: factorization of the univariate
...
I<the same> as the main variable of C<x>. For example,

  ? nffactor(nf, x^2 + Mod(z, z^2+1)); \\  OK
  ? nffactor(nf, x^2 + Mod(z, z^2+1)); \\  OK
  ? nffactor(nf, x^2 + Mod(z, z^2+1)); \\  WRONG

X<nffactor>The library syntax is B<nffactor>C<(>I<nf>C<,x)>.

\subsecidx{nffactormod}C<(>I<nf>C<,x,>I<pr>C<)>: factorization of the
...
main variable of I<nf> must be of lower priority than that of C<x>
...
these polmods I<must> be the same as the main variable of C<t> (see
C<nffactor>).

X<nffactormod>The library syntax is B<nffactormod>C<(>I<nf>C<,x,>I<pr>C<)>.

\subsecidx{nfgaloisapply}C<(>I<nf>C<,>I<aut>C<,x)>: I<nf> being a
...
\subsecidx{nfgaloisconj}C<(>I<nf>C<,{>I<flag>C< = 0},{d})>: I<nf> being a
...
This routine can only compute B<Q>-automorphisms, but it may be used to get
...
\funs{galoisconj}{I<nf>} for I<flag>C< = 0>,
\funs{galoisconj2}{I<nf>,n,I<prec>} for I<flag>C< = 2> where C<n> is a bound
on the number of conjugates, and  \funs{galoisconj4}{I<nf>,d}
corresponding to I<flag>C< = 4>.

\subsecidx{nfhilbert}C<(>I<nf>C<,a,b,{>I<pr>C<})>: if I<pr> is omitted,
...
\subsecidx{nfhnf}C<(>I<nf>C<,x)>: given a pseudo-matrix C<(A,I)>, finds a
...
\subsecidx{nfhnfmod}C<(>I<nf>C<,x,>I<detx>C<)>: given a pseudo-matrix C<(A,I)>
...
\subsecidx{nfinit}C<(>I<pol>C<,{>I<flag>C< = 0})>: I<pol> being a non-constant,
...
I<nf>C<[2]> contains C<[r1,r2]> (I<nf>C<.sign>, I<nf>C<.r1>,
I<nf>C<.r2>), the number of real and complex places of C<K>.

I<nf>C<[3]> contains the discriminant C<d(K)> (I<nf>C<.disc>) of C<K>.

I<nf>C<[4]> contains the index of I<nf>C<[1]> (I<nf>C<.index>),
i.e.S< >C<[>B<Z>C<_K : >B<Z>C<[>F<theta>C<]]>, where F<theta> is any root of I<nf>C<[1]>.

I<nf>C<[5]> is a vector containing 7 matrices C<M>, C<G>, C<T2>, C<T>,
...
S< >S< >B<*> C<G> is such that C<T2 = ^t G G>, where C<T2> is the quadratic
form C<T_2(x) = >F<sum>C< |>F<sigma>C<(x)|^2>, F<sigma> running over the embeddings of
C<K> into B<C>.

S< >S< >B<*> The C<T2> component is deprecated and currently unused.
...
integral basis. Note also that  F<det> C<(T)> is equal to the discriminant of the
field C<K>.
...
I<nf>C<[7]> is an integral basis for B<Z>C<_K> (I<nf>C<.zk>) expressed
on the powers ofS< >F<theta>. Its first element is guaranteed to be C<1>. This
...
the special input format C<[>I<pol>C<,B]> is also accepted where I<pol> is a
...
\subsecidx{nfisideal}C<(>I<nf>C<,x)>: returns 1 if C<x> is an ideal in
the number field I<nf>, 0 otherwise.

X<isideal>The library syntax is B<isideal>C<(x)>.

\subsecidx{nfisincl}C<(x,y)>: tests whether the number field C<K> defined
...
\subsecidx{nfisisom}C<(x,y)>: as X<nfisincl>C<nfisincl>, but tests
...
\subsecidx{nfnewprec}C<(>I<nf>C<)>: transforms the number field I<nf>
...
\subsecidx{nfkermodpr}C<(>I<nf>C<,a,>I<pr>C<)>: kernel of the matrix C<a> in
B<Z>C<_K/>I<pr>, where I<pr> is in B<modpr> format
(see C<nfmodprinit>).

X<nfkermodpr>The library syntax is B<nfkermodpr>C<(>I<nf>C<,a,>I<pr>C<)>.

\subsecidx{nfmodprinit}C<(>I<nf>C<,>I<pr>C<)>: transforms the prime ideal
...
\subsecidx{nfsubfields}C<(>I<pol>C<,{d = 0})>: finds all subfields of degree
...
polynomial defining I<nf>. This routine uses J.S< >KlE<uuml>ners's algorithm
...
\subsecidx{nfroots}C<({>I<nf>C<},x)>: roots of the polynomial C<x> in the
number field I<nf> given by C<nfinit> without multiplicity (in B<Q>
if I<nf> is omitted). C<x> has coefficients in the number field (scalar,
polmod, polynomial, column vector). The main variable of I<nf> must be
...
\subsecidx{nfrootsof1}C<(>I<nf>C<)>: computes the number of roots of unity
...
\subsecidx{nfsnf}C<(>I<nf>C<,x)>: given a torsion module C<x> as a 3-component
row
...
B<Z>C<_K/d_1\oplus...\oplus>B<Z>C<_K/d_n> and C<d_i> divides C<d_{i-1}> for C<i E<gt>= 2>.
...
S<  >C< (b_1e_1\oplus...\oplus b_ne_n) / (a_1A_1\oplus...\oplus a_nA_n)
, >
...
\subsecidx{nfsolvemodpr}C<(>I<nf>C<,a,b,>I<pr>C<)>: solution of C<a.x = b>
...
\subsecidx{polcompositum}C<(P,Q,{>I<flag>C< = 0})>:X<compositum> C<P> and C<Q>
being squarefree polynomials in B<Z>C<[X]> in the same variable, outputs
the simple factors of the E<eacute>tale B<Q>-algebra C<A = >B<Q>C<(X, Y) / (P(X), Q(Y))>.
The factors are given by a list of polynomials C<R> in B<Z>C<[X]>, associated to
the number field B<Q>C<(X)/ (R)>, and sorted by increasing degree (with respect
...
is in general I<much> faster to proceed as follows
...
If I<flag>C< = 1>, outputs a vector of 4-component vectors C<[R,a,b,k]>, where C<R>
...
B<Q>C<(X)/(R)>. Finally, C<k> is a small integer such that C<b + ka = X> modulo
C<R>.
...
the field B<Q>C<(>F<zeta>C<_5, 5^{1/5})>:

  ? pol = z[1]
  %5 = x^20 + 25*x^10 + 5
  ? a = subst(a.pol, x, z[2])  \\ C<pol> defines the compositum
  ? pol = z[1]
  %5 = x^20 + 25*x^10 + 5
  ? a = subst(a.pol, x, z[2])  \\ C<a> is a fifth root of 5
  ? pol = z[1]
  %5 = x^20 + 25*x^10 + 5
  ? a = subst(a.pol, x, z[2])  \\ look for a simpler polynomial
  ? pol = z[1]
  %5 = x^20 + 25*x^10 + 5
  ? a = subst(a.pol, x, z[2])  \\ C<a> in the new coordinates
  %6 = Mod(-5/22*x^19 + 1/22*x^14 - 123/22*x^9 + 9/11*x^4, x^20 + 25*x^10 + 5)

X<polcompositum0>The library syntax is B<polcompositum0>C<(P,Q,>I<flag>C<)>.

\subsecidx{polgalois}C<(x)>: X<Galois>Galois group of the non-constant
polynomial C<x\in>B<Q>C<[X]>. In the present version , C<x> must be irreducible
...
to eleven'', G.S< >Butler and J.S< >McKay, I<Communications in Algebra>, vol.S< >11,
1983,
...
X<polgalois>The library syntax is B<polgalois>C<(x,>I<prec>C<)>. To enable the new format in library mode,
set the global variable X<new_galois_format>C<new_galois_format> to C<1>.

\subsecidx{polred}C<(x,{>I<flag>C< = 0},{fa})>: finds polynomials with reasonably
...
X<polred0>The library syntax is B<polred0>C<(x,>I<flag>C<,fa)>, where an omitted C<fa> is coded by C<NULL>. Also
available are \funs{polred}{x} and \funs{factoredpolred}{x,fa}, both
corresponding to I<flag>C< = 0>.

\subsecidx{polredabs}C<(x,{>I<flag>C< = 0})>: finds one of the polynomial defining
...
X<polredabs0>The library syntax is B<polredabs0>C<(x,>I<flag>C<)>.

\subsecidx{polredord}C<(x)>: finds polynomials with reasonably small
...
\subsecidx{poltschirnhaus}C<(x)>:  applies a random Tschirnhausen
...
\subsecidx{rnfalgtobasis}C<(>I<rnf>C<,x)>:  expresses C<x> on the relative
integral basis. Here, I<rnf> is a relative number field extension C<L/K>
as output by C<rnfinit>, and C<x> an element of C<L> in absolute form, i.e.
expressed as a polynomial or polmod with polmod coefficients, I<not> on
the relative integral basis.

X<rnfalgtobasis>The library syntax is B<rnfalgtobasis>C<(>I<rnf>C<,x)>.

\subsecidx{rnfbasis}C<(>I<bnf>C<, M)>: let C<K> the field represented by
I<bnf>, as output by C<bnfinit>. C<M> is a projective B<Z>C<_K>-module
given by a pseudo-basis, as output by C<rnfhnfbasis>. The routine returns
either a true B<Z>C<_K>-basis of C<M> if it exists, or an C<n+1>-element
generating set of C<M> if not, where C<n> is the rank of C<M> over I<K>.
...
(C<P> is assumed irreducible over C<K>), viewed as a B<Z>C<_K>-module.

X<rnfbasis>The library syntax is B<rnfbasis>C<(>I<bnf>C<,x)>.

\subsecidx{rnfbasistoalg}C<(>I<rnf>C<,x)>: computes the representation of C<x>
as a polmod with polmods coefficients. Here, I<rnf> is a relative number
...
\subsecidx{rnfcharpoly}C<(>I<nf>C<,T,a,{v = x})>: characteristic polynomial of
...
\subsecidx{rnfconductor}C<(>I<bnf>C<,>I<pol>C<,{>I<flag>C< = 0})>: given I<bnf>
...
group on the given generators gen. If I<flag> is non-zero, check that I<pol>
indeed defines an Abelian extension, return 0 if it does not.

X<rnfconductor>The library syntax is B<rnfconductor>C<(>I<rnf>C<,>I<pol>C<,>I<flag>C<)>.

\subsecidx{rnfdedekind}C<(>I<nf>C<,>I<pol>C<,>I<pr>C<)>: given a number field
...
\subsecidx{rnfdet}C<(>I<nf>C<,M)>: given a pseudo-matrix C<M> over the maximal
order of I<nf>, computes its determinant.

X<rnfdet>The library syntax is B<rnfdet>C<(>I<nf>C<,M)>.

\subsecidx{rnfdisc}C<(>I<nf>C<,>I<pol>C<)>: given a number field I<nf> as
output by C<nfinit> and a polynomial I<pol> with coefficients in
I<nf> defining a relative extension C<L> of I<nf>, computes the
...
I<nf> I<must> be of lower priority than that of I<pol>, see
L<Label se:priority>.

X<rnfdiscf>The library syntax is B<rnfdiscf>C<(>I<bnf>C<,>I<pol>C<)>.

\subsecidx{rnfeltabstorel}C<(>I<rnf>C<,x)>: I<rnf> being a relative
...
I<rnf>C<.pol>, computes C<x> as an element of the relative extension
C<L/K> as a polmod with polmod coefficients.

X<rnfelementabstorel>The library syntax is B<rnfelementabstorel>C<(>I<rnf>C<,x)>.

\subsecidx{rnfeltdown}C<(>I<rnf>C<,x)>: I<rnf> being a relative number
...
\subsecidx{rnfeltreltoabs}C<(>I<rnf>C<,x)>: I<rnf> being a relative
number field extension C<L/K> as output by C<rnfinit> and C<x> being an
...
a polynomial modulo the absolute equation I<rnf>C<.pol>. If C<x> is
...
\subsecidx{rnfeltup}C<(>I<rnf>C<,x)>: I<rnf> being a relative number
field extension C<L/K> as output by C<rnfinit> and C<x> being an element of
...
I<rnf>C<.pol>. If C<x> is given on the integral basis of C<K>, apply
...
\subsecidx{rnfequation}C<(>I<nf>C<,>I<pol>C<,{>I<flag>C< = 0})>: given a number field
...
not check whether the relative equation is irreducible over I<nf>, but
...
\subsecidx{rnfhnfbasis}C<(>I<bnf>C<,x)>: given I<bnf> as output by
C<bnfinit>, and either a polynomial C<x> with coefficients in I<bnf>
defining a relative extension C<L> of I<bnf>, or a pseudo-basis C<x> of
such an extension, gives either a true I<bnf>-basis of C<L> in upper
triangular Hermite normal form, if it exists, and returns C<0> otherwise.

X<rnfhnfbasis>The library syntax is B<rnfhnfbasis>C<(>I<nf>C<,x)>.

\subsecidx{rnfidealabstorel}C<(>I<rnf>C<,x)>: let I<rnf> be a relative
number field extension C<L/K> as output by C<rnfinit>, and C<x> an ideal of
the absolute extension C<L/>B<Q> given by a B<Z>-basis of elements of C<L>.
...
instance as output by C<X<idealhnf>idealhnf(>I<nf>C<,...)>,
use C<rnfidealabstorel(rnf, nf.zk * x)> to convert it to a relative ideal.

X<rnfidealabstorel>The library syntax is B<rnfidealabstorel>C<(>I<rnf>C<,x)>.X<Label se:rnfidealabstorel>

\subsecidx{rnfidealdown}C<(>I<rnf>C<,x)>: let I<rnf> be a relative number
field extension C<L/K> as output by C<rnfinit>, and C<x> an ideal of
C<L>, given either in relative form or by a B<Z>-basis of elements of C<L>
...
\subsecidx{rnfidealhnf}C<(>I<rnf>C<,x)>: I<rnf> being a relative number
...
C<x>, viewed as a B<Z>C<_K>-module.

X<rnfidealhermite>The library syntax is B<rnfidealhermite>C<(>I<rnf>C<,x)>.

\subsecidx{rnfidealmul}C<(>I<rnf>C<,x,y)>: I<rnf> being a relative number
...
\subsecidx{rnfidealnormabs}C<(>I<rnf>C<,x)>: I<rnf> being a relative
...
C<idealnorm(rnfidealnormrel(>I<rnf>C<,x))>, but faster.

X<rnfidealnormabs>The library syntax is B<rnfidealnormabs>C<(>I<rnf>C<,x)>.

\subsecidx{rnfidealnormrel}C<(>I<rnf>C<,x)>: I<rnf> being a relative
number field extension C<L/K> as output by C<rnfinit> and C<x> being a
relative ideal (which can be, as in the absolute case, of many different
...
\subsecidx{rnfidealreltoabs}C<(>I<rnf>C<,x)>: I<rnf> being a relative
number field extension C<L/K> as output by C<rnfinit> and C<x> being a
relative ideal, gives the ideal C<x>B<Z>C<_L> as an absolute ideal of C<L/>B<Q>, in
the form of a B<Z>-basis, given by a vector of polynomials (modulo
C<rnf.pol>).
...
\subsecidx{rnfidealtwoelt}C<(>I<rnf>C<,x)>: I<rnf> being a relative
number field extension C<L/K> as output by C<rnfinit> and C<x> being an
ideal of the relative extension C<L/K> given by a pseudo-matrix, gives a
vector of two generators of C<x> over B<Z>C<_L> expressed as polmods with polmod
coefficients.

X<rnfidealtwoelement>The library syntax is B<rnfidealtwoelement>C<(>I<rnf>C<,x)>.

\subsecidx{rnfidealup}C<(>I<rnf>C<,x)>: I<rnf> being a relative number
field extension C<L/K> as output by C<rnfinit> and C<x> being an ideal of
C<K>, gives the ideal C<x>B<Z>C<_L> as an absolute ideal of C<L/>B<Q>, in the form of a
B<Z>-basis, given by a vector of polynomials (modulo C<rnf.pol>).
The following routine might be useful:
...
\subsecidx{rnfinit}C<(>I<nf>C<,>I<pol>C<)>: I<nf> being a number field in
C<nfinit>
...
(see L<Label se:priority>) than that of I<nf>, and the coefficients of
I<pol> must be in I<nf>.

The result is a row vector, whose components are technical. In the following
description, we let C<K> be the base field defined by I<nf>, C<m> the
...
I<rnf>C<[2]> is currently unused.

I<rnf>C<[3]> is a two-component row vector C<[>B<d>C<(L/K),s]> where
B<d>C<(L/K)> is the relative ideal discriminant of C<L/K> and C<s> is the
...
I<rnf>C<[5]> is currently unused.

I<rnf>C<[6]> is currently unused.
...
I<rnf>C<[9]> is currently unused.

I<rnf>C<[10]> is I<nf>.

I<rnf>C<[11]> is the output of C<rnfequation(nf, pol, 1)>. Namely, a
vector I<vabs> with 3 entries describing the I<absolute> extension
C<L/>B<Q>. I<vabs>C<[1]> is an absolute equation, more conveniently obtained
as C<rnf.pol>. I<vabs>C<[2]> expresses the generator F<alpha> of the
number field I<nf> as a polynomial modulo the absolute equation
I<vabs>C<[1]>. I<vabs>C<[3]> is a small integer C<k> such that, if F<beta>
is an abstract root of I<pol> and F<alpha> the generator of I<nf>,
the generator whose root is I<vabs> will be
F<beta>C< + k >F<alpha>. Note that one must be very careful if C<k != 0> when
...
F<beta>C< + k >F<alpha>. Typically, the GP instruction would be

C<pol = subst(pol, x, x - k*Mod(y,>I<nf>C<.pol))>

I<rnf>C<[12]> is by default unused and set equal to 0. This
...
\subsecidx{rnfisfree}C<(>I<bnf>C<,x)>: given I<bnf> as output by
C<bnfinit>, and either a polynomial C<x> with coefficients in I<bnf>
defining a relative extension C<L> of I<bnf>, or a pseudo-basis C<x> of
...
\subsecidx{rnfisnorm}C<(T,a,{>I<flag>C< = 0})>: similar to
...
The output is a vector C<[x,q]>, where C<a = > F<Norm> C<(x)*q>. The
...
Galois, then this is enough; otherwise, I<flag> is used to add more primes to
C<S>: all the places above the primes C<p E<lt>= >I<flag> (resp.S< >C<p|>I<flag>) if I<flag>C< E<gt> 0>
(resp.S< >I<flag>C< E<lt> 0>).
...
C<12> F<log> C<^2|> F<disc> C<(M)|>, where C<M> is the normal
closure of C<L/K>.

If X<rnfisnorminit>C<rnfisnorminit> has determined (or was told) that C<L/K> is
X<Galois>Galois, and I<flag>C< != 0>, a Warning is issued (so that you can set
I<flag>C< = 1> to check whether C<L/K> is known to be Galois, according to C<T>).
Example:
...
X<rnfisnorm>The library syntax is B<rnfisnorm>C<(>I<T>C<,x,>I<flag>C<)>.

\subsecidx{rnfisnorminit}C<(>I<pol>C<,>I<polrel>C<,{>I<flag>C< = 2})>:
...
or I<bnf>, etc; if I<pol> has degree C<1> (the base field is B<Q>),
...
If I<flag>C< = 0>, do not care whether C<L/K> is Galois or not.

If I<flag>C< = 1>, C<L/K> is assumed to be Galois (unchecked), which speeds up
X<rnfisnorm>C<rnfisnorm>.

If I<flag>C< = 2>, let the routine determine whether C<L/K> is Galois.

X<rnfisnorminit>The library syntax is B<rnfisnorminit>C<(>I<pol>C<,>I<polrel>C<,>I<flag>C<)>.

\subsecidx{rnfkummer}C<(>I<bnr>C<,{>I<subgroup>C<},{deg = 0})>: I<bnr>
...
the I<same> conductor as C<(>I<bnr>C<, >I<subgroup>C<)>.
...
X<rnfkummer>The library syntax is B<rnfkummer>C<(>I<bnr>C<,>I<subgroup>C<,>I<deg>C<,>I<prec>C<)>, where
I<deg> is a C<long> and an omitted I<subgroup> is coded as
C<NULL>

\subsecidx{rnflllgram}C<(>I<nf>C<,>I<pol>C<,>I<order>C<)>: given a polynomial
...
C<rnfpseudobasis>C<(>I<nf>C<,>I<pol>C<)> or similar, gives
C<[[>I<neworder>C<],U]>, where I<neworder> is a reduced order and C<U> is
the unimodular transformation matrix.

X<rnflllgram>The library syntax is B<rnflllgram>C<(>I<nf>C<,>I<pol>C<,>I<order>C<,>I<prec>C<)>.

\subsecidx{rnfnormgroup}C<(>I<bnr>C<,>I<pol>C<)>: I<bnr> being a big ray
...
\subsecidx{rnfpolred}C<(>I<nf>C<,>I<pol>C<)>: relative version of C<polred>.
...
containing the original field. In the present version , this is slower
and less efficient than C<rnfpolredabs>.

X<rnfpolred>The library syntax is B<rnfpolred>C<(>I<nf>C<,>I<pol>C<,>I<prec>C<)>.

\subsecidx{rnfpolredabs}C<(>I<nf>C<,>I<pol>C<,{>I<flag>C< = 0})>: relative version of
C<polredabs>. Given a monic polynomial I<pol> with coefficients in
I<nf>, finds a simpler relative polynomial defining the same field. The
binary digits of I<flag> mean
...
\subsecidx{rnfpseudobasis}C<(>I<nf>C<,>I<pol>C<)>: given a number field
I<nf> as output by C<nfinit> and a polynomial I<pol> with
coefficients in I<nf> defining a relative extension C<L> of I<nf>,
computes a pseudo-basis C<(A,I)> for the maximal order B<Z>C<_L> viewed as a
B<Z>C<_K>-module, and the relative discriminant of C<L>. This is output as a
...
\subsecidx{rnfsteinitz}C<(>I<nf>C<,x)>: given a number field I<nf> as
...
X<Steinitz class>Steinitz class of the B<Z>C<_K>-module B<Z>C<_L> (its image in C<SK_0(>B<Z>C<_K)>).

X<rnfsteinitz>The library syntax is B<rnfsteinitz>C<(>I<nf>C<,x)>.

\subsecidx{subgrouplist}C<(>I<bnr>C<,{>I<bound>C<},{>I<flag>C< = 0})>:
...
If I<flag>C< = 0> (default) and I<bnr> is as output by C<bnrinit>, gives
...
B<Q>C<(>F<zeta>C<_{120})>, of degree C<8> and conductor C<120 oo > (by setting I<flag>,
...
X<subgrouplist0>The library syntax is B<subgrouplist0>C<(>I<bnr>C<,>I<bound>C<,>I<flag>C<)>, where I<flag>
is a long integer, and an omitted I<bound> is coded by C<NULL>.

\subsecidx{zetak}C<(>I<znf>C<,x,{>I<flag>C< = 0})>: I<znf> being a number
...
B<*> The accuracy of the result depends in an essential way on the
...
B<*> As the precision increases, results become unexpectedly
completely wrong:
...
\funs{gzetak}{I<znf>,x,I<prec>}.

\subsecidx{zetakinit}C<(x)>: computes a number of initialization data
...
\subsecidx{O}C<(p^e)>: if C<p> is an integer
...
X<zeropadic>The library syntax is B<zeropadic>C<(p,e)> for a C<p>-adic and \funs{zeroser}{v,e} for a
power series zero in variable C<v>, which is a C<long>. The precision C<e>
is a C<long>.

\subsecidx{deriv}C<(x,{v})>: derivative of C<x> with respect to the main
...
\subsecidx{eval}C<(x)>: replaces in C<x> the formal variables by the values that
...
X<geval>The library syntax is B<geval>C<(x)>. The more basic functions \funs{poleval}{q,x},
\funs{qfeval}{q,x}, and \funs{hqfeval}{q,x} evaluate C<q> at C<x>, where C<q>
...
\subsecidx{factorpadic}C<(>I<pol>C<,p,r,{>I<flag>C< = 0})>: C<p>-adic factorization
...
\subsecidx{intformal}C<(x,{v})>: X<formal integration>formal integration of C<x> with
...
\subsecidx{padicappr}C<(>I<pol>C<,a)>: vector of C<p>-adic roots of the
...
C<p>-adic number (type C<t_PADIC>, i.e.S< >an element of B<Z>C<_p>) or can be an
integral element of a finite extension of B<Q>C<_p>, given as a C<t_POLMOD>
at least one of whose coefficients is a C<t_PADIC>. In this case, the result
is the vector of roots belonging to the same extension of B<Q>C<_p> as C<a>.

X<padicappr>The library syntax is B<padicappr>C<(>I<pol>C<,a)>.

\subsecidx{polcoeff}C<(x,s,{v})>: coefficient of degree C<s> of the
...
as C<-1>. Also available is \funs{truecoeff}{x,v}.

\subsecidx{poldegree}C<(x,{v})>: degree of the polynomial C<x> in the main
...
omitted C<v> is coded as C<-1>). Also available is \funs{degree}{x}, which is
equivalent to C<poldegree(x,-1)>.

\subsecidx{polcyclo}C<(n,{v = x})>: C<n>-th cyclotomic polynomial, in variable
...
\subsecidx{poldisc}C<(>I<pol>C<,{v})>: discriminant of the polynomial
...
X<poldisc0>The library syntax is B<poldisc0>C<(x,v)>. Also available is \funs{discsr}{x}, equivalent
to C<poldisc0(x,-1)>.

\subsecidx{poldiscreduced}C<(f)>: reduced discriminant vector of the
...
\subsecidx{polhensellift}C<(x, y, p, e)>: given a prime C<p>, an integral
...
\subsecidx{polinterpolate}C<(xa,{ya},{v = x},{&e})>: given the data vectors
...
\subsecidx{polisirreducible}C<(>I<pol>C<)>: I<pol> being a polynomial
(univariate in the present version ), returns 1 if I<pol> is
...
\subsecidx{pollead}C<(x,{v})>: leading coefficient of the polynomial or
...
C<-1>. Also available is \funs{leading_term}{x}.

\subsecidx{pollegendre}C<(n,{v = x})>: creates the C<n-th>
...
\subsecidx{polrecip}C<(>I<pol>C<)>: reciprocal polynomial of I<pol>,
i.e.S< >the coefficients are in reverse order. I<pol> must be a polynomial.

X<polrecip>The library syntax is B<polrecip>C<(x)>.

\subsecidx{polresultant}C<(x,y,{v},{>I<flag>C< = 0})>: resultant of the two
...
If I<flag>C< = 0>, uses the X<subresultant algorithm>subresultant algorithm.
...
is coded as C<-1>. Also available are \funs{subres}{x,y} (I<flag>C< = 0>) and
\funs{resultant2}{x,y} (I<flag>C< = 1>).

\subsecidx{polroots}C<(>I<pol>C<,{>I<flag>C< = 0})>: complex roots of the polynomial
...
X<roots>The library syntax is B<roots>C<(>I<pol>C<,>I<prec>C<)> or \funs{rootsold}{I<pol>,I<prec>}.

\subsecidx{polrootsmod}C<(>I<pol>C<,p,{>I<flag>C< = 0})>: row vector of roots modulo
...
If C<p> is very small, you may try setting I<flag>C< = 1>, which uses a naive search.

X<rootmod>The library syntax is B<rootmod>C<(>I<pol>C<,p)> (I<flag>C< = 0>) or
\funs{rootmod2}{I<pol>,p} (I<flag>C< = 1>).

\subsecidx{polrootspadic}C<(>I<pol>C<,p,r)>: row vector of C<p>-adic roots of
the polynomial I<pol>, given to C<p>-adic precision C<r>. Multiple roots are
I<not> repeated. C<p> is assumed to be a prime, and I<pol> to be
non-zero modulo C<p>. Note that this is not the same as the roots in
B<Z>C</p^r>B<Z>, rather it gives approximations in B<Z>C</p^r>B<Z> of the true
roots living in B<Q>C<_p>.
...
to B<Z>. Hence the roots given are approximations of the roots of a
...
\subsecidx{polsturm}C<(>I<pol>C<,{a},{b})>: number of real roots of the real
...
\funs{sturm}{I<pol>} is equivalent to
\funs{sturmpart}{I<pol>,C<NULL>,C<NULL>}. The result is a
C<long>.

\subsecidx{polsubcyclo}C<(n,d,{v = x})>: gives polynomials (in variable
C<v>) defining the sub-Abelian extensions of degree C<d> of the cyclotomic
field B<Q>C<(>F<zeta>C<_n)>, where C<d | >F<phi>C<(n)>.
...
variable number. When C<(>B<Z>C</n>B<Z>C<)^*> is cyclic, you can use
\funs{subcyclo}{n,d,v}, where C<n>, C<d> and C<v> are C<long> and C<v> is a
variable number.

\subsecidx{polsylvestermatrix}C<(x,y)>: forms the Sylvester matrix
...
\subsecidx{polsym}C<(x,n)>: creates the vector of the X<symmetric powers>symmetric powers
of the roots of the polynomial C<x> up to power C<n>, using Newton's
formula.

X<polsym>The library syntax is B<polsym>C<(x)>.

\subsecidx{poltchebi}C<(n,{v = x})>: creates the C<n-th>
...
\subsecidx{polzagier}C<(n,m)>: creates Zagier's polynomial C<P_n^{(m)}> used in
the functions C<sumalt> and C<sumpos> (with I<flag>C< = 1>). One must have C<m E<lt>= 
...
with real coefficients to the precision I<prec>, or \funs{polzag}{n,m}
if the result is wanted exactly, where C<n> and C<m> are C<long>s.

\subsecidx{serconvol}C<(x,y)>: convolution (or X<Hadamard product>Hadamard product) of the
...
\subsecidx{serlaplace}C<(x)>: C<x> must be a power series with non-negative
exponents. If C<x = >F<sum>C< (a_k/k!)*X^k> then the result is F<sum>C< a_k*X^k>.

X<laplace>The library syntax is B<laplace>C<(x)>.

\subsecidx{serreverse}C<(x)>: reverse power series (i.e.S< >C<x^{-1}>, not C<1/x>)
of C<x>. C<x> must be a power series whose valuation is exactly equal to one.

X<recip>The library syntax is B<recip>C<(x)>.

\subsecidx{subst}C<(x,y,z)>:
...
\subsecidx{substvec}C<(x,v,w)>: C<v> being a vector of monomials (variables),
...
\subsecidx{taylor}C<(x,y)>: Taylor expansion around C<0> of C<x> with respect
toX<Label se:taylor>
...
are likewise conditional, I<except> for a right-hand side of C<F<+-> 1>.

X<thue>The library syntax is B<thue>C<(>I<tnf>C<,a,>I<sol>C<)>, where an omitted I<sol> is coded
as C<NULL>.

\subsecidx{thueinit}C<(P,{>I<flag>C< = 0})>: initializes the I<tnf>
...
I<If> the conditional computed class group is trivial I<or> you are
...
and work over its field of fractions. If the base ring is I<not>
...
specifically assume the base ring is B<Z>.

\subsecidx{algdep}C<(x,k,{>I<flag>C< = 0})>:X<algebraic dependence>
...
Internally, X<lindep>C<lindep>C<([1,x,...,x^k], >I<flag>C<)> is used. If
...
A suitable non-zero value of I<flag> may improve on the default behaviour:
...
Lisonek, Applications of integer relation algorithms, I<Discrete Math.>,
...
Also available is \funs{algdep}{x,k,I<prec>} (I<flag>C< = 0>).

\subsecidx{charpoly}C<(A,{v = x},{>I<flag>C< = 0})>: X<characteristic polynomial>characteristic polynomial
...
are the functions \funs{caract}{A,v} (I<flag>C< = 1>), \funs{carhess}{A,v}
(I<flag>C< = 2>), and \funs{caradj}{A,v,I<pt>} where, in this last case,
...
\subsecidx{concat}C<(x,{y})>: concatenation of C<x> and C<y>. If C<x> or C<y> is
...
\subsecidx{lindep}C<(x,{>I<flag>C< = 0})>:X<linear dependence>C<x> being a
vector with C<p>-adic or real/complex coefficients, finds a small integral
linear combination among these coefficients.

If C<x> is C<p>-adic, I<flag> is meaningless and the algorithm LLL-reduces a
suitable (dual) lattice.

Otherwise, the value of I<flag> determines the algorithm used; in the current
version of PARI, we suggest to use I<non-negative> values, since it is by
far the fastest and most robust implementation. See the detailed example in
L<Label se:algdep> (C<algdep>).

If I<flag>C< E<gt>= 0>, uses a floating point (variable precision) LLL algorithm.
This is in general much faster than the other variants. 
If I<flag>C< = 0> the accuracy is chosen internally using a crude heuristic.
If I<flag>C< E<gt> 0> the computation is done with an accuracy of I<flag> decimal digits.
In that case, the parameter I<flag> should be between 0.6 and 0.9 times the
number of correct decimal digits in the input.

If I<flag>C< = -1>, uses a variant of the X<LLL>LLL algorithm due to Hastad,
Lagarias and Schnorr (STACS 1986). If the precision is too low, the routine
may enter an infinite loop.

If I<flag>C< = -2>, C<x> is allowed to be (and in any case interpreted as) a matrix.
...
If I<flag>C< = -3>, uses the PSLQ algorithm. This may return a real number C<B>,
indicating that the input accuracy was exhausted and that no relation exist
whose sup norm is less than C<B>.

If I<flag>C< = -4>, uses an experimental 2-level PSLQ, which does not work at all.
(Should be rewritten.)

X<lindep0>The library syntax is B<lindep0>C<(x,>I<flag>C<,>I<prec>C<)>. Also available is
\funs{lindep}{x,I<prec>} (I<flag>C< = 0>).

\subsecidx{listcreate}C<(n)>: creates an empty list of maximal length C<n>.

This function is useless in library mode.

\subsecidx{listinsert}C<(>I<list>C<,x,n)>: inserts the object C<x> at
...
\subsecidx{listkill}C<(>I<list>C<)>: kill I<list>. This deletes all
elements from I<list> and sets its effective length to C<0>. The maximal
length is not affected.

This function is useless in library mode.

\subsecidx{listput}C<(>I<list>C<,x,{n})>: sets the C<n>-th element of the list
...
\subsecidx{listsort}C<(>I<list>C<,{>I<flag>C< = 0})>: sorts I<list> (which must
...
\subsecidx{matadjoint}C<(x)>: X<adjoint matrix>adjoint matrix of C<x>, i.e.S< >the matrix C<y>
of cofactors of C<x>, satisfying C<x*y = > F<det> C<(x)*> F<Id> . C<x> must be a
(non-necessarily invertible) square matrix.

X<adj>The library syntax is B<adj>C<(x)>.

\subsecidx{matcompanion}C<(x)>: the left companion matrix to the polynomial C<x>.

X<assmat>The library syntax is B<assmat>C<(x)>.

\subsecidx{matdet}C<(x,{>I<flag>C< = 0})>: determinant of C<x>. C<x> must be a
square matrix.
...
X<det>The library syntax is B<det>C<(x)> (I<flag>C< = 0>) and \funs{det2}{x} (I<flag>C< = 1>).

\subsecidx{matdetint}C<(x)>: C<x> being an C<m x n> matrix with integer
coefficients, this function computes a I<multiple> of the determinant of the
...
\subsecidx{matdiagonal}C<(x)>: C<x> being a vector, creates the diagonal matrix
whose diagonal entries are those of C<x>.

X<diagonal>The library syntax is B<diagonal>C<(x)>.

\subsecidx{mateigen}C<(x)>: gives the eigenvectors of C<x> as columns of a
matrix.

X<eigen>The library syntax is B<eigen>C<(x)>.

\subsecidx{matfrobenius}C<(M,{>I<flag>C< = 0},{v = x})>: returns the Frobenius form of
the square matrix C<M>. If I<flag>C< = 1>, returns only the elementary divisors as
a vectr of polynomials in the variable C<v>.  If I<flag>C< = 2>, returns a
two-components vector [F,B] where C<F> is the Frobenius form and C<B> is
the basis change so that C<M = B^{-1}FB>.

X<matfrobenius>The library syntax is B<matfrobenius>C<(M,>I<flag>C<,v)>, where C<v> is the variable number.

\subsecidx{mathess}C<(x)>: Hessenberg form of the square matrix C<x>.

X<hess>The library syntax is B<hess>C<(x)>.

\subsecidx{mathilbert}C<(x)>: C<x> being a C<long>, creates the
...
\subsecidx{mathnf}C<(x,{>I<flag>C< = 0})>: if C<x> is a (not necessarily square)
matrix with integer entries, finds the I<upper triangular>
...
If I<flag>C< = 0>, uses the naive algorithm. This should never be used if the
...
If I<flag>C< = 1>, uses Batut's algorithm, which is much faster than the default.
Outputs a two-component row vector C<[H,U]>, where C<H> is the I<upper
...
If I<flag>C< = 3>, uses Batut's algorithm, but outputs C<[H,U,P]>, such that C<H> and
C<U> are as before and C<P> is a permutation of the rows such that C<P> applied
to C<xU> gives C<H>. The matrix C<U> is smaller than with I<flag>C< = 1>, but may still
be large.

If I<flag>C< = 4>, as in case 1 above, but uses a heuristic variant of X<LLL>LLL
...
X<mathnf0>The library syntax is B<mathnf0>C<(x,>I<flag>C<)>. Also available are \funs{hnf}{x} (I<flag>C< = 0>) and
\funs{hnfall}{x} (I<flag>C< = 1>). To reduce I<huge> (say C<400  x 400> and
...
\subsecidx{mathnfmod}C<(x,d)>: if C<x> is a (not necessarily square) matrix of
...
\subsecidx{mathnfmodid}C<(x,d)>: outputs the (upper triangular)
...
\subsecidx{matid}C<(n)>: creates the C<n x n> identity matrix.

X<matid>The library syntax is B<matid>C<(n)> where C<n> is a C<long>.

Related functions are \funs{gscalmat}{x,n}, which creates C<x> times the
identity matrix (C<x> being a C<GEN> and C<n> a C<long>), and
\funs{gscalsmat}{x,n} which is the same when C<x> is a C<long>.

\subsecidx{matimage}C<(x,{>I<flag>C< = 0})>: gives a basis for the image of the
...
X<matimage0>The library syntax is B<matimage0>C<(x,>I<flag>C<)>. Also available is \funs{image}{x} (I<flag>C< = 0>).

\subsecidx{matimagecompl}C<(x)>: gives the vector of the column indices which
...
\subsecidx{matindexrank}C<(x)>: C<x> being a matrix of rank C<r>, gives two
...
\subsecidx{matintersect}C<(x,y)>: C<x> and C<y> being two matrices with the same
...
\subsecidx{matinverseimage}C<(M,y)>: gives a column vector belonging to the
...
\subsecidx{matisdiagonal}C<(x)>: returns true (1) if C<x> is a diagonal matrix,
false (0) if not.

X<isdiagonal>The library syntax is B<isdiagonal>C<(x)>, and this returns a C<long>
integer.

\subsecidx{matker}C<(x,{>I<flag>C< = 0})>: gives a basis for the kernel of the
matrix C<x> as columns of a matrix. A priori the matrix can have entries of
any type.
...
integer entries I<reduced mod p> and C<p> is prime, is equivalent to, but
...
X<matker0>The library syntax is B<matker0>C<(x,>I<flag>C<)>. Also available are \funs{ker}{x} (I<flag>C< = 0>),
\funs{keri}{x} (I<flag>C< = 1>).

\subsecidx{matkerint}C<(x,{>I<flag>C< = 0})>: gives an X<LLL>LLL-reduced B<Z>-basis
...
\funs{kerint}{x} (I<flag>C< = 0>).

\subsecidx{matmuldiagonal}C<(x,d)>: product of the matrix C<x> by the diagonal
...
\subsecidx{matmultodiagonal}C<(x,y)>: product of the matrices C<x> and C<y>
...
\subsecidx{matpascal}C<(x,{q})>: creates as a matrix the lower triangular
...
to omit C<q>. Also available is \funs{matpascal}{x}.

\subsecidx{matrank}C<(x)>: rank of the matrix C<x>.

X<rank>The library syntax is B<rank>C<(x)>, and the result is a C<long>.

\subsecidx{matrix}C<(m,n,{X},{Y},{>I<expr>C< = 0})>: creation of the
...
\subsecidx{matrixqz}C<(x,p)>: C<x> being an C<m x n> matrix with C<m E<gt>= n>
...
\subsecidx{matsize}C<(x)>: C<x> being a vector or matrix, returns a row vector
with two components, the first being the number of rows (1 for a row vector),
the second the number of columns (1 for a column vector).

X<matsize>The library syntax is B<matsize>C<(x)>.

\subsecidx{matsnf}C<(X,{>I<flag>C< = 0})>: if C<X> is a (singular or non-singular)
...
X<matsnf0>The library syntax is B<matsnf0>C<(X,>I<flag>C<)>. Also available is \funs{smith}{X} (I<flag>C< = 0>).

\subsecidx{matsolve}C<(x,y)>: C<x> being an invertible matrix and C<y> a column
...
\subsecidx{matsolvemod}C<(m,d,y,{>I<flag>C< = 0})>: C<m> being any integral matrix,
...
are \funs{gaussmodulo}{m,d,y} (I<flag>C< = 0>)
and \funs{gaussmodulo2}{m,d,y} (I<flag>C< = 1>).

\subsecidx{matsupplement}C<(x)>: assuming that the columns of the matrix C<x>
...
\subsecidx{mattranspose}C<(x)> or C<x~>: transpose of C<x>.
This has an effect only on vectors and matrices.

X<gtrans>The library syntax is B<gtrans>C<(x)>.

\subsecidx{minpoly}C<(A,{v = x},{>I<flag>C< = 0})>: X<minimal polynomial>minimal polynomial
...
\subsecidx{qfgaussred}C<(q)>: X<decomposition into squares>decomposition into squares of the
...
\subsecidx{qfjacobi}C<(x)>: C<x> being a real symmetric matrix, this gives a
...
\subsecidx{qflll}C<(x,{>I<flag>C< = 0})>: X<LLL>LLL algorithm applied to the
I<columns> of the matrix C<x>. The columns of C<x> must be linearly
...
If I<flag>C< = 0> (default), the computations are done with floating point numbers,
...
If I<flag>C< = 1>, it is assumed that C<x> is integral. The computation is done
...
accurate than I<flag>C< = 0> above if C<x> has small dimension (say C<100> or less).

If I<flag>C< = 2>, C<x> should be an integer matrix whose columns are linearly
independent. Returns a partially reduced basis for C<x>, using an unpublished
algorithm by Peter Montgomery: a basis is said to be I<partially reduced>
if C<|v_i F<+-> v_j| E<gt>= |v_i|> for any two distinct basis vectors C<v_i, 
v_j>.

This is significantly faster than I<flag>C< = 1>, esp. when one row is huge compared
to the other rows. Note that the resulting basis is I<not> LLL-reduced in
general.
...
If I<flag>C< = 8>, same as case C<0>, but C<x> may have polynomial coefficients.

X<qflll0>The library syntax is B<qflll0>C<(x,>I<flag>C<,>I<prec>C<)>. Also available are
\funs{lll}{x,I<prec>} (I<flag>C< = 0>), \funs{lllint}{x} (I<flag>C< = 1>), and
\funs{lllkerim}{x} (I<flag>C< = 4>).

\subsecidx{qflllgram}C<(G,{>I<flag>C< = 0})>: same as C<qflll>, except that the
...
If I<flag>C< = 1>: C<G> has integer entries, still positive but not necessarily
...
I<flag>C< = 4>: C<G> has integer entries, gives the kernel and reduced image of C<x>.

I<flag>C< = 5>: same as case C<4>, but C<G> may have polynomial coefficients.

X<qflllgram0>The library syntax is B<qflllgram0>C<(G,>I<flag>C<,>I<prec>C<)>. Also available are
\funs{lllgram}{G,I<prec>} (I<flag>C< = 0>), \funs{lllgramint}{G} (I<flag>C< = 1>), and
\funs{lllgramkerim}{G} (I<flag>C< = 4>).

\subsecidx{qfminim}C<(x,{b},{m},{>I<flag>C< = 0})>: C<x> being a square and symmetric
...
If I<flag>C< = 0> (default), seeks at most C<2m> vectors. The result is a
...
If I<flag>C< = 1>, ignores C<m> and returns the first vector whose norm is less
...
If I<flag>C< = 2>, C<x> can have non integral real entries. In this case, if C<b>
...
X<qfminim0>The library syntax is B<qfminim0>C<(x,b,m,>I<flag>C<,>I<prec>C<)>, also available are \funs{minim}{x,b,m}
(I<flag>C< = 0>), \funs{minim2}{x,b,m} (I<flag>C< = 1>). In all cases, an omitted C<b>
or C<m> is coded as C<NULL>.

\subsecidx{qfperfection}C<(x)>: C<x> being a square and symmetric matrix with
...
\subsecidx{qfrep}C<(q, B, {>I<flag>C< = 0})>: C<q> being a square and symmetric
...
B<*> 1: count vectors of even norm from C<1> to C<2B>.

B<*> 2: return a C<t_VECSMALL> instead of a C<t_GEN>

X<qfrep0>The library syntax is B<qfrep0>C<(q, B, >I<flag>C<)>.

\subsecidx{qfsign}C<(x)>: signature of the quadratic form represented by the
symmetric matrix C<x>. The result is a two-component vector.

X<signat>The library syntax is B<signat>C<(x)>.

\subsecidx{setintersect}C<(x,y)>: intersection of the two sets C<x> and C<y>.

X<setintersect>The library syntax is B<setintersect>C<(x,y)>.

\subsecidx{setisset}C<(x)>: returns true (1) if C<x> is a set, false (0) if
...
\subsecidx{setminus}C<(x,y)>: difference of the two sets C<x> and C<y>,
i.e.S< >set of elements of C<x> which do not belong to C<y>.

X<setminus>The library syntax is B<setminus>C<(x,y)>.

\subsecidx{setsearch}C<(x,y,{>I<flag>C< = 0})>: searches if C<y> belongs to the set
...
\subsecidx{setunion}C<(x,y)>: union of the two sets C<x> and C<y>.

X<setunion>The library syntax is B<setunion>C<(x,y)>.

\subsecidx{trace}C<(x)>: this applies to quite general C<x>. If C<x> is not a
...
\subsecidx{vecextract}C<(x,y,{z})>: extraction of components of the
...
is as explained above.
  ...
  ? vecextract(v, "^2")       \\ mask
  %4 = [e, d, c]
  ? vecextract(v, "^2")       \\ component list
  %4 = [e, d, c]
  ? vecextract(v, "^2")       \\ interval
  %4 = [e, d, c]
  ? vecextract(v, "^2")       \\ interval + reverse order
  %4 = [e, d, c]
...
X<extract>The library syntax is B<extract>C<(x,y)> or \funs{matextract}{x,y,z}.

\subsecidx{vecsort}C<(x,{k},{>I<flag>C< = 0})>: sorts the vector C<x> in ascending
...
B<*> 4: use descending instead of ascending order.
...
\funs{sort}{x} ( = \funs{vecsort0}{x,C<NULL>,0}).

\funs{indexsort}{x} ( = \funs{vecsort0}{x,C<NULL>,1}).

\funs{lexsort}{x} ( = \funs{vecsort0}{x,C<NULL>,2}).
...
\subsecidx{vector}C<(n,{X},{>I<expr>C< = 0})>: creates a row vector (type
...
\subsecidx{vectorsmall}C<(n,{X},{>I<expr>C< = 0})>: creates a row vector of small integers (type
...
\subsecidx{vectorv}C<(n,X,>I<expr>C<)>: as X<vector>C<vector>, but returns a
...
words asymptotic to C<C(x-a)^{->F<alpha>C<}> for some F<alpha> such that
F<alpha>C< E<gt> -1> when C<x> is close to C<a>), or to correspond to simple
...
\subsecidx{intcirc}C<(X = a,R,>I<expr>C<, {>I<tab>C<})>: numerical
integration of I<expr> with respect to C<X> on the circle C<|X-a |= R>,
divided by C<2i>F<pi>. In other words, when I<expr> is a meromorphic
...
\subsecidx{intfouriercos}C<(X = a,b,z,>I<expr>C<,{>I<tab>C<})>: numerical
integration of I<expr>C<(X)> F<cos> C<(2>F<pi>C< zX)> from C<a> to C<b>, in other words
...
at infinity, but if they are, oscillations (i.e. C<[[F<+->1],>F<alpha>C< I]>) are
forbidden.

X<intfouriercos>The library syntax is B<intfouriercos>C<(void *E, GEN (*eval)(GEN,void*), GEN a, GEN b, GEN z, GEN tab, long prec)>.

\subsecidx{intfourierexp}C<(X = a,b,z,>I<expr>C<,{>I<tab>C<})>: numerical
integration of I<expr>C<(X)> F<exp> C<(-2>F<pi>C< zX)> from C<a> to C<b>, in other words
...
C<[[F<+->1],>F<alpha>C< I]>) are forbidden.

X<intfourierexp>The library syntax is B<intfourierexp>C<(void *E, GEN (*eval)(GEN,void*), GEN a, GEN b, GEN z, GEN tab, long prec)>.

\subsecidx{intfouriersin}C<(X = a,b,z,>I<expr>C<,{>I<tab>C<})>: numerical
integration of I<expr>C<(X)> F<sin> C<(2>F<pi>C< zX)> from C<a> to C<b>, in other words
...
\subsecidx{intfuncinit}C<(X = a,b,>I<expr>C<,{>I<flag>C< = 0},{m = 0})>:
initalize tables for use with integral transforms such as C<intmellininv>,
etc., where C<a> and C<b> are coded as in C<intnum>, I<expr> is the
...
I<flag> is nonzero, assumes that C<s(-X) = \overline{s(X)}>, which makes the
...
Note that the order of C<m> and I<flag> are reversed compared to the C<GP>
syntax.

\subsecidx{intlaplaceinv}C<(X = sig,z,>I<expr>C<,{>I<tab>C<})>:
numerical integration of I<expr>C<(X)e^{Xz}> with respect to C<X> on the line
 F<Re> C<(X) = sig>, divided by C<2i>F<pi>, in other words, inverse Laplace transform
of the function corresponding to I<expr> at the value C<z>.

C<sig> is coded as follows. Either it is a real number F<sigma>, equal to the
...
C<F<+-> oo >. Or it is a two component vector C<[>F<sigma>C<,>F<alpha>C<]>, where
F<sigma> is as before, and either F<alpha>C< = 0> for slowly decreasing functions,
or F<alpha>C< E<gt> 0> for functions decreasing like  F<exp> C<(->F<alpha>C< t)>. Note that it
is not necessary to choose the exact value of F<alpha>. I<tab> is as in
C<intnum>.
...
F<sigma>. For example:
  ...
  time = 330 ms.
  %7 = 1.07... E-72 + 3.2... E-72*I \\ not so good
  ? intlaplaceinv(x=100, 1, 1/x) - 1
  time = 330 ms.
  %7 = 1.07... E-72 + 3.2... E-72*I \\ better
  ? intlaplaceinv(x=100, 1, 1/x) - 1
  time = 330 ms.
  %7 = 1.07... E-72 + 3.2... E-72*I \\ perfect but slow.
  ? intlaplaceinv(x=100, 1, 1/x) - 1
  time = 330 ms.
  %7 = 1.07... E-72 + 3.2... E-72*I \\ better than %1
  ? intlaplaceinv(x=100, 1, 1/x) - 1
  time = 330 ms.
  %7 = 1.07... E-72 + 3.2... E-72*I \\ perfect, fast.
  ? intlaplaceinv(x=100, 1, 1/x) - 1
  time = 330 ms.
  %7 = 1.07... E-72 + 3.2... E-72*I \\ perfect, fastest, but why sig = 10?
  ? intlaplaceinv(x=100, 1, 1/x) - 1
  time = 330 ms.
...
\subsecidx{intmellininv}C<(X = sig,z,>I<expr>C<,{>I<tab>C<})>: numerical
integration of I<expr>C<(X)z^{-X}> with respect to C<X> on the line
 F<Re> C<(X) = sig>, divided by C<2i>F<pi>, in other words, inverse Mellin transform of
the function corresponding to I<expr> at the value C<z>.

C<sig> is coded as follows. Either it is a real number F<sigma>, equal to the
abcissa of integration, and then the function to be integrated is assumed to
decrease exponentially fast, of the order of  F<exp> C<(-t)> when the imaginary
part of the variable tends to C<F<+-> oo >. Or it is a two component vector
C<[>F<sigma>C<,>F<alpha>C<]>, where F<sigma> is as before, and either F<alpha>C< = 0> for
slowly decreasing functions, or F<alpha>C< E<gt> 0> for functions decreasing like
 F<exp> C<(->F<alpha>C< t)>, such as gamma products. Note that it is not necessary to
choose the exact value of F<alpha>, and that F<alpha>C< = 1> (equivalent to C<sig>
...
\subsecidx{intmellininvshort}C<(sig,z,tab)>: numerical integration
of C<s(X)z^{-X}> with respect to C<X> on the line  F<Re> C<(X) = sig>, divided by
C<2i>F<pi>, in other words, inverse Mellin transform of C<s(X)> at the value C<z>.
...
F<Gamma>C<(s)^3> given in C<intmellininv>:

  ? tab2 = intfuncinit(t=-oo, oo, gamma(2+I*t)^3, 1);
  ? intmellininvshort(2,4, tab2)
  %6 = -1.2...E-42 - 3.2...E-109*I  \\ for clarity
  ? tab2 = intfuncinit(t=-oo, oo, gamma(2+I*t)^3, 1);
  ? intmellininvshort(2,4, tab2)
  %6 = -1.2...E-42 - 3.2...E-109*I  \\ not too fast because of F<Gamma>(s)^3.
  ? tab2 = intfuncinit(t=-oo, oo, gamma(2+I*t)^3, 1);
  ? intmellininvshort(2,4, tab2)
  %6 = -1.2...E-42 - 3.2...E-109*I  \\ function of real type, decreasing as  F<exp> (-3F<pi>/2.|t|)
  ? tab2 = intfuncinit(t=-oo, oo, gamma(2+I*t)^3, 1);
  ? intmellininvshort(2,4, tab2)
  %6 = -1.2...E-42 - 3.2...E-109*I  \\ 50 times faster than C<A> and perfect.
  ? tab2 = intfuncinit(t=-oo, oo, gamma(2+I*t)^3, 1);
  ? intmellininvshort(2,4, tab2)
  %6 = -1.2...E-42 - 3.2...E-109*I  \\ 63 digits lost

In the computation of I<tab>, it was not essential to include the
I<exact> exponential decrease of F<Gamma>C<(2+it)^3>. But as the last
example shows, a rough indication I<must> be given, otherwise slow
...
\subsecidx{intnum}C<(X = a,b,>I<expr>C<,{>I<tab>C<})>: numerical integration
...
C<[a,>F<alpha>C<]>, where the function is assumed to have a singularity of the
form C<(x-a)^{>F<alpha>C<+>F<epsilon>C<}> at C<a>, where F<epsilon> indicates that powers
of logarithms are neglected. In particular, C<[a,>F<alpha>C<]> with F<alpha>C< E<gt>= 0>
...
the number of digits will be correct if F<alpha>C< = -1/2> is omitted.

The endpoints of integration can be C<F<+-> oo >, which is coded as
C<[F<+-> 1]> or as C<[[F<+->1],>F<alpha>C<]>. Here F<alpha> codes the behaviour of the
function at C<F<+-> oo > as follows.

B<*> F<alpha>C< = 0> (or no F<alpha> at all, i.e. simply C<[F<+->1]>) assumes that the
function to be integrated tends to zero, but not exponentially fast, and not
oscillating such as  F<sin> C<(x)/x>.

B<*> F<alpha>C< E<gt> 0> assumes that the function tends to zero exponentially fast
approximately as  F<exp> C<(->F<alpha>C< x)>, including reasonably oscillating
functions such as  F<exp> C<(-x)> F<sin> C<(x)>. The precise choice of F<alpha>, while
useful in extreme cases, is not critical, and may be off by a I<factor>
of C<10> or more from the correct value.

B<*> F<alpha>C< E<lt> -1> assumes that the function tends to C<0> slowly, like
C<x^{>F<alpha>C<}>. Here it is essential to give the correct F<alpha>, if possible,
but on the other hand F<alpha>C< E<lt>= -2> is equivalent to F<alpha>C< = 0>, in other
words to no F<alpha> at all.
...
B<*> F<alpha>C< = k I> assumes that the function behaves like  F<cos> C<(kx)g(x)>.

B<*> F<alpha>C< = -kI> assumes that the function behaves like  F<sin> C<(kx)g(x)>.
...
Otherwise you will get nonsense. Note that  F<cos> C<(kx)> (and similarly
 F<sin> C<(kx)>) means that very function, and not a translated version such as
 F<cos> C<(kx+a)>.

If for instance C<f(x) = > F<cos> C<(kx)g(x)> where C<g(x)> tends to zero exponentially
fast as  F<exp> C<(->F<alpha>C< x)>, it is up to the user to choose between
C<[[F<+->1],>F<alpha>C<]> and C<[[F<+->1],kI]>, but a good rule of thumb is that if the
oscillations are much weaker than the exponential decrease, choose
C<[[F<+->1],>F<alpha>C<]>, otherwise choose C<[[F<+->1],kI]>, although the latter can
...
\misctitle{Note.} If you do not like the code C<[F<+->1]> for C<F<+-> oo >, you
...
/(> F<exp> C<(x)-1) - > F<exp> C<(-x)/x>, then F<int>C<_0^ oo f(x)dx = >F<gamma>, Euler's
...
A better solution is
  ...
  %2 = 0.E-106 \\ expansion around t = 0
  ? intnum(x = 0, [oo,1],  g(x)) - Euler
  %2 = 0.E-106 \\ note that 6.18 > 105
  ? intnum(x = 0, [oo,1],  g(x)) - Euler
  %2 = 0.E-106 \\ perfect
...
worse. For instance
  ...
  %2 = 0.E-105 \\ only 59 correct decimals
...
C<(1/2)>F<int>C<_0^ oo > F<sin> C<(x)/xdx>. We finish with some more complicated
examples:
  ...
  %6 = 5.45... E-107 \\ OK
  ? intnum(x = 0, oo, sin(x)^3*exp(-x), tab) - 0.3
  %6 = 5.45... E-107 \\ lost 16 decimals. Try higher m:
  ? intnum(x = 0, oo, sin(x)^3*exp(-x), tab) - 0.3
  %6 = 5.45... E-107 \\ the value of m actually used above.
  ? intnum(x = 0, oo, sin(x)^3*exp(-x), tab) - 0.3
  %6 = 5.45... E-107 \\ try m one higher.
  ? intnum(x = 0, oo, sin(x)^3*exp(-x), tab) - 0.3
...
S<  > F<int>C<_0^ oo > F<sin> C<(x)/x^sdx = > F<cos> C<(>F<pi>C< s/2) >F<Gamma>C<(1-s) , >

a priori valid only for C<0 E<lt> > F<Re> C<(s) E<lt> 2>, but the right hand side provides an
...
I<internal> integrations at least!
...
speed by trial and error. Continuing the above example:
  ...
  ? test(m - 3)
  time = 120 ms.
  %3 = -7.23... E-60 \\ what value of m did it take ?
  ? test(m - 3)
  time = 120 ms.
  %3 = -7.23... E-60 \\ 4 = 2^2 times faster and still OK.
  ? test(m - 3)
  time = 120 ms.
  %3 = -7.23... E-60 \\ 16 = 2^4 times faster and still OK.
  ? test(m - 3)
  time = 120 ms.
...
\subsecidx{intnuminit}C<(a,b,{m = 0})>: initialize tables for integration from
...
Let C<x = >F<phi>C<(t)> be the change of variable which is used. I<tab>[1] contains
...
corresponding to C<t = 0> (F<phi>C<(0)> and F<phi>C<'(0)>). I<tab>[4] and
I<tab>[5] contain the abcissas and weights corresponding to positive
C<t = nh> for C<1 E<lt>= n E<lt>= N> and C<h = 1/2^m> (F<phi>C<(nh)> and F<phi>C<'(nh)>). Finally
...
not always) if F<phi>C<(t)> is an odd function (implicitly we would have
I<tab>C<[6] = ->I<tab>C<[4]> and I<tab>C<[7] = >I<tab>C<[5]>).

X<intnuminit>The library syntax is B<intnuminit>C<(GEN a, GEN b, long m, long prec)>.

\subsecidx{intnumromb}C<(X = a,b,>I<expr>C<,{>I<flag>C< = 0})>: numerical integration of
...
\subsecidx{intnumstep}C<()>: give the value of C<m> used in all the
...
\subsecidx{prod}C<(X = a,b,>I<expr>C<,{x = 1})>: product of expression
...
\subsecidx{prodeuler}C<(X = a,b,>I<expr>C<)>: product of expression I<expr>,
...
\subsecidx{prodinf}C<(X = a,>I<expr>C<,{>I<flag>C< = 0})>: X<infinite product>infinite product of
...
(I<flag>C< = 0>), or X<prodinf1>B<prodinf1> with the same arguments (I<flag>C< = 1>).

\subsecidx{solve}C<(X = a,b,>I<expr>C<)>: find a real root of expression
...
\subsecidx{sum}C<(X = a,b,>I<expr>C<,{x = 0})>: sum of expression I<expr>,
...
\subsecidx{sumalt}C<(X = a,>I<expr>C<,{>I<flag>C< = 0})>: numerical summation of the
...
available is X<sumalt2>C<sumalt2> with the same arguments (I<flag>C< = 1>).

\subsecidx{sumdiv}C<(n,X,>I<expr>C<)>: sum of expression I<expr> over
...
, there is no way to indicate that I<expr> is multiplicative in
...
\subsecidx{suminf}C<(X = a,>I<expr>C<)>: X<infinite sum>infinite sum of expression
...
\subsecidx{sumnum}C<(X = a,sig,>I<expr>C<,{>I<tab>C<}),{>I<flag>C< = 0}>: numerical
...
C<f(X)> for  F<Re> C<(X) E<gt>= >F<sigma>.

The parameter F<sigma>C<\in>B<R> is coded in the argument C<sig> as follows: it
is either

B<*> a real number F<sigma>. Then the function C<f> is assumed to
decrease at least as C<1/X^2> at infinity, but not exponentially;

B<*> a two-component vector C<[>F<sigma>C<,>F<alpha>C<]>, where F<sigma> is as
before, F<alpha>C< E<lt> -1>. The function C<f> is assumed to decrease like
C<X^{>F<alpha>C<}>. In particular, F<alpha>C< E<lt>= -2> is equivalent to no F<alpha> at all.

B<*> a two-component vector C<[>F<sigma>C<,>F<alpha>C<]>, where F<sigma> is as
before, F<alpha>C< E<gt> 0>. The function C<f> is assumed to decrease like
 F<exp> C<(->F<alpha>C< X)>. In this case it is essential that F<alpha> be exactly the
...
The function uses the X<intnum>C<intnum> routines and integration on the line
 F<Re> C<(s) = >F<sigma>. The optional argument I<tab> is as in intnum, except it
...
Finally, if I<flag> is nonzero, we assume that the function C<f> to be summed is
of real type, i.e. satisfies C<\overline{f(z)} = f(\overline{z})>, which
speeds up the computation.

  ? d - c
  time = 0 ms.
  %5 = 1.97... E-306 \\ slower but done once and for all.
  ? d - c
  time = 0 ms.
  %5 = 1.97... E-306 \\ 3 times as fast as C<sumpos>
  ? d - c
  time = 0 ms.
  %5 = 1.97... E-306 \\ perfect.
  ? d - c
  time = 0 ms.
  %5 = 1.97... E-306 \\ function of real type
  ? d - c
  time = 0 ms.
  %5 = 1.97... E-306 \\ twice as fast, no imaginary part.
  ? d - c
  time = 0 ms.
  %5 = 1.97... E-306 \\ fast
  ? d - c
...
For slowly decreasing function, we must indicate singularities:
  ...
  time = 12,210 ms.
  ? b - zeta(4/3)
  %3 = 1.05... E-300 \\ slow because of the computation of n^{-4/3}.
  time = 12,210 ms.
  ? b - zeta(4/3)
  %3 = 1.05... E-300 \\ lost 200 decimals because of singularity at  oo 
  time = 12,210 ms.
  ? b - zeta(4/3)
  %3 = 1.05... E-300 \\ of real type
  time = 12,210 ms.
  ? b - zeta(4/3)
  %3 = 1.05... E-300 \\ better

Since the I<complex> values of the function are used, beware of
determination problems. For instance:

  ? sumnum(n=1,[2,-3/2], 1/n^(3/2), tab,1) - zeta(3/2)
  time = 8,990 ms.
  %3 = -1.19... E-305 \\ fast and correct
  ? sumnum(n=1,[2,-3/2], 1/n^(3/2), tab,1) - zeta(3/2)
  time = 8,990 ms.
  %3 = -1.19... E-305 \\ nonsense. However
  ? sumnum(n=1,[2,-3/2], 1/n^(3/2), tab,1) - zeta(3/2)
  time = 8,990 ms.
...
indicate the exact rate of decrease and increase C<m> by C<1> or C<2>:
  ...
  time = 11,770 ms.
  %5 = -1.9... E-305 \\ fast and perfect
  ? m = intnumstep()
  %4 = 9
  ? sumnum(n=1,[2,log(2)], 2^(-n), m+1, 1) - 1
  time = 11,770 ms.
  %5 = -1.9... E-305 \\ also fast and perfect
  ? m = intnumstep()
  %4 = 9
  ? sumnum(n=1,[2,log(2)], 2^(-n), m+1, 1) - 1
  time = 11,770 ms.
  %5 = -1.9... E-305 \\ nonsense
  ? m = intnumstep()
  %4 = 9
  ? sumnum(n=1,[2,log(2)], 2^(-n), m+1, 1) - 1
  time = 11,770 ms.
  %5 = -1.9... E-305 \\ of real type
  ? m = intnumstep()
  %4 = 9
  ? sumnum(n=1,[2,log(2)], 2^(-n), m+1, 1) - 1
  time = 11,770 ms.
  %5 = -1.9... E-305 \\ slow and lost 70 decimals
  ? m = intnumstep()
  %4 = 9
  ? sumnum(n=1,[2,log(2)], 2^(-n), m+1, 1) - 1
  time = 11,770 ms.
...
\subsecidx{sumnumalt}C<(X = a,sig,>I<expr>C<,{>I<tab>C<},{>I<flag>C< = 0})>: numerical
summation of C<(-1)^X>I<expr>C<(X)>, the variable C<X> taking integer values from
ceiling of C<a> to C<+ oo >, where I<expr> is assumed to be a holomorphic
function for  F<Re> C<(X) E<gt>= sig> (or C<sig[1]>).
...
B<Warning2..> The expression I<expr> must I<not> include the
...
(where C<sig> is coded as C<[>F<sigma>C<,>F<alpha>C<]> with F<alpha>C< E<lt> -1>), it is not
really important to indicate F<alpha>. In fact, as for C<sumalt>, the
...
C<sumnuminit>. If I<flag> is nonzero, assumes that the function C<f> to be
summed is of real type, i.e. satisfies C<\overline{f(z)} = f(\overline{z})>, and
then twice faster when I<tab> is precomputed.

  ? a - b
  time = 0 ms.
  %1 = -1.66... E-308 \\ abcissa F<sigma> = 2, alternating sums.
  ? a - b
  time = 0 ms.
  %1 = -1.66... E-308 \\ slow, but done once and for all.
  ? a - b
  time = 0 ms.
  %1 = -1.66... E-308 \\ similar speed to C<sumnum>
  ? a - b
  time = 0 ms.
  %1 = -1.66... E-308 \\ infinitely faster!
  ? a - b
  time = 0 ms.
...
\subsecidx{sumnuminit}C<(sig,{m = 0},{sgn = 1})>: initialize tables for numerical
summation using C<sumnum> (with I<sgn>C< = 1>) or C<sumnumalt> (with
I<sgn>C< = -1>), C<sig> is the abcissa of integration coded as in C<sumnum>,
and C<m> is as in C<intnuminit>.

X<sumnuminit>The library syntax is B<sumnuminit>C<(GEN sig, long m, long sgn, long prec)>.

\subsecidx{sumpos}C<(X = a,>I<expr>C<,{>I<flag>C< = 0})>: numerical summation of the
...
available is X<sumpos2>C<sumpos2> with the same arguments (I<flag>C< = 1>).
...
Note that in the present version  all plotting functions (both low
...
I<None of the graphic functions are available
within the PARI library, you must be under >C<gp>I< to use them>. The reason
...
\subsecidx{plot}C<(X = a,b,>I<expr>C<,{>I<Ymin>C<},{>I<Ymax>C<})>: crude
...
\subsecidx{plotbox}C<(w,x2,y2)>: let C<(x1,y1)> be the current position of the
...
\subsecidx{plotclip}C<(w)>: `clips' the content of rectwindow C<w>, i.e
...
\subsecidx{plotcolor}C<(w,c)>: set default color to C<c> in rectwindow C<w>.
In present version , this is only implemented for the X11 window system,
...
\subsecidx{plotcopy}C<(w1,w2,dx,dy)>: copy the contents of rectwindow
C<w1> to rectwindow C<w2>, with offset C<(dx,dy)>.

\subsecidx{plotcursor}C<(w)>: give as a 2-component vector the current
(scaled) position of the virtual cursor corresponding to the rectwindow C<w>.

\subsecidx{plotdraw}C<(list)>: physically draw the rectwindows given in C<list>
...
\subsecidx{ploth}C<(X = a,b,>I<expr>C<,{>I<flag>C< = 0},{n = 0})>: high precision
...
B<*> C<1 = Parametric>: X<parametric plot>I<parametric plot>. Here I<expr> must
...
B<*> C<2 = Recursive>: X<recursive plot>I<recursive plot>. If this flag is set,
only I<one> curve can be drawn at a time, i.e.S< >I<expr> must be either a
two-component vector (for a single parametric curve, and the parametric flag
I<has> to be set), or a scalar function. The idea is to choose pairs of
...
B<*> C<4 = no_Rescale>: do not rescale plot according to the
...
B<*> C<8 = no_X_axis>: do not print the C<x>-axis.

B<*> C<16 = no_Y_axis>: do not print the C<y>-axis.

B<*> C<32 = no_Frame>: do not print frame.

B<*> C<64 = no_Lines>: only plot reference points, do not join them.

B<*> C<128 = Points_too>: plot both lines and points.

B<*> C<256 = Splines>: use splines to interpolate the points.

B<*> C<512 = no_X_ticks>: plot no C<x>-ticks.

B<*> C<1024 = no_Y_ticks>: plot no C<y>-ticks.

B<*> C<2048 = Same_ticks>: plot all ticks with the same length.

\subsecidx{plothraw}C<(>I<listx>C<,>I<listy>C<,{>I<flag>C< = 0})>: given
...
\subsecidx{plothsizes}C<()>: return data corresponding to the output window
...
\subsecidx{plotinit}C<(w,x,y,{>I<flag>C<})>: initialize the rectwindow C<w>,
...
C<y>. If I<flag>C< = 0>, C<x> and C<y> represent pixel units. Otherwise, C<x> and C<y>
...
\subsecidx{plotkill}C<(w)>: erase rectwindow C<w> and free the corresponding
...
\subsecidx{plotlines}C<(w,X,Y,{>I<flag>C< = 0})>: draw on the rectwindow C<w>
...
\subsecidx{plotlinetype}C<(w,>I<type>C<)>: change the type of lines
...
\subsecidx{plotmove}C<(w,x,y)>: move the virtual cursor of the rectwindow C<w>
to position C<(x,y)>.

\subsecidx{plotpoints}C<(w,X,Y)>: draw on the rectwindow C<w> the
...
\subsecidx{plotpointsize}C<(w,size)>: changes the ``size'' of following
points in rectwindow C<w>. If C<w = -1>, change it in all rectwindows.
This only works in the C<gnuplot> interface.

\subsecidx{plotpointtype}C<(w,>I<type>C<)>:  change the type of
...
\subsecidx{plotrbox}C<(w,dx,dy)>: draw in the rectwindow C<w> the outline of
...
\subsecidx{plotrecth}C<(w,X = a,b,>I<expr>C<,{>I<flag>C< = 0},{n = 0})>: writes to
rectwindow C<w> the curve output of C<ploth>C<(w,X = a,b,>I<expr>C<,>I<flag>C<,n)>.

\subsecidx{plotrecthraw}C<(w,>I<data>C<,{>I<flag>C< = 0})>: plot graph(s) for
...
\subsecidx{plotrline}C<(w,dx,dy)>: draw in the rectwindow C<w> the part of the
...
\subsecidx{plotrmove}C<(w,dx,dy)>: move the virtual cursor of the rectwindow
...
\subsecidx{plotrpoint}C<(w,dx,dy)>: draw the point C<(x1+dx,y1+dy)> on the
...
\subsecidx{plotscale}C<(w,x1,x2,y1,y2)>: scale the local coordinates of the
...
\subsecidx{plotstring}C<(w,x,{>I<flag>C< = 0})>: draw on the rectwindow C<w> the
...
\subsecidx{psdraw}C<(>I<list>C<)>: same as C<plotdraw>, except that the
output is a PostScript program appended to the C<psfile>.

\subsecidx{psploth}C<(X = a,b,>I<expr>C<)>: same as C<ploth>, except that the
output is a PostScript program appended to the C<psfile>.

\subsecidx{psplothraw}C<(>I<listx>C<,>I<listy>C<)>: same as C<plothraw>,
...
\subsubsecidx{break}C<({n = 1})>: interrupts execution of current I<seq>, and
...
\subsubsecidx{for}C<(X = a,b,>I<seq>C<)>: evaluates I<seq>, where 
the formal variable C<X> goes from C<a> to C<b>. Nothing is done if C<a E<gt> b>.
C<a> and C<b> must be in B<R>.

\subsubsecidx{fordiv}C<(n,X,>I<seq>C<)>: evaluates I<seq>, where
...
\subsubsecidx{forell}C<(E,a,b,>I<seq>C<)>: evaluates I<seq>, where 
...
\subsubsecidx{forprime}C<(X = a,b,>I<seq>C<)>: evaluates I<seq>,
...
\subsubsecidx{forstep}C<(X = a,b,s,>I<seq>C<)>: evaluates I<seq>,
...
\subsubsecidx{forsubgroup}C<(H = G,{B},>I<seq>C<)>: evaluates I<seq> for
...
>B<Z>C</2>B<Z>C< g_1  x >B<Z>C</2>B<Z>C< g_2>:
...
\subsubsecidx{forvec}C<(X = v,>I<seq>C<,{>I<flag>C< = 0})>: Let C<v> be an C<n>-component
...
moves the fastest.) If I<flag>C< = 1>, generate only nondecreasing vectors C<X>, and
if I<flag>C< = 2>, generate only strictly increasing vectors C<X>.

\subsubsecidx{if}C<(a,{>I<seq1>C<},{>I<seq2>C<})>:
...
C<if (a,>I<seq>C<)> evaluates I<seq> if C<a> is not equal to zero
(you don't have to write the second comma), and does nothing otherwise,

C<if (a,,>I<seq>C<)> evaluates I<seq> if C<a> is equal to zero, and
does nothing otherwise. You could get the same result using the C<!>
(C<not>) operator: C<if (!a,>I<seq>C<)>.
...
\subsubsecidx{next}C<({n = 1})>: interrupts execution of current C<seq>,
...
\subsubsecidx{return}C<({x = 0})>: returns from current subroutine, with
result C<x>. If C<x> is omitted, return the C<(void)> value (return no
result, like C<print>).

\subsubsecidx{until}C<(a,>I<seq>C<)>: evaluates I<seq> until C<a> is not
...
I<after> execution of the I<seq>, not before as in C<while>).

\subsubsecidx{while}C<(a,>I<seq>C<)>: while C<a> is non-zero, evaluates the
...
\subsubsecidx{addhelp}C<(S,>I<str>C<)>:X<Label se:addhelp> changes the help
...
\subsubsecidx{alias}C<(>I<newkey>C<,>I<key>C<)>: defines the keyword
...
\subsubsecidx{allocatemem}C<({x = 0})>: this is a very special operation which
...
\subsubsecidx{default}C<({>I<key>C<},{>I<val>C<})>: returns
...
\subsubsecidx{error}C<({>I<str>C<}*)>: outputs its argument list (each of
...
\subsubsecidxunix{extern}C<(>I<str>C<)>: the string I<str> is the name
...
\subsubsecidx{getheap}C<()>: returns a two-component row vector giving the
...
\subsubsecidx{getrand}C<()>: returns the current value of the random number
seed. Useful mainly for debugging purposes.

X<getrand>The library syntax is B<getrand>C<()>, returns a C long.

\subsubsecidx{getstack}C<()>: returns the current value of
...
\subsubsecidx{gettime}C<()>: returns the time (in milliseconds) elapsed since
...
\subsubsecidx{global}C<(>I<list of variables>C<)>: X<Label se:global>
...
\subsubsecidx{input}C<()>: reads a string, interpreted as a GP expression,
...
\subsubsecidxunix{install}C<(>I<name>C<,>I<code>C<,{>I<gpname>C<},{>I<lib>C<})>:
...
\subsubsecidx{kill}C<(s)>:X<Label se:kill> kills the present value of the
...
\subsubsecidx{print}C<({>I<str>C<}*)>: outputs its (string) arguments in raw
format, ending with a newline.

\subsubsecidx{print1}C<({>I<str>C<}*)>: outputs its (string) arguments in raw
format, without ending with a newline (note that you can still embed newlines
within your strings, using the C<\n> notationS< >!).

\subsubsecidx{printp}C<({>I<str>C<}*)>: outputs its (string) arguments in
prettyprint (beautified) format, ending with a newline.

\subsubsecidx{printp1}C<({>I<str>C<}*)>: outputs its (string) arguments in
prettyprint (beautified) format, without ending with a newline.

\subsubsecidx{printtex}C<({>I<str>C<}*)>: outputs its (string) arguments in
...
\subsubsecidx{quit}C<()>: exits C<gp>.X<Label se:quit>

\subsubsecidx{read}C<({>I<filename>C<})>: reads in the file
...
\subsubsecidx{readvec}C<({>I<str>C<})>:  reads in the file
I<filename> (subject to string expansion). If I<filename> is
omitted, re-reads the last file that was fed into C<gp>. The return
...
\subsubsecidx{reorder}C<({x = []})>: C<x> must be a vector. If C<x> is the
...
\subsubsecidx{setrand}C<(n)>: reseeds the random number generator to the value
C<n>. The initial seed is C<n = 1>.

X<setrand>The library syntax is B<setrand>C<(n)>, where C<n> is a C<long>. Returns C<n>.

\subsubsecidxunix{system}C<(>I<str>C<)>: I<str> is a string representing
...
\subsubsecidx{trap}C<({e}, {>I<rec>C<}, {>I<seq>C<})>: tries to
...
individual exceptions. In the current version , the following keywords
...
\subsubsecidx{type}C<(x)>: this is useful only under C<gp>. Returns the
...
X<type0>The library syntax is B<type0>C<(>I<x>C<)>, though the macro C<typ> is usually simpler to use
...
\subsubsecidx{version}C<()>: Returns the current version number as a C<t_VEC>
...
\subsubsecidx{whatnow}C<(>I<key>C<)>: if keyword I<key> is the name
...
\subsubsecidx{writetex}C<(>I<filename>C<,{>I<str>C<}*)>: as C<write>,
