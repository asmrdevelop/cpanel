...
sub import {
    ...
    for (@_) {
        ...
        eval { $path = lib::_nativize($path); };    # we eval since older lib.pm's don't have this

        if ( $ENV{'lib::restrict-!-d_ok_in'} ) {
            require Carp;
            Carp::carp('$ENV{lib::restrict-!-d_ok_in} is deprecated use $lib::restrict::d_ok_in');
        }

        if ( !-d $path && $d_ok_in && ref $d_ok_in eq 'ARRAY' ) {
            my $ok = 0;
            require File::Spec;
