use strict;
use warnings;
package Email::Sender::Util 2.500;
...
sub _recipients_from_email {
    ...
    my @to = List::Util::uniq(
        ...
        map { $email->get_header($_) } qw(to cc bcc)
    );
...
sub _sender_from_email {
    ...
    my ($sender) = map { $_->address }
      map { Email::Address::XS->parse($_) } scalar $email->get_header('from');
...
sub _failure {
    ...
    if ($smtp) {
        ...
        $message =
            !defined $message ? "(no SMTP error message)"
