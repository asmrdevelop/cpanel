...
classes directly; instead, use Endpoint’s C<create_message()> method.
...
L<free now besides|https://letsencrypt.org>, and it will randomize the data
stream anyway. So, nowadays we just use Perl’s C<rand()> built-in.
...
See L<Net::WebSocket::PMCE::deflate> for details.
