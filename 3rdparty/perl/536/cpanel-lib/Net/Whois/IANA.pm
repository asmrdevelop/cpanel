...
sub ripe_read_query ($$) {
    ...
    my %query = ( fullinfo => '' );
    syswrite($sock,"-r $ip\n");
    my ($ok,$data) = _nonblocking_slurp_from_socket($sock);
    return () if !$ok;
    foreach (split(m{\n}, $data)) {
        $query{fullinfo} .= $_;
...
sub _nonblocking_slurp_from_socket {
    my ($sock)     = @_;
    my $start_time   = time();
    my $timeout_time = $start_time + $READ_TIMEOUT;
    my $reached_eof  = 0;
    my $remaining;
    my $buffer = '';
    my $select;
    ...
    $sock->blocking(0);
    while ( !$reached_eof ) {
        my $ret = sysread( $sock, $buffer, $READ_SIZE, length $buffer );
        if ( !defined $ret || $ret == -1 ) {
            if ( $! == Errno::EAGAIN() || $! == Errno::EINTR() || $! == Errno::EWOULDBLOCK() ) {
                if ( ( $remaining = $timeout_time - time() ) > 0 ) {
