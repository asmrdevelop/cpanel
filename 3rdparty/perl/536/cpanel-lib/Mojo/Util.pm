...
sub decode {
  ...
  if ($encoding eq 'UTF-8')  {
    return undef if !utf8::decode($bytes);
    return $bytes;
  }
...
sub extract_usage {
  ...
  Pod::Usage::pod2usage(-exitval => 'noexit', -input => $file, -output => $handle);
  $output =~ s/^.*\n|\n$//;
  $output =~ s/\n$//;
...
sub getopt {
  ...
  my $result = Getopt::Long::GetOptionsFromArray($array, @_);
...
## This sub was modified for the perl compiler
## see https://jira.cpanel.net/browse/CPANEL-30160
## https://stackoverflow.com/q/58677042/124486
sub monkey_patch {
  my ($class, %patch) = @_;
  no strict 'refs';
...
sub _encoding { require Encode; $ENCODING{$_[0]} //= Encode::find_encoding($_[0]) // croak "Unknown encoding '$_[0]'" }
