...
sub basename { require File::Basename; File::Basename::basename(${shift()}, @_) }
...
sub copy_to {
  my ($self, $to) = @_;
  require File::Copy;
  File::Copy::copy($$self, $to) or croak qq{Can't copy file "$$self" to "$to": $!};
...
sub dirname { require File::Basename; $_[0]->new(scalar File::Basename::dirname(${$_[0]})) }
...
sub list_tree {
  ...
  File::Find::find( {wanted => $wanted, no_chdir => 1}, $$self ) if -d $$self;
  delete $all{$$self};

  return Mojo::Collection->new(map { $self->new(canonpath $_) } sort keys %all);
}

sub lstat { require File::stat; File::stat::lstat(${shift()}) }
...
sub move_to {
  my ($self, $to) = @_;
  require File::Copy;
  File::Copy::move($$self, $to) or croak qq{Can't move file "$$self" to "$to": $!};
  require File::Basename;
  return $self->new(-d $to ? ($to, File::Basename::basename($self)) : $to);
}
...
sub stat { require File::stat; File::stat::stat(${shift()}) }

sub tap { shift->Mojo::Base::tap(@_) }

sub tempdir { require File::Temp; __PACKAGE__->new(File::Temp->newdir(@_)) }

sub tempfile { require File::Temp; __PACKAGE__->new(File::Temp->new(@_)) }
