...

{
    ...
    use constant SSL_WANT_READ  => Scalar::Util::dualvar( \$x, 'SSL wants a read first' );
    my $y = $Net_SSLeay_ERROR_WANT_WRITE;
    use constant SSL_WANT_WRITE => Scalar::Util::dualvar( \$y, 'SSL wants a write first' );
...
sub connect_SSL {
	...
	Scalar::Util::weaken($SSL_OBJECT{$ssl}[0]);
...

{
	...
	if ( CAN_IPV6 and $identity =~m{:} ) {
	    ...
	    $ipn = Socket::inet_pton(AF_INET6,$identity) or return; # invalid name
        ...
        $identity = IO::Socket::SSL::PublicSuffix::idn_to_ascii($identity)
...
sub _internal_error {
    ...
    $error = Scalar::Util::dualvar( -1, $error );
