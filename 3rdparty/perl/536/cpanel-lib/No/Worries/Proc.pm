...
sub setsid {
   ...
   require POSIX;
   my $setsid = POSIX->can('setsid');

   if ( !$setsid ) {
      require Cpanel::Sys::Setsid::Fast;
      $setsid = Cpanel::Sys::Setsid::Fast->can('fast_setsid');
   }

   return -1 unless $setsid;
...
Use POSIX::setsid when available otherwise fallback to Cpanel::Sys::Setsid::Fast
