	...
	******/
	...
	class Compressor{
		...
		function check_gzip_use()
		{
			...
			if( isset($_SERVER['HTTP_USER_AGENT']) && preg_match("/(?:msie )([0-9.]+)/i", $_SERVER['HTTP_USER_AGENT'], $ie))
			{
				if($ie[1]<7)
		...
		function load_files()
		{
			...
			$loader = preg_replace_callback("/(t\.scripts_to_load=\s*)\[([^\]]*)\];/",
						function($matches) {
							return $this->replace_scripts( 'script_list', $matches[1], $matches[2]);
						},
						$loader
			);
			...
			$loader = preg_replace_callback("/(t\.sub_scripts_to_load=\s*)\[([^\]]*)\];/",
						function($matches) {
							return $this->replace_scripts( 'sub_script_list', $matches[1], $matches[2]);
						},
						$loader
			);
