...
class Postgres extends ADODB_base {
	...
	function getDatabases($currentdatabase = NULL) {
		...
		if (isset($conf['owned_only']) && $conf['owned_only'] && !$this->isSuperUser()) {
			...
			$clause = " AND (pg_has_role({$username_q}::name,pr.rolname,'USAGE') OR pg_has_role({$before_underscore_q}::name,pr.rolname,'USAGE') OR pg_has_role({$username_q}::name,pr.rolname,'USAGE') OR pg_has_role({$before_underscore_q}::name,pr.rolname,'USAGE'))";
		}
		else $clause = '';
