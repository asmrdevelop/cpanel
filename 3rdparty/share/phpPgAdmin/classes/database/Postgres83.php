...
class Postgres83 extends Postgres84 {
	...
	function getDatabases($currentdatabase = NULL) {
		...
		if (isset($conf['owned_only']) && $conf['owned_only'] && !$this->isSuperUser()) {
			...
			$this->clean($username);
			$split_user = explode('_', $username);
			/* for cPanel temp session */
			if ($split_user[0] == "cpses" && $_ENV['REAL_DBOWNER']) {
				$split_user[0] = $this->clean($_ENV['REAL_DBOWNER']);
			}
			$username_q = pg_escape_literal($username);
			$before_underscore_q = pg_escape_literal($split_user[0]);
			$clause = " AND (pg_has_role({$username_q},pr.rolname,'USAGE') OR pg_has_role({$before_underscore_q},pr.rolname,'USAGE') OR pg_has_role({$username_q},pr.rolname,'USAGE') OR pg_has_role({$before_underscore_q},pr.rolname,'USAGE'))";
		}
		else $clause = '';
		...
		$sql = "
			...
			FROM pg_catalog.pg_database pdb LEFT JOIN pg_catalog.pg_roles pr ON (pdb.datdba = pr.oid)
				LEFT JOIN pg_catalog.pg_auth_members pam ON (pam.roleid = pr.oid)
				LEFT JOIN pg_catalog.pg_roles pr ON (pam.member = pr.oid)
			WHERE true
				{$where}
				{$clause}
			{$orderby}";
