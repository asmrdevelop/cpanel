...

def ScriptURL(target, web_page_url=None, absolute=False, force_vhost=False):
    """target - scriptname only, nothing extra
    web_page_url - the list's configvar of the same name
    absolute - a flag which if set, generates an absolute url
    force_vhost - a flag which if set, prevents cpanel relative links from being used when called via cpsrvd
    """
    is_cpanel = os.environ.get('CPANEL')
    is_apache_https = os.environ.get('REQUEST_SCHEME') is not None and os.environ.get('REQUEST_SCHEME').lower() == 'https'
    if web_page_url is None:
        web_page_url = mm_cfg.DEFAULT_URL_PATTERN % get_domain()
        if web_page_url[-1] <> '/':
            web_page_url = web_page_url + '/'
    if is_apache_https and web_page_url.startswith('http://'):
        web_page_url = web_page_url.replace('http', 'https', 1)
    fullpath = os.environ.get('REQUEST_URI')
    if fullpath is None:
        fullpath = os.environ.get('SCRIPT_NAME', '') + \
                   os.environ.get('PATH_INFO', '')
    baseurl = urlparse.urlparse(web_page_url)[2]
    if is_cpanel and not force_vhost:
