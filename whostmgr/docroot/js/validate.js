...
var match_validators = {
    ...
    "emailaddress": /^[^\@]\@[^\.]\./,
    "emailchars": /^[a-z0-9\_\-\@\.]+$/,
    /* http://www.remote.org/jochen/mail/info/chars.html*/ "email_localpart_chars": /^[a-z0-9\_\-\.]+$/,
    /* http://www.remote.org/jochen/mail/info/chars.html*/ "email_localpart_chars_cap": /^[A-Za-z0-9\_\-\.]+$/,
    ...
    "fqdn_domain": /^((\*\.|[a-zA-Z0-9])([a-zA-Z0-9\-]{0,61}[a-zA-Z0-9])?\.){2}[a-zA-Z]{2,63}$/,
    "fullipaddr": /^(\d{1,3}\.){3}\d{1,3}$/,
    "fullipv6addr": CPANEL.inet6.isValid,
    ...
    for (var i = 0; i < validators[formId].length; i++) {
        ...
        switch (validators[formId][i].method) {
            case "match":
                ...
                if (matchkey == null) {
                    alert("Unknown regex key used for match: " + validators[formId][i].vkey + ". Valid keys are: " + validate_listmatchers());
                }
                if (validators[formId][i].vkey === "fullipv6addr" && typeof (matchkey) === "function") {
                    is_valid = matchkey.call(CPANEL.inet6, validators[formId][i].Els[0].value);
                } else if (!validators[formId][i].Els[0].value.match(matchkey)) {
                    is_valid = 0;
                }
                break;
            case "nomatch":
        ...
        if (!is_valid) {
            ...
            if (!validateFocusEl) {
                validateFocusEl = validators[formId][i].Els[0];
            }
            make_elements_valid_status(validators[formId][i].Els, 0, element_statuses);
            form_valid = 0;
        } else {
...
function _deactivate_validator(formId, formValidators, validatorId) {
...
function remove_validator(config) {
...
function reposition_validator_img(imgElCon) {
    ...
    try {
        parentElPos = parentEl.getAttribute("validatorposition");
    } catch (e) {
    ...
    if ((parentElPos && parentElPos == "right") || parentEl.tagName == "SELECT" || parentEl.tagName == "TEXTAREA" || parentEl.type == "file") {
        ...
        imgElCon.style.left = (parentReg.left + parentEl.offsetWidth + 2) + "px";
    }
