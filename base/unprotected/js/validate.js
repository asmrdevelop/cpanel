...
var match_validators = {
	'fullemailaddress': /^([a-zA-Z0-9\_\'\+\*\$\%\^\&\!\.\-])+\@(([a-zA-Z0-9\-])+\.)+([a-zA-Z0-9:]{2,4})+$/,
    'emailaddress': /^[^\@]\@[^\.]\./,
    'emailchars': /^[a-z0-9\_\-\@\.]+$/,   /*http://www.remote.org/jochen/mail/info/chars.html*/
    'email_localpart_chars': /^[a-z0-9\_\-\.]+$/,   /*http://www.remote.org/jochen/mail/info/chars.html*/
...
function do_validate(formId,noalert,noupdate,check_element) {
    ...
    for(var i=0;i<validators[formId].length;i++) {
        ...
        switch( validators[formId][i].method ) {
            ...
            case 'nomatch':
                var matchkey = ( typeof(validators[formId][i].vkey) == 'string' ? match_validators[validators[formId][i].vkey] : validators[formId][i].vkey);
                if (matchkey == null) { alert('Unknown regex key used for nomatch: ' + validators[formId][i].vkey + '. Valid keys are: ' + validate_listmatchers() ); }
                if (validators[formId][i].Els[0].value.match(matchkey)) {
                    is_valid = 0;
                }
                break;
            case 'noeqtxt':
                if (validators[formId][i].Els[0].value == validators[formId][i].vkey) {
                    is_valid = 0;
                }
                break;
            case 'minlength':
                if (validators[formId][i].Els[0].value.length < validators[formId][i].vkey) {
                    is_valid = 0;
                }
                break;
            case 'maxlength':
                ...
                break;
            case 'Eleq':
                if (validators[formId][i].Els[0].value != validators[formId][i].Els[1].value) {
                    is_valid = 0;
                }
                break;
            case 'func':
...
function reposition_validator_img(imgElCon) {
    ...
    if (parentEl.tagName == "SELECT" || parentEl.tagName == "TEXTAREA" || parentEl.type == 'file') {
