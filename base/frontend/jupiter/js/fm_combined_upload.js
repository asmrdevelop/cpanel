...
/*
# filemanager_upload.js                      Copyright(c) 1997-2020 cPanel, L.L.C.
#                                                           All rights reserved.
...
/* jshint -W098 */
window.uploadService = (function(PAGE, $, LOCALE, YAHOO, sprintf, toHumanSize) {
/* jshint +W098 */
    "use strict";
    ...
    var uploadFree = PAGE.startUploadFree;
    var maxUploadBytes = PAGE.maxUploadBytes;
    var userHasQuota = PAGE.userHasQuota;
    ...
    function updateUploadFree(newValue, newValueText) {
        file_upload_remain.textContent = newValueText;
        uploadFree = parseInt(newValue, 10);
    }
        ...
        self.request.onload = function(event) {
            ...
            if (payload) {
                ...
                if (payload.data.diskinfo) {
                    updateUploadFree(payload.data.diskinfo.file_upload_remain, payload.data.diskinfo.file_upload_remain_humansize);
                }
            } else if (event.target.status === HTTP_PAYLOAD_TOO_LARGE) {
                message = LOCALE.maketext("The file’s size exceeds your account’s remaining disk usage quota. Because of this, you cannot upload this file.") + " " + LOCALE.maketext("Delete some files, or contact your hosting provider to request an increase to your account’s quota.");
            } else if (event.target.status > 399) {
                message = LOCALE.maketext("The upload failed. The server indicated [asis,HTTP] error [numf,_1] ([_2]).", event.target.status, event.target.statusText);
            }
        ...
        iframe.onload = function() {
            ...
            if (jdata && jdata.cpanelresult) {
                ...
                if (payload.data && payload.data[0].diskinfo) {
                    updateUploadFree(payload.data[0].diskinfo.file_upload_remain, payload.data[0].diskinfo.file_upload_remain_humansize);
                }
            }
    ...
    function uploadfile(file) {
        ...
        if ((supports_XHR2() && supports_FileAPI()) ) {
            ...
            var limits = [ maxUploadBytes ];
            if (userHasQuota) {
                limits.push(uploadFree);
            }

            var limit = Math.min.apply(null, limits);

            if (theFile.size > limit) {
                var phrases = [
                    LOCALE.maketext("“[_1]” requires [format_bytes,_2] of disk space, but you can only upload up to [format_bytes,_3].", theFile.name, theFile.size, limit),
                ];

                if (limit === maxUploadBytes) {
                    phrases.push( LOCALE.maketext("Because of this limitation, you cannot upload this file.") );
                } else {
                    phrases.push( LOCALE.maketext("Because of this limitation, you cannot currently upload this file.") );
                    phrases.push( LOCALE.maketext("Delete some files, or contact your hosting provider to request an increase to your account’s quota.") );
                }
