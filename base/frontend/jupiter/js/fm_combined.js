...
/* eslint-disable camelcase, strict, no-unused-vars, no-useless-escape, no-undef */

var isWpSquared = is_server_type_wp_squared || false;
...
function FileOpCallBack(o) {
    ...
    if (o.argument.fileop === "chmod") {
        ...
        // This API does not return failure the usual way with:
        //
        //  root.cpanelresult.event.result === 0
        //
        // We have to look at the individual data elements to
        // find out what fails.

        // Update the state for the files whose permissions
        // were successfully modified.
        var hasErrors = 0;
        var errors = [];
        var length = root.cpanelresult.data.length;
        for (var index = 0; index < length; index++) {
            var item = root.cpanelresult.data[index];

            // The src path is rooted, we want one relative to the currently
            // selected directory.
            var relativePath = item.src.replace(new RegExp("^" + cwd + "/"), "");

            // Find the row with the same file as the
            // item has.
            var found = 0;
            var r = 0;
            while ( !found && r < rows.length ) {
                var cur_row = rows[r];
                var oRecord = filesDataTable.getRecord(cur_row);
                if (oRecord.getData("file") === relativePath) {

                    // This is the file, see if the operation was successful or not.
                    if (item.result !== 0) {
                        filesDataTable.updateCell(oRecord, "nicemode", o.argument.metadata);
                    } else {
                        hasErrors = 1;
                        errors.push(" • The system failed to change the permissions for " + relativePath + " with the error: " + item.err);
                    }
        ...
        // Report the errors if there were some.
        if (hasErrors) {

            // Let the screen update before we alert for
            // any of the successes from above.
            setTimeout(function() {
                alert("FileOp Failure: \n" + errors.join("\n"));
            }, 0);
        }
...
var cancel_btn_txt = LOCALE.maketext("Cancel");
...
var panels = {
    ...
    newfile: {
        ...
        buttons: [{
            text: LOCALE.maketext("Create New File"),
            isDefault: true,
        }, {
            text: cancel_btn_txt,
        }],
    ...
    newfolder: {
        ...
        buttons: [{
            text: LOCALE.maketext("Create New Folder"),
            isDefault: true,
        }, {
            text: cancel_btn_txt,
        }],
    ...
    "emptytrash": {
        ...
        buttons: [{
            text: LOCALE.maketext("Empty Trash"),
            isDefault: true,
        }, {
            text: cancel_btn_txt,
        }],
    ...
    "delete": {
        ...
        submit: handleDelete,
        buttons: [{
            text: LOCALE.maketext("Delete Files"),
            isDefault: true,
        }, {
            text: cancel_btn_txt,
        }],
    ...
    "restore": {
        ...
        buttons: [{
            text: LOCALE.maketext("Restore Files"),
            isDefault: true,
        }, {
            text: cancel_btn_txt,
        }],
    ...
    extract: {
        ...
        buttons: [{
            text: LOCALE.maketext("Extract Files"),
            isDefault: true,
        }, {
            text: cancel_btn_txt,
        }],
    ...
    compress: {
        ...
        buttons: [{
            text: LOCALE.maketext("Compress Files"),
            isDefault: true,
        }, {
            text: cancel_btn_txt,
        }],
    ...
    copy: {
        ...
        submit: handleCopy,
        buttons: [{
            text: LOCALE.maketext("Copy Files"),
            isDefault: true,
        }, {
            text: cancel_btn_txt,
        }],
    ...
    move: {
        ...
        submit: handleMove,
        buttons: [{
            text: LOCALE.maketext("Move Files"),
            isDefault: true,
        }, {
            text: cancel_btn_txt,
        }],
    ...
    rename: {
        ...
        submit: handleRename,
        buttons: [{
            text: LOCALE.maketext("Rename File"),
            isDefault: true,
        }, {
            text: cancel_btn_txt,
        }],
    ...
    chmod: {
        ...
        submit: handleChmod,
        buttons: [{
            text: LOCALE.maketext("Change Permissions"),
            isDefault: true,
        }, {
            text: cancel_btn_txt,
        }],
