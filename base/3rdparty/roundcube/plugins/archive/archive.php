...
class archive extends rcube_plugin
{
    ...
    function config_get($args) {
        // If it isn't the one we wanna hijack, return
        if( $args['name'] !== 'archive_mbox' ) return $args['result'];
        // Also return if the current result is set to something, as we don't wanna override the user
        if( !empty($args['result']) ) return $args['result'];
        $args['result'] = 'INBOX.Archive';
        return $args;
    }
