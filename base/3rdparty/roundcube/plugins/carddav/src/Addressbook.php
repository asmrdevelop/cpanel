...
class Addressbook extends rcube_addressbook
{
    ...
    public function insert($save_data, $check = false)
    {
        ...
        try {
            ...
            $uri = preg_replace( '/^http[s]?:\/\/[a-zA-Z0-9.-]+:?[0-9]*/', "", $uri );
             ...
             */
            $contact = $db->get(['cuid' => (string) $vcard->UID, "abook_id" => $this->id], ['id'], 'contacts') ?? [ null ];
            $contact = $rows[0] ?? null;
            if (!isset($contact)) {
                /** @var array{id: string} */
     ...
     */
    public function resync( bool $force = false ): int
    {
        $infra = Config::inst();
        $logger = $infra->logger();
        $db = $infra->db();
        ...
        try {
            ...
            /* Blank the token if you wanna force it. */
            if($force) {
                $this->config['sync_token'] = '';
            }
