...
function rcube_calendar_ui(settings)
{
    ...
    var add_attendee = function(data, readonly, before)
    {
      ...
      var attendee_email = data.email;
      var lencal = $('#edit-calendar').val().length || 0;

      // If we're using something other than the Default calendar (we are using the cPanel CalDAV calendar), disable invite sending by default, because cpdavd handles this for us when needed.
      if ( lencal > 3 ) {
        data.noreply = true;
      }
