...
class database_driver extends calendar_driver
{
    ...
    public function remove_event($event, $force = true)
    {
        if (!empty($this->calendars)) {
            ...
            if (isset($event['recurrence']) || isset($event['recurrence_id'])) {
                $master   = isset($event['recurrence_id']) ? $this->get_event(array('id' => $event['recurrence_id'])) : $event;
            ...
            default:  // 'all' is default
                ...
                if ( isset($event['attachments']) ) {
                    foreach ($event['attachments'] as $attachment) {
                        $this->remove_attachment($attachment['id'], $id);
                    }
                }
