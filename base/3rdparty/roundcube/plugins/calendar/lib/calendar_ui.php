...
class calendar_ui
{
    ...
    {
        $categories    = $this->cal->get_default_driver()->list_categories();
        $calendars     = $this->cal->get_calendars();
        $js_categories = [];
    ...
    function calendar_list($attrib = [], $js_only = false)
    {
        ...
        $tree      = true;
        $calendars = [];
        $calendars = $this->cal->get_calendars(0, $tree);
    ...
    public function calendar_list_item($id, $prop, &$jsenv, $activeonly = false)
    {
        ...
        if (empty($prop['virtual'])) {
            ...
            $prop['alarms']      = $this->cal->get_default_driver()->alarms;
            $prop['attendees']   = $this->cal->get_default_driver()->attendees;
            $prop['freebusy']    = $this->cal->get_default_driver()->freebusy;
            $prop['attachments'] = $this->cal->get_default_driver()->attachments;
            $prop['undelete']    = $this->cal->get_default_driver()->undelete;
    ...
    function category_select($attrib = [])
    {
        ...
        $select = new html_select($attrib);
        $select->add('---', '');
        foreach (array_keys((array) $this->cal->get_default_driver()->list_categories()) as $cat) {
              $select->add($cat, $cat);
        }
    ...
    {
        return $this->cal->lib->alarm_select($attrib, $this->cal->get_default_driver()->alarm_types, $this->cal->get_default_driver()->alarm_absolute);
    }
    ...
    function calendarform($attrib)
    {
        ...
        if (!empty($this->cal->get_default_driver()->alarms)) {
        ...
        return html::tag('form', $attrib + ['action' => '#', 'method' => 'get', 'id' => 'calendarpropform'],
            $this->cal->get_default_driver()->calendar_form($this->action, $this->calendar, $formfields)
        );
    }
