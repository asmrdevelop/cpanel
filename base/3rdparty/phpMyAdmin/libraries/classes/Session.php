...
class Session
{
    ...
    /**
     * Return a unique name (for user instance) that can be used for session_name()
     * cPanel case FB-45311
     *
     * CPANEL-36082: Also remove colons in IPv6 addresses.
     */
    public static function cpGetSessionName(){
        $user = posix_getpwuid(posix_getuid());
        $ip = $_SERVER['SERVER_ADDR'];
        return 'phpMyAdmin'.$user['name'].str_replace(':','',str_replace('.','',$ip));
    }
    ...
    public static function setUp(Config $config, ErrorHandler $errorHandler): void
    {
        ...
        } elseif (! empty(ini_get('session.auto_start')) && session_name() !== Session::cpGetSessionName() && ! empty(session_id())) {
        ...
        $httpCookieName = Session::cpGetSessionName();
