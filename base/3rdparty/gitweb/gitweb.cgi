...
our $GIT = "/usr/local/cpanel/3rdparty/bin/git";
...
our $logo_url = "https://go.cpanel.net/Gitweb";
our $logo_label = "gitweb documentation";
...
sub evaluate_path_info {
	if ( defined $input_params{'project'} ) {
		my $project = $input_params{'project'};
		$project =~ s,/+$,,;
...
sub run_request {
	...
	# This style of jailing only works if projectroot is '/' because all working directory in gitweb is
	# actually $projectroot/$project. This works for cPanel version of gitweb, but doesn't work for some
	# tests where $projectroot is moved.
	if ( $projectroot eq '/' && $input_params{project} ) {
		my $p = $input_params{'project'};
		$p = '/' . $p unless $p =~ m:^/:; # needs this, otherwise, safedir simply adds $HOME in front
		$p = safe_dir($p);
		$p =~ s:^/::; # no / in front
		$input_params{'project'} = $p;
	}
...
sub safe_dir {
	...
	# The directory must be under home directory and does not contain
	# a link to somewhere else. If these conditions can not be met,
	# just return the home directory. 
	if ( $dir !~ /^$abshomedir/ ) {
	        $dir = $abshomedir;
	}
...
sub print_nav_breadcrumbs {
	...
	if (defined $project) {
		...
		$projectbasename = pop @dirname if $projectbasename eq '.git';
...
EOF
	...
	if (defined $logo) {
		print $cgi->img({-src => esc_url($logo),
				-width => 72, -height => 27,
				-alt => "git",
				-class => "logo"});
	}
...
sub git_footer_html {
	...
	if (defined $project) {
		...
		if (defined $descr) {
			print "<div class=\"page_footer_custom\">";
			print esc_html($descr) . ' ';
			print "</div>";
		}

		print "<div class=\"float-right\">";
		print $cgi->a({-href => "https://go.cpanel.net/Gitweb",
				-class => "doc-link",
				-target => "gitWebDoc"},
