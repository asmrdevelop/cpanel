...
Cpanel::DAV::LockManager - Provides support for WebDAV locking


=head1 VERSION

This document describes Cpanel::DAV::LockManager version 0.030
...
=head1 SYNOPSIS
    ...
    my $mgr = Cpanel::DAV::LockManager->new();
...
The C<Cpanel::DAV::LockManager> class provides the ability to handle resource locking
...
Create a new C<Cpanel::DAV::LockManager> object to manage locking.
...
specification RFC 4918, two values are accepted I<0>,<1> and I<infinity>. A value
of I<0> applies only to the specified resource. A value of |<1> applies only
to a source, and its immediate childern. The depth of I<infinity>
...
Returns a C<Cpanel::DAV::Lock> object describing the lock on success and C<undef>
on failure.
...
C<Cpanel::DAV::Lock> object representing the lock, or C<undef> if none is found.
...
array of C<Cpanel::DAV::Lock> objects representing the locks.
