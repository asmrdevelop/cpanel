package Cpanel::DAV::LockManager::DB;
...
sub new {
    ...
    my $self = bless { 'db' => DBI->connect( $dsn, '', '' ) }, $class;
...
sub get {
    ...
    return $row ? Cpanel::DAV::Lock->reanimate($row) : undef;
}
...
sub list_descendants {
    ...
    if ( $path eq '/' ) {
        return map { Cpanel::DAV::Lock->reanimate($_) } @{ $self->{'db'}->selectall_arrayref( q(select * from lock where path != '/'), { 'Slice' => {} } ) };
    }

    my $sql = q/select * from lock where path like ?/;

    return map { Cpanel::DAV::Lock->reanimate($_) } @{ $self->{'db'}->selectall_arrayref( $sql, { 'Slice' => {} }, "$path/%" ) };
}

#
# Given an instance of Cpanel::DAV::Lock, update any entries in the
...
sub update {
    ...
    $self->{'db'}->do(
        q/update lock set expiry = ? where path = ?/, {},
        $lock->expiry,
        $lock->path
    );
...
#
# Insert the data passed in an instance of Cpanel::DAV::Lock into the
...
#
# Given a Cpanel::DAV::Lock object, the database record which contains the
# corresponding path.
#
sub remove {
    my ( $self, $lock ) = @_;
