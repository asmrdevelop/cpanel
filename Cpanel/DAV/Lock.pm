...
# cpanel - Cpanel/DAV/Lock.pm                      Copyright 2022 cPanel, L.L.C.
#                                                           All rights reserved.
# copyright@cpanel.net                                         http://cpanel.net
# This code is subject to the cPanel license. Unauthorized copying is prohibited

package Cpanel::DAV::Lock;
...
sub new {
    ...
    die( 'Creator contains invalid characters:' . $hash->{'creator'} ) unless $hash->{'creator'} =~ /^[A-Za-z_.][-A-Za-z0-9_.]*(?:@[a-z0-9_.][-a-z0-9_.]*)?$/;
    ...
    if ( defined $hash->{'depth'} ) {
        die('Depth is a non-RFC 4918 value') unless $hash->{'depth'} =~ /^(?:[01]|infinity)$/i;
    ...
    if ( $hash->{'uuid'} ) {
        ...
        $obj->{'uuid'} = Cpanel::DAV::UUID::generate( @{$hash}{qw/path owner/} );
    }
