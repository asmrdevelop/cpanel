package Cpanel::Validate::DomainCreation::Parked;

# cpanel - Cpanel/Validate/DomainCreation/Parked.pm
...
sub init {
    ...
    $self->add_optional_arguments(qw( force main_userdata_ref target_domain validation_context ));

    delete @{$system_provided_arguments_hash}{qw( domain )};
    ...
    $system_provided_arguments_hash->{'target_domain'} = $user_main_domain;

    if ( !length $system_provided_arguments_hash->{'target_domain'} ) {
    ...
    $system_provided_arguments_hash->{'target_domain'} = Cpanel::Validate::Domain::Normalize::normalize( $system_provided_arguments_hash->{'target_domain'} );

    die Cpanel::Exception::create( 'InvalidParkedTarget', 'You cannot park a domain on top of itself!' ) if $user_main_domain eq $user_provided_argument_hash->{'domain'};

    # Don't display these in $self->get_validation_arguments* as they don't need to be passed in
    $self->add_internal_arguments(qw( limit_name limit_display_name limit_current_count current_limit feature_name target_domain ));
    ...
    $self->add_validation_components(
        ...
        Cpanel::Validate::Component::Domain::InvalidName->new( %combined_input, 'domain' => $user_main_domain ),
        Cpanel::Validate::Component::Domain::ContainsReservedSubdomain->new( %combined_input, only_if_exists => 1 ),
        ...
        Cpanel::Validate::Component::Domain::UserdataDoesNotExist->new( %combined_input, 'domain' => $user_main_domain ),
        $self->has_root()       ? () : Cpanel::Validate::Component::Domain::OwnedByAnotherUser->new(%combined_input),
        $self->is_whm_context() ? () : Cpanel::Validate::Component::Account::MissingFeature->new( %combined_input, 'feature_name' => 'parkeddomains' ),
        $self->is_whm_context() ? () : Cpanel::Validate::Component::Account::OverLimit->new(
            %combined_input,
            'limit_name'          => 'MAXPARK',
            'limit_display_name'  => 'parked domains',
            'limit_current_count' => scalar @{ $userdata_main_ref->{'parked_domains'} }
        ),
        ...
        ( $self->is_whm_context() || $force ) ? () : Cpanel::Validate::Component::Domain::DomainRegistration->new(%combined_input),
    );

    # This comes after the components to avoid these parameters being set to required
    $self->add_optional_arguments(qw( target_domain force ));
