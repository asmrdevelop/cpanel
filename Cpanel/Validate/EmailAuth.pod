...
# cpanel - Cpanel/Validate/EmailAuth.pod           Copyright 2022 cPanel, L.L.C.
#                                                           All rights reserved.
...
Cpanel::Validate::EmailAuth - Utilities for validating email addresses used as usernames


=head1 SYNOPSIS

    use Cpanel::Validate::EmailAuth;

    if ( !Cpanel::Validate::EmailAuth::is_valid( $user ) ) {
...
Return true if C<$user> is a valid email address as used in username fields in
cPanel. This definition is somewhat different than the definition from the
relevant RFCs.

The local part and domain of an email username may be separated by one of the
four characters [+:%@]. If there are more than one of these characters, the
address is invalid.
...
lowercases any letters in the domain portion. The final separator character is
also converted to '@' (unless a '@' already exists in the string). Also removes
'\0' characters.  Does not try to force a given string to be valid. Returns the
normalized string.
...
domain in a consistent fashion. If C<$address> contains a '@' character, the
string is split on the last '@' character. Otherwise it is split on the last
separator character from the set [+:%].
...
Tests the supplied string to see if it contains one of the defined email separators.
This function serves to put the knowledge of the separator strings in one place.

=head1 CONFIGURATION AND ENVIRONMENT

C<Cpanel::Validate::EmailAuth> requires no configuration.
