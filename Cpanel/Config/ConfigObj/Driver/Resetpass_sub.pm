package Cpanel::Config::ConfigObj::Driver::Resetpass_sub;

# cpanel - Cpanel/Config/ConfigObj/Driver/Resetpass_sub.pm
...
use Cpanel::Config::ConfigObj::Driver::Resetpass_sub::META ();
*VERSION = \$Cpanel::Config::ConfigObj::Driver::Resetpass_sub::META::VERSION;
...
sub status {
    ...
    $current_setting = $cpconf_ref->{'resetpass_sub'};
...
sub _configFeature {
    ...
    my $result = Whostmgr::TweakSettings::set_value( 'Main', 'resetpass_sub', $new_value, $self->status(), 1 );
    if ( !$result ) {
        $interface->set_error( Cpanel::Config::ConfigObj::E_ERROR, "Could not run Reset Subaccount Password post-action.\n" );
        return;
    }
