...
sub set {
    ...
    for (
        ...
        binip error ip intformat hexformat mask last_bin prefix is_prefix)
      ) {
        delete( $self->{$_} );
    }
...
sub last_bin {
    ...
    if ( $self->is_prefix() ) {
        $last = ip_last_address_bin(
            $self->binip(), $self->prefixlen(),
            $self->version()
        );
    }
    else {
...
sub find_prefixes {
    ...
    my @list = ip_range_to_prefix( $self->binip(), $self->last_bin(), $self->version() );
...
sub binadd {
    ...
    my $new = Cpanel::CPAN::Net::IP->new( ip_bintoip( $ip, $self->version() ) ) or return;
...
sub aggregate {
    ...
    return ( Cpanel::CPAN::Net::IP->new($r) );
}
...
sub ip_range_to_prefix {
    ...
    while ( ip_bincomp( $binip, 'le', $endbinip ) == 1 ) {
        ...
        # Find Prefix length
        $len = ( ip_iplengths($ip_version) ) - ip_get_prefix_length( $binip, $current );
...
sub ip_check_prefix {
    ...
    if ( $len > length($binip) ) {
        $ERROR = "Prefix length $len is longer than IP address (" . length($binip) . ")";
        $ERRNO = 170;
        return;
    }
...
sub ip_normalize {
    ...
    my ( $len, $ip, $ip2, $curr_bin, $addcst, $clen );
