package Cpanel::CPAN::Hash::Merge;
...
my $clone_behavior = 0;                               # Never use Clone this way

sub set_behavior {
    my $value = uc(shift);
    if ( !exists $behaviors->{$value} ) {
        require Carp;
        Carp::carp( 'Behavior must be one of : ' . join( ', ', keys %{$behaviors} ) );
        return;
    }
    ...
    foreach my $left (@required) {
        foreach my $right (@required) {
            if ( !exists $matrix->{$left}->{$right} ) {
                require Carp;
                Carp::carp("Behavior does not specify action for '$left' merging with '$right'");
                return;
            }
        }
    }
...
sub _merge_hashes {
    ...
    if ( ref $left ne 'HASH' || ref $right ne 'HASH' ) {
        require Carp;
        Carp::carp('Arguments for _merge_hashes must be hash references');
        return;
    }
    ...
    foreach my $leftkey ( keys %$left ) {
        if ( exists $right->{$leftkey} ) {
            ...
            $newhash{$leftkey} = $clone_behavior ? _my_clone( $left->{$leftkey} ) : $left->{$leftkey};
        }
    }

    foreach my $rightkey ( keys %$right ) {
        if ( !exists $left->{$rightkey} ) {
            $newhash{$rightkey} = $clone_behavior ? _my_clone( $right->{$rightkey} ) : $right->{$rightkey};
        }
    }
...
sub _hashify {
    ...
    if ( ref $arg eq 'HASH' ) {
        require Carp;
        Carp::carp('Arguement for _hashify must not be a HASH ref');
        return;
    }
...
Cpanel::CPAN::Hash::Merge - Merges arbitrarily deep hashes into a single hash

=head1 SYNOPSIS

    use Cpanel::CPAN::Hash::Merge qw( merge );
    my %a = (
		'foo'    => 1,
	    'bar'    => [ qw( a b e ) ],
	    'querty' => { 'bob' => 'alice' },
	...
	Cpanel::CPAN::Hash::Merge::specify_behavior(
	    {
			'SCALAR' => {
				'SCALAR' => sub { $_[1] },
				'ARRAY'  => sub { [ $_[0], @{$_[1]} ] },
			...
			'HASH' => {
				'SCALAR' => sub { $_[1] },
				'ARRAY'  => sub { [ values %{$_[0]}, @{$_[1]} ] },
				'HASH'   => sub { Cpanel::CPAN::Hash::Merge::_merge_hashes( $_[0], $_[1] ) },
			},
		},
		'My Behavior',
	);
...
Cpanel::CPAN::Hash::Merge merges two arbitrarily deep hashes into a single hash.  That
...
values when keys are conflicting, Cpanel::CPAN::Hash::Merge provides several preset
...
Sets how the data cloning is handled by Cpanel::CPAN::Hash::Merge.  If this is true,
...
Returns the behavior that is currently in use by Cpanel::CPAN::Hash::Merge.

=item specify_behavior( <hashref>, [<name>] )

Specify a custom merge behavior for Cpanel::CPAN::Hash::Merge.  This must be a hashref
