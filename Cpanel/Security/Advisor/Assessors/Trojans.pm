...
# cpanel - Cpanel/Security/Advisor/Assessors/Trojans.pm
...
sub _check_for_NCOM_rootkit {
    ...
    if (@bad_libs) {
        $self->add_bad_advice(
            ...
            'suggestion' => $self->_lh->maketext(
                'Check the following for more information: [output,url,_1,We Live Security More Information,_2,_3]',    # FIXME: Neither "Server Status" nor the "Operation Windigo" white paper cover NCOM.
                'https://go.cpanel.net/checkyourserver',
                'target',
                '_blank'
            ),
...
sub _check_for_jynx2_rootkit {
    ...
    if ( scalar @found_jynx2_files > 0 ) {
        $self->add_bad_advice(
            ...
            'suggestion' => $self->_lh->maketext(
                'Check the following to determine if this server is compromised [output,url,_1,Determine your Systems Status,_2,_3]',    # FIXME: same here
                'https://go.cpanel.net/checkyourserver',
                'target',
                '_blank'
            ),
...
sub _check_sha1_sigs_libkeyutils {
    ...
    if (@trojaned_lib) {
        $self->add_bad_advice(
            ...
            'text'       => $self->_lh->maketext( "Libkey rootkit check: The following suspicious [numerate,_1,file was,files were] found that [numerate,_1,matches,match] a specific SHA-1 checksum which could indicate a root level compromise: [list_and,_2]", scalar @trojaned_lib, \@trojaned_lib ),
            'suggestion' => $self->_lh->maketext(
                'Check the following for more information: [output,url,_1,We Live Security,_2,_3]',
...
sub _check_sha1sum {
    ...
    my $fh;
    open $fh, '<', $file or do {
        warn "Cannot open “$file”";    # FIXME: It may be important to note that the file couldn't be opened.
    };
