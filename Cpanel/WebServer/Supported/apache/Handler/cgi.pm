package Cpanel::WebServer::Supported::apache::Handler::cgi;

# cpanel - Cpanel/WebServer/Supported/apache/Handler/cgi.pm
...
Cpanel::WebServer::Supported::apache::Handler::cgi
...
=head1 SYNOPSIS
    ...
    my $cgi = Cpanel::WebServer::Supported::apache::Handler->new( 'type' => 'cgi', 'lang_obj' => $lang_obj, 'webserver' => $apache );

    my $htaccess_str = $cgi->get_htaccess_string();

    my $conf_str = $cgi->get_config_string();

    my $default_str = $cgi->get_default_string();

    $cgi->unset_lang_handler();   # deletes /cgi-sys wrapper
    $cgi->set_lang_handler();     # creates /cgi-sys wrapper
...
=head2 Cpanel::WebServer::Supported::apache::Handler::cgi-E<gt>new()
...
See I<Cpanel::WebServer::Supported::apache::Handler::base::init()> for
required arguments.

=head3 Returns

A blessed reference to a
Cpanel::WebServer::Supported::apache::Handler::cgi object.
...
sub new {
    ...
    my $self = bless( {}, $class );
    $self->init( \%args );
    $self->module_check_or( [qw(mod_cgi mod_cgid)] );
    $self->sapi_check('cgi');
    return $self;
}
...
sub get_config_string {
    ...
    my $str     = <<"EOF";
# CGI configuration for $package
<IfModule actions_module>
  Action $type /cgi-sys/$package
</IfModule>
EOF
    return $str;
}
...
sub set_lang_handler {
    ...
    my $wrapper     = "$CPANEL_WRAPPERS_DIR/$pkg";
    ...
    if ( -e $wrapper ) {
    ...
    elsif ( grep { $pkg eq $_ } @{ $lang_obj->get_lang->get_installed_packages() } ) {
        if ( !unlink $cgisys_file ) {
            if ( exists $!{'ENOENT'} && !$!{'ENOENT'} ) {
                die Cpanel::Exception::create( 'IO::UnlinkError', [ 'path' => $cgisys_file, 'error' => $! ] );
            }
        ...
        my $sapi   = $lang_obj->get_sapi_info('cgi');
        my $binary = $sapi->{'path'};
...
Cpanel::Exception, Cpanel::FileUtils, Cpanel::SafetyBits::Chown, and
